...
instance AWSPager ListUsers where
  page rq rs
    | stop (rs ^. lursPaginationToken) = Nothing
    | stop (rs ^. lursUsers) = Nothing
    | otherwise =
      Just $ rq & luPaginationToken .~ rs ^. lursPaginationToken
...
instance ToHeaders ListUsers where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AWSCognitoIdentityProviderService.ListUsers" :: ByteString),
