...
mkWorkGroup
    ...
    -> WorkGroup
mkWorkGroup name
  = WorkGroup'{name, configuration = Core.Nothing,
...
instance Core.FromJSON WorkGroup where
        parseJSON
          = Core.withObject "WorkGroup" Core.$
              \ x ->
                WorkGroup' Core.<$>
                  (x Core..: "Name") Core.<*> x Core..:? "Configuration" Core.<*>
                    x Core..:? "CreationTime"
