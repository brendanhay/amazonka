...
{-# INLINEABLE glbLoadBalancerName #-}
...
instance Core.AWSRequest GetLoadBalancer where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 GetLoadBalancerResponse' Core.<$>
                   (x Core..:? "loadBalancer") Core.<*> Core.pure (Core.fromEnum s))
...
mkGetLoadBalancerResponse responseStatus
  = GetLoadBalancerResponse'{loadBalancer = Core.Nothing,
                             responseStatus}
...
{-# INLINEABLE glbrrsLoadBalancer #-}
{-# DEPRECATED loadBalancer "Use generic-lens or generic-optics with 'loadBalancer' instead"  #-}
...
{-# INLINEABLE glbrrsResponseStatus #-}
