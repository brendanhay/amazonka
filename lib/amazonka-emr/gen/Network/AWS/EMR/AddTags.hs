...
data AddTags = AddTags' {_atResourceId :: !Text, _atTags :: ![Tag]}
...
addTags pResourceId_ =
  AddTags' {_atResourceId = pResourceId_, _atTags = mempty}
...
instance ToHeaders AddTags where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("ElasticMapReduce.AddTags" :: ByteString),
...
instance ToJSON AddTags where
  toJSON AddTags' {..} =
    object
      ( catMaybes
          [Just ("ResourceId" .= _atResourceId), Just ("Tags" .= _atTags)]
      )
