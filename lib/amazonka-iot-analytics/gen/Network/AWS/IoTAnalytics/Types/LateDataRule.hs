...
mkLateDataRule
    :: Types.LateDataRuleConfiguration -- ^ 'ruleConfiguration'
    -> LateDataRule
...
{-# INLINEABLE ldrRuleConfiguration #-}
{-# DEPRECATED ruleConfiguration "Use generic-lens or generic-optics with 'ruleConfiguration' instead"  #-}
...
instance Core.FromJSON LateDataRule where
        parseJSON
          = Core.withObject "LateDataRule" Core.$
              \ x ->
                LateDataRule' Core.<$>
                  (x Core..: "ruleConfiguration") Core.<*> x Core..:? "ruleName"
