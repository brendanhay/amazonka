...
cancelMaintenanceWindowExecution
    :: Text -- ^ 'cmweWindowExecutionId'
    -> CancelMaintenanceWindowExecution
cancelMaintenanceWindowExecution pWindowExecutionId_ =
  CancelMaintenanceWindowExecution'
    {_cmweWindowExecutionId = pWindowExecutionId_}
...
instance AWSRequest CancelMaintenanceWindowExecution
         where
...
instance ToHeaders CancelMaintenanceWindowExecution
         where
        toHeaders
          = const
              (mconcat
...
instance ToJSON CancelMaintenanceWindowExecution
         where
...
instance ToPath CancelMaintenanceWindowExecution
         where
        toPath = const "/"

instance ToQuery CancelMaintenanceWindowExecution
         where
...
data CancelMaintenanceWindowExecutionResponse = CancelMaintenanceWindowExecutionResponse'
  { _cmwersWindowExecutionId :: !(Maybe Text)
  , _cmwersResponseStatus    :: !Int
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
