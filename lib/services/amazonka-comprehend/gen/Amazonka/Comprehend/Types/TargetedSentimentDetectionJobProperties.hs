...
--
-- 'dataAccessRoleArn', 'targetedSentimentDetectionJobProperties_dataAccessRoleArn' - The Amazon Resource Name (ARN) of the IAM role that grants Amazon
-- Comprehend read access to your input data.
--
...
-- processing.
--
-- 'volumeKmsKeyId', 'targetedSentimentDetectionJobProperties_volumeKmsKeyId' - ID for the KMS key that Amazon Comprehend uses to encrypt the data on
-- the storage volume attached to the ML compute instance(s) that process
-- the targeted sentiment detection job. The VolumeKmsKeyId can be either
-- of the following formats:
--
