...
mkTargetInstances
    :: TargetInstances
mkTargetInstances
  = TargetInstances'{autoScalingGroups = Core.Nothing,
                     ec2TagSet = Core.Nothing, tagFilters = Core.Nothing}
...
{-# INLINEABLE tiAutoScalingGroups #-}
          ...
          = Core.withObject "TargetInstances" Core.$
              \ x ->
                TargetInstances' Core.<$>
                  (x Core..:? "autoScalingGroups") Core.<*> x Core..:? "ec2TagSet"
