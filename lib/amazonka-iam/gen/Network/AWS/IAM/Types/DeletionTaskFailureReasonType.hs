...
mkDeletionTaskFailureReasonType
    :: DeletionTaskFailureReasonType
mkDeletionTaskFailureReasonType
  = DeletionTaskFailureReasonType'{reason = Core.Nothing,
...
{-# INLINEABLE dtfrtRoleUsageList #-}
{-# DEPRECATED roleUsageList "Use generic-lens or generic-optics with 'roleUsageList' instead"  #-}

instance Core.FromXML DeletionTaskFailureReasonType where
        parseXML x
          = DeletionTaskFailureReasonType' Core.<$>
              (x Core..@? "Reason") Core.<*>
                x Core..@? "RoleUsageList" Core..<@> Core.parseXMLList "member"
