...
instance AWSPager DescribeSpotPriceHistory where
  page rq rs
    ...
    | otherwise = Just $ rq & dsphNextToken .~ rs ^. dsphrsNextToken
...
instance ToQuery DescribeSpotPriceHistory where
  toQuery DescribeSpotPriceHistory' {..} =
    mconcat
      [ "Action" =: ("DescribeSpotPriceHistory" :: ByteString),
        "Version" =: ("2016-11-15" :: ByteString),
        toQuery (toQueryList "InstanceType" <$> _dsphInstanceTypes),
        ...
        toQuery
          (toQueryList "ProductDescription" <$> _dsphProductDescriptions),
        "DryRun" =: _dsphDryRun,
