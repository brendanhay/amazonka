...
mkProcessingS3Input
    :: Types.S3Uri -- ^ 's3Uri'
    -> Types.LocalPath -- ^ 'localPath'
    -> Types.ProcessingS3DataType -- ^ 's3DataType'
    -> Types.ProcessingS3InputMode -- ^ 's3InputMode'
    -> ProcessingS3Input
mkProcessingS3Input s3Uri localPath s3DataType s3InputMode
  = ProcessingS3Input'{s3Uri, localPath, s3DataType, s3InputMode,
...
{-# INLINEABLE psiS3CompressionType #-}
{-# DEPRECATED s3CompressionType "Use generic-lens or generic-optics with 's3CompressionType' instead"  #-}
...
{-# INLINEABLE psiS3DataDistributionType #-}
