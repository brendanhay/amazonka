...
{-# INLINEABLE ldCloudWatchLogsLogGroup #-}
{-# DEPRECATED cloudWatchLogsLogGroup "Use generic-lens or generic-optics with 'cloudWatchLogsLogGroup' instead"  #-}

instance Core.FromJSON LogDestination where
        toJSON LogDestination{..}
          = Core.object
              (Core.catMaybes
                 [("cloudWatchLogsLogGroup" Core..=) Core.<$>
