...
mkHeaderObject
    :: HeaderObject
mkHeaderObject
  = HeaderObject'{headersAllowList = Core.Nothing,
                  option = Core.Nothing}
...
{-# INLINEABLE hoHeadersAllowList #-}
{-# DEPRECATED headersAllowList "Use generic-lens or generic-optics with 'headersAllowList' instead"  #-}
...
instance Core.FromJSON HeaderObject where
        parseJSON
          = Core.withObject "HeaderObject" Core.$
              \ x ->
                HeaderObject' Core.<$>
                  (x Core..:? "headersAllowList") Core.<*> x Core..:? "option"
