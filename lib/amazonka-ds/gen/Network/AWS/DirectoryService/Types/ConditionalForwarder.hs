...
mkConditionalForwarder
    :: ConditionalForwarder
mkConditionalForwarder
  = ConditionalForwarder'{dnsIpAddrs = Core.Nothing,
                          remoteDomainName = Core.Nothing, replicationScope = Core.Nothing}
...
{-# INLINEABLE cfReplicationScope #-}
{-# DEPRECATED replicationScope "Use generic-lens or generic-optics with 'replicationScope' instead"  #-}

instance Core.FromJSON ConditionalForwarder where
        parseJSON
          = Core.withObject "ConditionalForwarder" Core.$
              \ x ->
                ConditionalForwarder' Core.<$>
                  (x Core..:? "DnsIpAddrs") Core.<*> x Core..:? "RemoteDomainName"
