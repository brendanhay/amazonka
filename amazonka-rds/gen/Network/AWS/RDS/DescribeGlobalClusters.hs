...
data DescribeGlobalClusters = DescribeGlobalClusters'
  { _dgcsGlobalClusterIdentifier :: !(Maybe Text)
...
--
-- * 'dgcsMaxRecords' - The maximum number of records to include in the response. If more records exist than the specified @MaxRecords@ value, a pagination token called a marker is included in the response so that the remaining results can be retrieved.  Default: 100 Constraints: Minimum 20, maximum 100.
describeGlobalClusters
    :: DescribeGlobalClusters
describeGlobalClusters =
  DescribeGlobalClusters'
...
instance AWSPager DescribeGlobalClusters where
        page rq rs
          ...
          | otherwise =
            Just $ rq & dgcsMarker .~ rs ^. drsMarker

instance AWSRequest DescribeGlobalClusters where
        type Rs DescribeGlobalClusters =
             DescribeGlobalClustersResponse
        request = postQuery rds
        response
          = receiveXMLWrapper "DescribeGlobalClustersResult"
              (\ s h x ->
                 DescribeGlobalClustersResponse' <$>
...
instance Hashable DescribeGlobalClusters where

instance NFData DescribeGlobalClusters where
        ...
        toQuery DescribeGlobalClusters'{..}
          = mconcat
              ["Action" =:
               ...
               "Filters" =:
                 toQuery (toQueryList "Filter" <$> _dgcsFilters),
               "Marker" =: _dgcsMarker,
...
data DescribeGlobalClustersResponse = DescribeGlobalClustersResponse'
  { _drsGlobalClusters :: !(Maybe [GlobalCluster])
...
instance NFData DescribeGlobalClustersResponse where
