...
mkForwardedValues
    :: Core.Bool -- ^ 'queryString'
    -> Types.CookiePreference -- ^ 'cookies'
    -> ForwardedValues
mkForwardedValues queryString cookies
  = ForwardedValues'{queryString, cookies, headers = Core.Nothing,
...
{-# INLINEABLE fvQueryStringCacheKeys #-}
{-# DEPRECATED queryStringCacheKeys "Use generic-lens or generic-optics with 'queryStringCacheKeys' instead"  #-}

instance Core.ToXML ForwardedValues where
        toXML ForwardedValues{..}
          = Core.toXMLElement "QueryString" queryString Core.<>
              Core.toXMLElement "Cookies" cookies
              Core.<>
              Core.maybe Core.mempty (Core.toXMLElement "Headers") headers
              Core.<>
              Core.maybe Core.mempty (Core.toXMLElement "QueryStringCacheKeys")
                queryStringCacheKeys

instance Core.FromXML ForwardedValues where
        parseXML x
          = ForwardedValues' Core.<$>
              (x Core..@ "QueryString") Core.<*> x Core..@ "Cookies" Core.<*>
                x Core..@? "Headers"
