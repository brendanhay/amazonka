...
mkDescribeRegions
    :: DescribeRegions
mkDescribeRegions
  = DescribeRegions'{allRegions = Core.Nothing,
...
{-# INLINEABLE drsAllRegions #-}
{-# DEPRECATED allRegions "Use generic-lens or generic-optics with 'allRegions' instead"  #-}
...
drsRegionNames :: Lens.Lens' DescribeRegions (Core.Maybe [Core.Text])
drsRegionNames = Lens.field @"regionNames"
{-# INLINEABLE drsRegionNames #-}
{-# DEPRECATED regionNames "Use generic-lens or generic-optics with 'regionNames' instead"  #-}

instance Core.ToQuery DescribeRegions where
        toQuery DescribeRegions{..}
          = Core.toQueryPair "Action" ("DescribeRegions" :: Core.Text)
              Core.<> Core.toQueryPair "Version" ("2016-11-15" :: Core.Text)
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "AllRegions") allRegions
              Core.<> Core.maybe Core.mempty (Core.toQueryPair "DryRun") dryRun
              Core.<> Core.maybe Core.mempty (Core.toQueryList "Filter") filters
              Core.<>
              Core.maybe Core.mempty (Core.toQueryList "RegionName") regionNames
          ...
          = Response.receiveXML
              (\ s h x ->
                 DescribeRegionsResponse' Core.<$>
                   (x Core..@? "regionInfo" Core..<@> Core.parseXMLList "item")
