...
{-# INLINEABLE pHlsContentProtection #-}
...
instance Core.FromJSON Playlist where
        parseJSON
          = Core.withObject "Playlist" Core.$
              \ x ->
                Playlist' Core.<$>
                  (x Core..:? "Format") Core.<*> x Core..:? "HlsContentProtection"
                    Core.<*> x Core..:? "Name"
