...
mkAssumeRole
    ...
    -> Types.RoleSessionNameType -- ^ 'roleSessionName'
    -> AssumeRole
mkAssumeRole roleArn roleSessionName
  = AssumeRole'{roleArn, roleSessionName,
                durationSeconds = Core.Nothing, externalId = Core.Nothing,
                policy = Core.Nothing, policyArns = Core.Nothing,
                serialNumber = Core.Nothing, tags = Core.Nothing,
                tokenCode = Core.Nothing, transitiveTagKeys = Core.Nothing}
...
{-# INLINEABLE arTransitiveTagKeys #-}
{-# DEPRECATED transitiveTagKeys "Use generic-lens or generic-optics with 'transitiveTagKeys' instead"  #-}

instance Core.ToQuery AssumeRole where
        toQuery AssumeRole{..}
          = Core.toQueryPair "Action" ("AssumeRole" :: Core.Text) Core.<>
              Core.toQueryPair "Version" ("2011-06-15" :: Core.Text)
              ...
              Core.maybe Core.mempty (Core.toQueryPair "DurationSeconds")
                durationSeconds
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "ExternalId") externalId
              ...
              Core.<>
              Core.toQueryPair "TransitiveTagKeys"
...
instance Core.AWSRequest AssumeRole where
        ...
        parseResponse
          = Response.receiveXMLWrapper "AssumeRoleResult"
              (\ s h x ->
                 AssumeRoleResponse' Core.<$>
                   (x Core..@? "AssumedRoleUser") Core.<*> x Core..@? "Credentials"
...
mkAssumeRoleResponse responseStatus
  = AssumeRoleResponse'{assumedRoleUser = Core.Nothing,
                        credentials = Core.Nothing, packedPolicySize = Core.Nothing,
                        responseStatus}
...
{-# INLINEABLE arrrsAssumedRoleUser #-}
...
{-# INLINEABLE arrrsPackedPolicySize #-}
