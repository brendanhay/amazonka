...
mkJobWatermark
    :: JobWatermark
mkJobWatermark
  = JobWatermark'{encryption = Core.Nothing, inputKey = Core.Nothing,
...
{-# INLINEABLE jwPresetWatermarkId #-}
{-# DEPRECATED presetWatermarkId "Use generic-lens or generic-optics with 'presetWatermarkId' instead"  #-}
...
instance Core.FromJSON JobWatermark where
        parseJSON
          = Core.withObject "JobWatermark" Core.$
              \ x ->
                JobWatermark' Core.<$>
                  (x Core..:? "Encryption") Core.<*> x Core..:? "InputKey" Core.<*>
                    x Core..:? "PresetWatermarkId"
