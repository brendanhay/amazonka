...
mkGetQueryResults queryExecutionId
  = GetQueryResults'{queryExecutionId, maxResults = Core.Nothing,
...
{-# INLINEABLE gqrQueryExecutionId #-}
          ...
          = Response.receiveJSON
              (\ s h x ->
                 GetQueryResultsResponse' Core.<$>
                   (x Core..:? "NextToken") Core.<*> x Core..:? "ResultSet" Core.<*>
                     x Core..:? "UpdateCount"
...
instance Pager.AWSPager GetQueryResults where
        page rq rs
          ...
          | Pager.stop
              (rs Lens.^?
                 Lens.field @"resultSet" Core.. Lens._Just Core..
                   Lens.field @"rows" Core.. Lens._Just)
            = Core.Nothing
          | Core.otherwise =
            Core.Just
              (rq Core.&
...
mkGetQueryResultsResponse responseStatus
  = GetQueryResultsResponse'{nextToken = Core.Nothing,
                             resultSet = Core.Nothing, updateCount = Core.Nothing,
                             responseStatus}
...
{-# INLINEABLE gqrrrsResultSet #-}
{-# DEPRECATED resultSet "Use generic-lens or generic-optics with 'resultSet' instead"  #-}
...
{-# INLINEABLE gqrrrsUpdateCount #-}
{-# DEPRECATED updateCount "Use generic-lens or generic-optics with 'updateCount' instead"  #-}
