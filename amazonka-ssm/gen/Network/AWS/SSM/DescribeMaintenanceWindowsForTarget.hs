...
data DescribeMaintenanceWindowsForTarget = DescribeMaintenanceWindowsForTarget'
  { _dmwftNextToken    :: !(Maybe Text)
  , _dmwftMaxResults   :: !(Maybe Nat)
  , _dmwftTargets      :: ![Target]
  , _dmwftResourceType :: !MaintenanceWindowResourceType
...
describeMaintenanceWindowsForTarget
    :: MaintenanceWindowResourceType -- ^ 'dmwftResourceType'
    -> DescribeMaintenanceWindowsForTarget
...
instance AWSPager DescribeMaintenanceWindowsForTarget
         where
        page rq rs
          | stop (rs ^. dmwftrsNextToken) = Nothing
          | stop (rs ^. dmwftrsWindowIdentities) = Nothing
          | otherwise =
            Just $ rq & dmwftNextToken .~ rs ^. dmwftrsNextToken

instance AWSRequest
           DescribeMaintenanceWindowsForTarget
         where
        type Rs DescribeMaintenanceWindowsForTarget =
             DescribeMaintenanceWindowsForTargetResponse
...
instance ToJSON DescribeMaintenanceWindowsForTarget
         where
...
instance ToPath DescribeMaintenanceWindowsForTarget
         where
        toPath = const "/"

instance ToQuery DescribeMaintenanceWindowsForTarget
         where
...
data DescribeMaintenanceWindowsForTargetResponse = DescribeMaintenanceWindowsForTargetResponse'
  { _dmwftrsWindowIdentities :: !(Maybe [MaintenanceWindowIdentityForTarget])
  , _dmwftrsNextToken        :: !(Maybe Text)
  , _dmwftrsResponseStatus   :: !Int
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
