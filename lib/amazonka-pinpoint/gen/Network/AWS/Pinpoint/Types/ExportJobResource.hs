...
mkExportJobResource s3UrlPrefix roleArn
  = ExportJobResource'{s3UrlPrefix, roleArn,
...
instance Core.FromJSON ExportJobResource where
        parseJSON
          = Core.withObject "ExportJobResource" Core.$
              \ x ->
                ExportJobResource' Core.<$>
                  (x Core..: "S3UrlPrefix") Core.<*> x Core..: "RoleArn" Core.<*>
                    x Core..:? "SegmentId"
