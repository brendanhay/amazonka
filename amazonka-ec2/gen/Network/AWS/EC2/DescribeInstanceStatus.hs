...
-- @system-reboot@ or @instance-reboot@. System reboot commonly occurs if
-- certain maintenance or upgrade operations require a reboot of the
-- underlying host that supports an instance. Instance reboot commonly occurs
-- if the instance must be rebooted, rather than the underlying host.
-- Rebooting events include a scheduled start and end time. System
-- Maintenance: When Amazon EC2 determines that an instance requires
-- maintenance that requires power or network impact, the instance status is
-- the event code @system-maintenance@. System maintenance is either power
-- maintenance or network maintenance. For power maintenance, your instance
-- will be unavailable for a brief period of time and then rebooted. For
-- network maintenance, your instance will experience a brief loss of network
...
-- for system maintenance. The email message indicates when your instance is
-- scheduled for maintenance. Scheduled Retirement: When Amazon EC2 determines
-- that an instance must be shut down, the instance status is the event code
-- @instance-retirement@. Retirement commonly occurs when the underlying host
-- is degraded and must be replaced. Retirement events include a scheduled
-- start and end time. You will also be notified by email if one of your
-- instances is set to retiring. The email message indicates when your
-- instance will be permanently retired. Scheduled Stop: When Amazon EC2
-- determines that an instance must be shut down, the instances status returns
-- an event code called @instance-stop@. Stop events include a scheduled start
-- and end time. You will also be notified by email if one of your instances
-- is set to stop. The email message indicates when your instance will be
-- stopped. When your instance is retired, it will either be terminated (if
-- its root device type is the instance-store) or stopped (if its root device
...
-- description of the event. @event.not-after@ - The latest end time for the
-- scheduled event, for example: @2010-09-15T17:15:20.000Z@.
-- @event.not-before@ - The earliest start time for the scheduled event, for
-- example: @2010-09-15T17:15:20.000Z@. @instance-state-code@ - A code
-- representing the state of the instance, as a 16-bit unsigned integer. The
-- high byte is an opaque internal value and should be ignored. The low byte
-- is set based on the state represented. The valid values are 0 (pending),
-- 16 (running), 32 (shutting-down), 48 (terminated), 64 (stopping), and 80
-- (stopped). @instance-state-name@ - The state of the instance (@pending@ |
