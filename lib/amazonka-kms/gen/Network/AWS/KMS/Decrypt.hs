...
mkDecrypt ciphertextBlob
  = Decrypt'{ciphertextBlob, encryptionAlgorithm = Core.Nothing,
...
{-# INLINEABLE dfEncryptionAlgorithm #-}
...
{-# INLINEABLE dfEncryptionContext #-}
...
instance Core.ToHeaders Decrypt where
        toHeaders Decrypt{..}
          = Core.pure ("X-Amz-Target", "TrentService.Decrypt") Core.<>
          ...
          = Response.receiveJSON
              (\ s h x ->
                 DecryptResponse' Core.<$>
                   (x Core..:? "EncryptionAlgorithm") Core.<*> x Core..:? "KeyId"
...
mkDecryptResponse responseStatus
  = DecryptResponse'{encryptionAlgorithm = Core.Nothing,
                     keyId = Core.Nothing, plaintext = Core.Nothing, responseStatus}
...
{-# INLINEABLE drrsEncryptionAlgorithm #-}
{-# DEPRECATED encryptionAlgorithm "Use generic-lens or generic-optics with 'encryptionAlgorithm' instead"  #-}
