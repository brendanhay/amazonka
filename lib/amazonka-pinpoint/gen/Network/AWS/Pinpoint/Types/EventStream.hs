...
mkEventStream applicationId roleArn destinationStreamArn
  = EventStream'{applicationId, roleArn, destinationStreamArn,
                 externalId = Core.Nothing, lastModifiedDate = Core.Nothing,
...
{-# INLINEABLE esDestinationStreamArn #-}
...
instance Core.FromJSON EventStream where
        parseJSON
          = Core.withObject "EventStream" Core.$
              \ x ->
                EventStream' Core.<$>
                  (x Core..: "ApplicationId") Core.<*> x Core..: "RoleArn" Core.<*>
                    x Core..: "DestinationStreamArn"
