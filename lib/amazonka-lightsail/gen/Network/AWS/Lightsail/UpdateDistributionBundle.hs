...
mkUpdateDistributionBundle
    :: UpdateDistributionBundle
mkUpdateDistributionBundle
  = UpdateDistributionBundle'{bundleId = Core.Nothing,
...
udbBundleId :: Lens.Lens' UpdateDistributionBundle (Core.Maybe Core.Text)
udbBundleId = Lens.field @"bundleId"
{-# INLINEABLE udbBundleId #-}
...
{-# INLINEABLE udbDistributionName #-}
...
instance Core.ToHeaders UpdateDistributionBundle where
        toHeaders UpdateDistributionBundle{..}
          = Core.pure
...
instance Core.AWSRequest UpdateDistributionBundle where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 UpdateDistributionBundleResponse' Core.<$>
...
mkUpdateDistributionBundleResponse responseStatus
  = UpdateDistributionBundleResponse'{operation = Core.Nothing,
                                      responseStatus}
...
{-# INLINEABLE udbrrsResponseStatus #-}
