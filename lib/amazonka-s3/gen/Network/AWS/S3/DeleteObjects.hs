...
{-# INLINEABLE doBypassGovernanceRetention #-}
...
{-# INLINEABLE doExpectedBucketOwner #-}
...
instance Core.ToQuery DeleteObjects where
        toQuery DeleteObjects{..}
          = Core.toQueryPair "delete" ("" :: Core.Text)
...
instance Core.AWSRequest DeleteObjects where
        ...
        parseResponse
          = Response.receiveXML
              (\ s h x ->
                 DeleteObjectsResponse' Core.<$>
                   (x Core..@? "Deleted") Core.<*> x Core..@? "Error" Core.<*>
...
mkDeleteObjectsResponse responseStatus
  = DeleteObjectsResponse'{deleted = Core.Nothing,
                           errors = Core.Nothing, requestCharged = Core.Nothing,
                           responseStatus}
