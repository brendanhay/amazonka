...
mkNewDeviceMetadataType
    :: NewDeviceMetadataType
mkNewDeviceMetadataType
  = NewDeviceMetadataType'{deviceGroupKey = Core.Nothing,
...
{-# INLINEABLE ndmtDeviceGroupKey #-}
{-# DEPRECATED deviceGroupKey "Use generic-lens or generic-optics with 'deviceGroupKey' instead"  #-}
...
instance Core.FromJSON NewDeviceMetadataType where
        parseJSON
          = Core.withObject "NewDeviceMetadataType" Core.$
              \ x ->
                NewDeviceMetadataType' Core.<$>
                  (x Core..:? "DeviceGroupKey") Core.<*> x Core..:? "DeviceKey"
