...
mkCreateGrant
    ...
    -> Types.PrincipalIdType -- ^ 'granteePrincipal'
    -> CreateGrant
mkCreateGrant keyId granteePrincipal
  = CreateGrant'{keyId, granteePrincipal, operations = Core.mempty,
                 constraints = Core.Nothing, grantTokens = Core.Nothing,
                 name = Core.Nothing, retiringPrincipal = Core.Nothing}
...
{-# INLINEABLE cgRetiringPrincipal #-}
...
instance Core.ToHeaders CreateGrant where
        toHeaders CreateGrant{..}
          = Core.pure ("X-Amz-Target", "TrentService.CreateGrant") Core.<>
...
instance Core.AWSRequest CreateGrant where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 CreateGrantResponse' Core.<$>
                   (x Core..:? "GrantId") Core.<*> x Core..:? "GrantToken" Core.<*>
...
mkCreateGrantResponse responseStatus
  = CreateGrantResponse'{grantId = Core.Nothing,
                         grantToken = Core.Nothing, responseStatus}
