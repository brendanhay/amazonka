...
instance AWSPager DescribeTableStatistics where
  page rq rs
    ...
    | otherwise = Just $ rq & dtsMarker .~ rs ^. dtsrsMarker
...
instance ToHeaders DescribeTableStatistics where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AmazonDMSv20160101.DescribeTableStatistics" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("Filters" .=) <$> _dtsFilters,
            ...
            Just ("ReplicationTaskArn" .= _dtsReplicationTaskARN)
          ]
      )
...
data DescribeTableStatisticsResponse = DescribeTableStatisticsResponse'
  { _dtsrsReplicationTaskARN ::
      !(Maybe Text),
    _dtsrsMarker ::
      !(Maybe Text),
    _dtsrsTableStatistics ::
