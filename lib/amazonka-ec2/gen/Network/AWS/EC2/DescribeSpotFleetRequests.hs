...
instance AWSPager DescribeSpotFleetRequests where
  page rq rs
    ...
    | stop (rs ^. dsfrrsSpotFleetRequestConfigs) = Nothing
    | otherwise = Just $ rq & dsfrNextToken .~ rs ^. dsfrrsNextToken
...
instance ToQuery DescribeSpotFleetRequests where
  toQuery DescribeSpotFleetRequests' {..} =
    mconcat
      [ "Action" =: ("DescribeSpotFleetRequests" :: ByteString),
        "Version" =: ("2016-11-15" :: ByteString),
        toQuery
          (toQueryList "SpotFleetRequestId" <$> _dsfrSpotFleetRequestIds),
...
data DescribeSpotFleetRequestsResponse = DescribeSpotFleetRequestsResponse'
  { _dsfrrsNextToken ::
      !(Maybe Text),
    _dsfrrsSpotFleetRequestConfigs ::
      !( Maybe
