...
-- shard iterator to to use in your first GetRecords request and then use the
-- shard iterator returned by the GetRecords request in NextShardIterator for
...
-- of the next GetRecords request. If a GetShardIterator request is made too
-- often, you receive a ProvisionedThroughputExceededException. For more
-- information about throughput limits, see GetRecords>. If the shard is
-- closed, the iterator can't return more data, and GetShardIterator returns
-- null for its ShardIterator. A shard can be closed using SplitShard> or
-- MergeShards>. GetShardIterator has a limit of 5 transactions per second per
