...
{-# INLINEABLE cspUntrustedArtifactOnDeployment #-}
{-# DEPRECATED untrustedArtifactOnDeployment "Use generic-lens or generic-optics with 'untrustedArtifactOnDeployment' instead"  #-}

instance Core.FromJSON CodeSigningPolicies where
        toJSON CodeSigningPolicies{..}
          = Core.object
              (Core.catMaybes
                 [("UntrustedArtifactOnDeployment" Core..=) Core.<$>
          ...
          = Core.withObject "CodeSigningPolicies" Core.$
              \ x ->
                CodeSigningPolicies' Core.<$>
                  (x Core..:? "UntrustedArtifactOnDeployment")
