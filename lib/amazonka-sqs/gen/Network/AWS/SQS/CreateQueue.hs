...
data CreateQueue = CreateQueue'
  { -- | The name of the new queue. The following limits apply to this name:
    ...
    attributes :: Core.Maybe (Core.HashMap Types.QueueAttributeName Types.String),
...
cqQueueName :: Lens.Lens' CreateQueue Types.String
...
cqAttributes :: Lens.Lens' CreateQueue (Core.Maybe (Core.HashMap Types.QueueAttributeName Types.String))
  ...
  request x@Core.Request {..} =
    Core.Request
      { Core._rqService = Types.mkServiceConfig,
        ...
        Core._rqBody =
          Core.toFormBody
            ( Core.pure ("Action", "CreateQueue")
                Core.<> (Core.pure ("Version", "2012-11-05"))
                Core.<> (Core.toQueryValue "QueueName" queueName)
                Core.<> (Core.toQueryMap "Attribute" "Name" "Value" Core.<$> attributes)
                Core.<> (Core.toQueryMap "Tag" "Key" "Value" Core.<$> tags)
            )
      }
  response =
    Response.receiveXMLWrapper
      ...
      ( \s h x ->
          CreateQueueResponse'
            Core.<$> (x Core..@? "QueueUrl") Core.<*> (Core.pure (Core.fromEnum s))
      )
...
data CreateQueueResponse = CreateQueueResponse'
  { -- | The URL of the created Amazon SQS queue.
    queueUrl :: Core.Maybe Types.String,
...
cqrrsQueueUrl :: Lens.Lens' CreateQueueResponse (Core.Maybe Types.String)
cqrrsQueueUrl = Lens.field @"queueUrl"
{-# DEPRECATED cqrrsQueueUrl "Use generic-lens or generic-optics with 'queueUrl' instead." #-}
