...
--
-- /See:/ 'mkAutoMLJobObjective' smart constructor.
...
--
-- * 'metricName' - The name of the objective metric used to measure the predictive quality of a machine learning system. This metric is optimized during training to provide the best estimate for model parameter values from data.
--
...
mkAutoMLJobObjective ::
  ...
  AutoMLMetricEnum ->
  AutoMLJobObjective
mkAutoMLJobObjective pMetricName_ =
...
amljoMetricName :: Lens.Lens' AutoMLJobObjective AutoMLMetricEnum
amljoMetricName = Lens.lens (metricName :: AutoMLJobObjective -> AutoMLMetricEnum) (\s a -> s {metricName = a} :: AutoMLJobObjective)
{-# DEPRECATED amljoMetricName "Use generic-lens or generic-optics with 'metricName' instead." #-}

instance Lude.FromJSON AutoMLJobObjective where
  parseJSON =
    Lude.withObject
      "AutoMLJobObjective"
      (\x -> AutoMLJobObjective' Lude.<$> (x Lude..: "MetricName"))

instance Lude.ToJSON AutoMLJobObjective where
  toJSON AutoMLJobObjective' {..} =
    Lude.object
      (Lude.catMaybes [Lude.Just ("MetricName" Lude..= metricName)])
