...
mkPartialFailure
    :: PartialFailure
mkPartialFailure
  = PartialFailure'{exceptionType = Core.Nothing,
                    failureCode = Core.Nothing, failureDescription = Core.Nothing,
...
{-# INLINEABLE pfExceptionType #-}
{-# DEPRECATED exceptionType "Use generic-lens or generic-optics with 'exceptionType' instead"  #-}
...
{-# INLINEABLE pfFailureDescription #-}
...
instance Core.FromXML PartialFailure where
        parseXML x
          = PartialFailure' Core.<$>
              (x Core..@? "ExceptionType") Core.<*> x Core..@? "FailureCode"
