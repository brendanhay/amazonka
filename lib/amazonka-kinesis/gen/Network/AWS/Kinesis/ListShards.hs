...
mkListShards
    :: ListShards
mkListShards
  = ListShards'{exclusiveStartShardId = Core.Nothing,
                maxResults = Core.Nothing, nextToken = Core.Nothing,
                shardFilter = Core.Nothing, streamCreationTimestamp = Core.Nothing,
...
{-# INLINEABLE lsExclusiveStartShardId #-}
...
{-# INLINEABLE lsShardFilter #-}
{-# DEPRECATED shardFilter "Use generic-lens or generic-optics with 'shardFilter' instead"  #-}
...
{-# INLINEABLE lsStreamCreationTimestamp #-}
...
instance Core.ToHeaders ListShards where
        toHeaders ListShards{..}
          = Core.pure ("X-Amz-Target", "Kinesis_20131202.ListShards") Core.<>
          ...
          = Core.object
              (Core.catMaybes
                 [("ExclusiveStartShardId" Core..=) Core.<$> exclusiveStartShardId,
                  ...
                  ("StreamCreationTimestamp" Core..=) Core.<$>
...
instance Core.AWSRequest ListShards where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 ListShardsResponse' Core.<$>
                   (x Core..:? "NextToken") Core.<*> x Core..:? "Shards" Core.<*>
...
mkListShardsResponse responseStatus
  = ListShardsResponse'{nextToken = Core.Nothing,
                        shards = Core.Nothing, responseStatus}
...
{-# INLINEABLE lsrrsShards #-}
{-# DEPRECATED shards "Use generic-lens or generic-optics with 'shards' instead"  #-}
