...
mkCompliantSummary
    :: CompliantSummary
mkCompliantSummary
  = CompliantSummary'{compliantCount = Core.Nothing,
...
{-# INLINEABLE csCompliantCount #-}
{-# DEPRECATED compliantCount "Use generic-lens or generic-optics with 'compliantCount' instead"  #-}
...
instance Core.FromJSON CompliantSummary where
        parseJSON
          = Core.withObject "CompliantSummary" Core.$
              \ x ->
                CompliantSummary' Core.<$>
                  (x Core..:? "CompliantCount") Core.<*> x Core..:? "SeveritySummary"
