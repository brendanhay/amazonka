...
instance AWSRequest CreateLoadBalancerPolicy where
  ...
  response =
    receiveXMLWrapper
      ...
      ( \s h x ->
          CreateLoadBalancerPolicyResponse' <$> (pure (fromEnum s))
      )
...
instance ToQuery CreateLoadBalancerPolicy where
  toQuery CreateLoadBalancerPolicy' {..} =
    mconcat
      [ "Action" =: ("CreateLoadBalancerPolicy" :: ByteString),
        "Version" =: ("2012-06-01" :: ByteString),
        "PolicyAttributes"
          =: toQuery (toQueryList "member" <$> _clbpPolicyAttributes),
