...
instance AWSPager DescribeRemediationExecutionStatus
         where
        page rq rs
          | stop (rs ^. dresrsNextToken) = Nothing
          | stop (rs ^. dresrsRemediationExecutionStatuses) =
            Nothing
          | otherwise =
            Just $ rq & dresNextToken .~ rs ^. dresrsNextToken
...
instance ToHeaders DescribeRemediationExecutionStatus
         where
        toHeaders
          = const
              (mconcat
...
instance ToJSON DescribeRemediationExecutionStatus
         where
...
instance ToPath DescribeRemediationExecutionStatus
         where
        toPath = const "/"

instance ToQuery DescribeRemediationExecutionStatus
         where
