...
mkTableVersionError
    :: TableVersionError
mkTableVersionError
  = TableVersionError'{errorDetail = Core.Nothing,
                       tableName = Core.Nothing, versionId = Core.Nothing}
...
instance Core.FromJSON TableVersionError where
        parseJSON
          = Core.withObject "TableVersionError" Core.$
              \ x ->
                TableVersionError' Core.<$>
                  (x Core..:? "ErrorDetail") Core.<*> x Core..:? "TableName" Core.<*>
                    x Core..:? "VersionId"
