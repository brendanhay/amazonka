...
-- can have the attributes @{ "first_name", "first_value" }@ and @{
-- "first_name", "second_value" }@. However, it cannot have two attribute
-- instances where both the @Item.X.Attribute.Y.Name@ and
-- @Item.X.Attribute.Y.Value@ are the same. Optionally, the requester can
-- supply the @Replace@ parameter for each individual value. Setting this
-- value to @true@ will cause the new attribute values to replace the existing
-- attribute values. For example, if an item @I@ has the attributes @{ 'a',
-- '1' }, { 'b', '2'}@ and @{ 'b', '3' }@ and the requester does a
-- BatchPutAttributes of @{'I', 'b', '4' }@ with the Replace parameter set to
-- true, the final attributes of the item will be @{ 'a', '1' }@ and @{ 'b',
-- '4' }@, replacing the previous values of the 'b' attribute with the new
-- value. This operation is vulnerable to exceeding the maximum URL size when
...
-- (503) responses. The following limitations are enforced for this operation:
-- 256 attribute name-value pairs per item 1 MB request size 1 billion
-- attributes per domain 10 GB of total user data storage per domain 25 item
-- limit per @BatchPutAttributes@ operation.
--
