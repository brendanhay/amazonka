...
mkHostedZoneConfig
    :: HostedZoneConfig
mkHostedZoneConfig
  = HostedZoneConfig'{comment = Core.Nothing,
...
{-# INLINEABLE hzcPrivateZone #-}
{-# DEPRECATED privateZone "Use generic-lens or generic-optics with 'privateZone' instead"  #-}

instance Core.ToXML HostedZoneConfig where
        toXML HostedZoneConfig{..}
          = Core.maybe Core.mempty (Core.toXMLElement "Comment") comment
              Core.<>
              Core.maybe Core.mempty (Core.toXMLElement "PrivateZone")
                privateZone

instance Core.FromXML HostedZoneConfig where
        parseXML x
          = HostedZoneConfig' Core.<$>
              (x Core..@? "Comment") Core.<*> x Core..@? "PrivateZone"
