...
--
-- * 'ccSnowballType' - The type of AWS Snow Family device to use for this cluster.
--
...
instance ToHeaders CreateCluster where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AWSIESnowballJobManagementService.CreateCluster" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("KmsKeyARN" .=) <$> _ccKMSKeyARN,
            ...
            ("ForwardingAddressId" .=) <$> _ccForwardingAddressId,
