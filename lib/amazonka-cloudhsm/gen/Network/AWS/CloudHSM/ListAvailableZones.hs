...
instance Core.AWSRequest ListAvailableZones where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 ListAvailableZonesResponse' Core.<$>
                   (x Core..:? "AZList") Core.<*> Core.pure (Core.fromEnum s))
...
mkListAvailableZonesResponse responseStatus
  = ListAvailableZonesResponse'{aZList = Core.Nothing,
                                responseStatus}
...
{-# INLINEABLE lazrrsAZList #-}
{-# DEPRECATED aZList "Use generic-lens or generic-optics with 'aZList' instead"  #-}
...
{-# INLINEABLE lazrrsResponseStatus #-}
