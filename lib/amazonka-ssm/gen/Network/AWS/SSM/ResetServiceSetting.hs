...
--
-- * 'rssSettingId' - The Amazon Resource Name (ARN) of the service setting to reset. The setting ID can be @/ssm/parameter-store/default-parameter-tier@ , @/ssm/parameter-store/high-throughput-enabled@ , or @/ssm/managed-instance/activation-tier@ . For example, @arn:aws:ssm:us-east-1:111122223333:servicesetting/ssm/parameter-store/high-throughput-enabled@ .
resetServiceSetting ::
  -- | 'rssSettingId'
  Text ->
  ResetServiceSetting
...
instance ToHeaders ResetServiceSetting where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("AmazonSSM.ResetServiceSetting" :: ByteString),
...
instance ToJSON ResetServiceSetting where
  toJSON ResetServiceSetting' {..} =
    object (catMaybes [Just ("SettingId" .= _rssSettingId)])
...
resetServiceSettingResponse pResponseStatus_ =
  ResetServiceSettingResponse'
    { _rssrsServiceSetting = Nothing,
      _rssrsResponseStatus = pResponseStatus_
    }
