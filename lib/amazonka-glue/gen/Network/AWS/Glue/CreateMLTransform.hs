...
mkCreateMLTransform
    ...
    -> Types.TransformParameters -- ^ 'parameters'
    -> Types.Role -- ^ 'role\''
    -> CreateMLTransform
mkCreateMLTransform name parameters role'
  = CreateMLTransform'{name, inputRecordTables = Core.mempty,
                       parameters, role', description = Core.Nothing,
                       glueVersion = Core.Nothing, maxCapacity = Core.Nothing,
                       maxRetries = Core.Nothing, numberOfWorkers = Core.Nothing,
                       tags = Core.Nothing, timeout = Core.Nothing,
                       transformEncryption = Core.Nothing, workerType = Core.Nothing}
...
{-# INLINEABLE cmltInputRecordTables #-}
...
{-# INLINEABLE cmltNumberOfWorkers #-}
...
{-# INLINEABLE cmltTransformEncryption #-}
...
instance Core.ToHeaders CreateMLTransform where
        toHeaders CreateMLTransform{..}
          = Core.pure ("X-Amz-Target", "AWSGlue.CreateMLTransform") Core.<>
...
instance Core.AWSRequest CreateMLTransform where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 CreateMLTransformResponse' Core.<$>
...
mkCreateMLTransformResponse responseStatus
  = CreateMLTransformResponse'{transformId = Core.Nothing,
                               responseStatus}
...
{-# INLINEABLE cmltrrsResponseStatus #-}
