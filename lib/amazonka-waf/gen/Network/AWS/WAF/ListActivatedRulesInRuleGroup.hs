...
instance Page.AWSPager ListActivatedRulesInRuleGroup where
  page rq rs
    | Page.stop (rs Lens.^. larirgrsNextMarker) = Lude.Nothing
    | Page.stop (rs Lens.^. larirgrsActivatedRules) = Lude.Nothing
    | Lude.otherwise =
      Lude.Just Lude.$
        rq
          Lude.& larirgNextMarker Lens..~ rs Lens.^. larirgrsNextMarker
...
instance Lude.ToHeaders ListActivatedRulesInRuleGroup where
  toHeaders =
    Lude.const
      ( Lude.mconcat
          [ "X-Amz-Target"
              Lude.=# ( "AWSWAF_20150824.ListActivatedRulesInRuleGroup" ::
                          Lude.ByteString
                      ),
            "Content-Type"
