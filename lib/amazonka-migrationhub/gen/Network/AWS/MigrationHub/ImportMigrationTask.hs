...
instance AWSRequest ImportMigrationTask where
  ...
  response =
    receiveEmpty
      (\s h x -> ImportMigrationTaskResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders ImportMigrationTask where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AWSMigrationHub.ImportMigrationTask" :: ByteString),
...
instance ToJSON ImportMigrationTask where
  toJSON ImportMigrationTask' {..} =
    object
      ( catMaybes
          [ ("DryRun" .=) <$> _imtDryRun,
            Just ("ProgressUpdateStream" .= _imtProgressUpdateStream),
