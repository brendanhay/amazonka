...
mkValidDBInstanceModificationsMessage
    :: ValidDBInstanceModificationsMessage
mkValidDBInstanceModificationsMessage
  = ValidDBInstanceModificationsMessage'{storage = Core.Nothing,
...
{-# INLINEABLE vdbimmValidProcessorFeatures #-}
{-# DEPRECATED validProcessorFeatures "Use generic-lens or generic-optics with 'validProcessorFeatures' instead"  #-}

instance Core.FromXML ValidDBInstanceModificationsMessage where
        parseXML x
          = ValidDBInstanceModificationsMessage' Core.<$>
              (x Core..@? "Storage" Core..<@>
                 Core.parseXMLList "ValidStorageOptions")
                Core.<*>
                x Core..@? "ValidProcessorFeatures" Core..<@>
