...
data CreateRelationalDatabaseSnapshot = CreateRelationalDatabaseSnapshot'
  { _crdsTags                           :: !(Maybe [Tag])
  , _crdsRelationalDatabaseName         :: !Text
  , _crdsRelationalDatabaseSnapshotName :: !Text
...
--
-- * 'crdsTags' - The tag keys and optional values to add to the resource during create. To tag a resource after it has been created, see the @tag resource@ operation.
--
...
createRelationalDatabaseSnapshot
    :: Text -- ^ 'crdsRelationalDatabaseName'
    -> Text -- ^ 'crdsRelationalDatabaseSnapshotName'
    -> CreateRelationalDatabaseSnapshot
createRelationalDatabaseSnapshot pRelationalDatabaseName_ pRelationalDatabaseSnapshotName_ =
  CreateRelationalDatabaseSnapshot'
    { _crdsTags = Nothing
    , _crdsRelationalDatabaseName = pRelationalDatabaseName_
    , _crdsRelationalDatabaseSnapshotName = pRelationalDatabaseSnapshotName_
    }
...
instance AWSRequest CreateRelationalDatabaseSnapshot
         where
...
instance ToHeaders CreateRelationalDatabaseSnapshot
         where
        toHeaders
          = const
              (mconcat
...
instance ToJSON CreateRelationalDatabaseSnapshot
         where
...
instance ToPath CreateRelationalDatabaseSnapshot
         where
        toPath = const "/"

instance ToQuery CreateRelationalDatabaseSnapshot
         where
...
data CreateRelationalDatabaseSnapshotResponse = CreateRelationalDatabaseSnapshotResponse'
  { _crdsrsOperations     :: !(Maybe [Operation])
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'crdsrsOperations' - An object describing the result of your create relational database snapshot request.
--
...
createRelationalDatabaseSnapshotResponse pResponseStatus_ =
  CreateRelationalDatabaseSnapshotResponse'
    {_crdsrsOperations = Nothing, _crdsrsResponseStatus = pResponseStatus_}
