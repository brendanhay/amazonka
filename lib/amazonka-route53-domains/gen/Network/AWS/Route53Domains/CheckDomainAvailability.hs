...
instance Core.ToHeaders CheckDomainAvailability where
        toHeaders CheckDomainAvailability{..}
          = Core.pure
              ("X-Amz-Target",
...
instance Core.AWSRequest CheckDomainAvailability where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 CheckDomainAvailabilityResponse' Core.<$>
                   (x Core..: "Availability") Core.<*> Core.pure (Core.fromEnum s))
...
mkCheckDomainAvailabilityResponse
    :: Types.DomainAvailability -- ^ 'availability'
...
{-# INLINEABLE cdarrsResponseStatus #-}
