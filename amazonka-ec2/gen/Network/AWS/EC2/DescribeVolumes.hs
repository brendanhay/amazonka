...
-- value that can be passed to a subsequent @DescribeVolumes@ request to
-- retrieve the remaining results. For more information about Amazon EBS
-- volumes, see
...
-- volume is deleted on instance termination. @attachment.device@ - The
-- device name that is exposed to the instance (for example, @/dev/sda1@).
-- @attachment.instance-id@ - The ID of the instance the volume is attached
-- to. @attachment.status@ - The attachment state (@attaching@ | @attached@
-- | @detaching@ | @detached@). @availability-zone@ - The Availability Zone
-- in which the volume was created. @create-time@ - The time stamp when the
-- volume was created. @encrypted@ - The encryption status of the volume.
-- @size@ - The size of the volume, in GiB. @snapshot-id@ - The snapshot
-- from which the volume was created. @status@ - The status of the volume
...
-- @volume-id@ - The volume ID. @volume-type@ - The Amazon EBS volume type.
-- This can be @gp2@ for General Purpose (SSD) volumes, @io1@ for
