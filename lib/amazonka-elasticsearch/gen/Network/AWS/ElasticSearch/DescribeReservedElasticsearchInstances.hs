...
instance AWSPager DescribeReservedElasticsearchInstances where
  page rq rs
    | stop (rs ^. dreirsNextToken) = Nothing
    | stop (rs ^. dreirsReservedElasticsearchInstances) = Nothing
    | otherwise = Just $ rq & dreiNextToken .~ rs ^. dreirsNextToken
