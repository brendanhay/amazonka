...
mkRestoreObject bucket key
  = RestoreObject'{bucket, key, expectedBucketOwner = Core.Nothing,
                   requestPayer = Core.Nothing, restoreRequest = Core.Nothing,
...
{-# INLINEABLE roExpectedBucketOwner #-}
...
{-# INLINEABLE roRestoreRequest #-}
{-# DEPRECATED restoreRequest "Use generic-lens or generic-optics with 'restoreRequest' instead"  #-}
...
instance Core.ToQuery RestoreObject where
        toQuery RestoreObject{..}
          = Core.maybe Core.mempty (Core.toQueryPair "versionId") versionId
              Core.<> Core.toQueryPair "restore" ("" :: Core.Text)
...
instance Core.AWSRequest RestoreObject where
        ...
        parseResponse
          = Response.receiveEmpty
              (\ s h x ->
                 RestoreObjectResponse' Core.<$>
                   (Core.parseHeaderMaybe "x-amz-request-charged" h) Core.<*>
                     Core.parseHeaderMaybe "x-amz-restore-output-path" h
...
mkRestoreObjectResponse responseStatus
  = RestoreObjectResponse'{requestCharged = Core.Nothing,
                           restoreOutputPath = Core.Nothing, responseStatus}
...
{-# INLINEABLE rorrsRequestCharged #-}
...
{-# INLINEABLE rorrsRestoreOutputPath #-}
{-# DEPRECATED restoreOutputPath "Use generic-lens or generic-optics with 'restoreOutputPath' instead"  #-}
