...
mkAppValidationConfiguration
    :: AppValidationConfiguration
mkAppValidationConfiguration
  = AppValidationConfiguration'{appValidationStrategy = Core.Nothing,
                                name = Core.Nothing, ssmValidationParameters = Core.Nothing,
...
{-# INLINEABLE avcAppValidationStrategy #-}
{-# DEPRECATED appValidationStrategy "Use generic-lens or generic-optics with 'appValidationStrategy' instead"  #-}
...
{-# INLINEABLE avcSsmValidationParameters #-}
{-# DEPRECATED ssmValidationParameters "Use generic-lens or generic-optics with 'ssmValidationParameters' instead"  #-}
          ...
          = Core.object
              (Core.catMaybes
                 [("appValidationStrategy" Core..=) Core.<$> appValidationStrategy,
                  ("name" Core..=) Core.<$> name,
                  ("ssmValidationParameters" Core..=) Core.<$>
...
instance Core.FromJSON AppValidationConfiguration where
        parseJSON
          = Core.withObject "AppValidationConfiguration" Core.$
              \ x ->
                AppValidationConfiguration' Core.<$>
                  (x Core..:? "appValidationStrategy") Core.<*> x Core..:? "name"
