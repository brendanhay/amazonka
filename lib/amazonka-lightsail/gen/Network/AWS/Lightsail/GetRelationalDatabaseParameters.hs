...
instance AWSPager GetRelationalDatabaseParameters where
  page rq rs
    ...
    | otherwise =
      Just $ rq & grdpPageToken .~ rs ^. grdprsNextPageToken
    ...
    object
      ( catMaybes
          [ ("pageToken" .=) <$> _grdpPageToken,
            Just ("relationalDatabaseName" .= _grdpRelationalDatabaseName)
          ]
      )
...
instance NFData GetRelationalDatabaseParametersResponse
