...
mkAddPermission functionName statementId action principal
  = AddPermission'{functionName, statementId, action, principal,
                   eventSourceToken = Core.Nothing, qualifier = Core.Nothing,
                   revisionId = Core.Nothing, sourceAccount = Core.Nothing,
...
{-# INLINEABLE apEventSourceToken #-}
{-# DEPRECATED eventSourceToken "Use generic-lens or generic-optics with 'eventSourceToken' instead"  #-}
...
instance Core.AWSRequest AddPermission where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 AddPermissionResponse' Core.<$>
                   (x Core..:? "Statement") Core.<*> Core.pure (Core.fromEnum s))
...
aprrsStatement :: Lens.Lens' AddPermissionResponse (Core.Maybe Core.Text)
aprrsStatement = Lens.field @"statement"
{-# INLINEABLE aprrsStatement #-}
