...
data RunJobFlow = RunJobFlow'
  { _rjfLogEncryptionKMSKeyId ::
    ...
    _rjfPlacementGroupConfigs :: !(Maybe [PlacementGroupConfig]),
    ...
    _rjfManagedScalingPolicy :: !(Maybe ManagedScalingPolicy),
...
instance ToHeaders RunJobFlow where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("ElasticMapReduce.RunJobFlow" :: ByteString),
...
instance ToJSON RunJobFlow where
  toJSON RunJobFlow' {..} =
    object
      ( catMaybes
          [ ("LogEncryptionKmsKeyId" .=) <$> _rjfLogEncryptionKMSKeyId,
            ...
            ("SecurityConfiguration" .=) <$> _rjfSecurityConfiguration,
