...
instance AWSRequest CreateHIT where
  ...
  response =
    receiveJSON
      ( \s h x ->
          CreateHITResponse' <$> (x .?> "HIT") <*> (pure (fromEnum s))
      )
...
instance ToHeaders CreateHIT where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("MTurkRequesterServiceV20170117.CreateHIT" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("HITReviewPolicy" .=) <$> _chitHITReviewPolicy,
            ("UniqueRequestToken" .=) <$> _chitUniqueRequestToken,
            ("AutoApprovalDelayInSeconds" .=)
              <$> _chitAutoApprovalDelayInSeconds,
            ("RequesterAnnotation" .=) <$> _chitRequesterAnnotation,
            ("MaxAssignments" .=) <$> _chitMaxAssignments,
            ("Keywords" .=) <$> _chitKeywords,
            ("HITLayoutId" .=) <$> _chitHITLayoutId,
            ("HITLayoutParameters" .=) <$> _chitHITLayoutParameters,
            ("QualificationRequirements" .=)
              <$> _chitQualificationRequirements,
            ("Question" .=) <$> _chitQuestion,
            ("AssignmentReviewPolicy" .=) <$> _chitAssignmentReviewPolicy,
