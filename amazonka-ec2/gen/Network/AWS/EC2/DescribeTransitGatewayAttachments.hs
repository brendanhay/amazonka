...
instance AWSPager DescribeTransitGatewayAttachments
         where
        page rq rs
          | stop (rs ^. dtgarsNextToken) = Nothing
          | stop (rs ^. dtgarsTransitGatewayAttachments) =
            Nothing
          | otherwise =
            Just $ rq & dtgaNextToken .~ rs ^. dtgarsNextToken

instance AWSRequest DescribeTransitGatewayAttachments
         where
...
instance ToHeaders DescribeTransitGatewayAttachments
         where
        toHeaders = const mempty

instance ToPath DescribeTransitGatewayAttachments
         where
        toPath = const "/"

instance ToQuery DescribeTransitGatewayAttachments
         where
