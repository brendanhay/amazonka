...
mkGetGeoLocation
    :: GetGeoLocation
mkGetGeoLocation
  = GetGeoLocation'{continentCode = Core.Nothing,
                    countryCode = Core.Nothing, subdivisionCode = Core.Nothing}
...
instance Core.ToQuery GetGeoLocation where
        toQuery GetGeoLocation{..}
          = Core.maybe Core.mempty (Core.toQueryPair "continentcode")
              continentCode
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "countrycode") countryCode
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "subdivisioncode")
                subdivisionCode
...
instance Core.AWSRequest GetGeoLocation where
        ...
        toRequest x@Core.Request{..}
          = Core.Request{Core._rqService = Types.mkServiceConfig,
                         ...
                         Core._rqPath = "/2013-04-01/geolocation",
        ...
        parseResponse
          = Response.receiveXML
              (\ s h x ->
                 GetGeoLocationResponse' Core.<$>
                   (x Core..@ "GeoLocationDetails") Core.<*>
...
mkGetGeoLocationResponse
    :: Types.GeoLocationDetails -- ^ 'geoLocationDetails'
...
{-# INLINEABLE gglrrsGeoLocationDetails #-}
{-# DEPRECATED geoLocationDetails "Use generic-lens or generic-optics with 'geoLocationDetails' instead"  #-}
...
{-# INLINEABLE gglrrsResponseStatus #-}
