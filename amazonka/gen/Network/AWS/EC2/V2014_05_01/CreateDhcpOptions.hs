...
-- Module      : Network.AWS.EC2.V2014_05_01.CreateDhcpOptions
...
-- example, MyCompany.com). ntp-servers - The IP addresses of up to four
-- Network Time Protocol (NTP) servers. netbios-name-servers - The IP
-- addresses of up to four NetBIOS name servers. netbios-node-type - The
-- NetBIOS node type (1, 2, 4, or 8). We recommend that you specify 2
-- (broadcast and multicast are not currently supported). For more information
-- about these node types, see RFC 2132. For more information about DHCP
-- options, see DHCP Options Sets in the Amazon Virtual Private Cloud User
-- Guide. Example This example creates a set of DHCP options with a domain
-- name example.com and two DNS servers (10.2.5.1 and 10.2.5.2). The DNS
-- servers' IP addresses are specified in a single parameter, separated by
-- commas, to preserve the order in which they are specified.
...
module Network.AWS.EC2.V2014_05_01.CreateDhcpOptions where
