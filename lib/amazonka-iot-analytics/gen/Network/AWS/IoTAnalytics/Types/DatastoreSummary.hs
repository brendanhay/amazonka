...
mkDatastoreSummary
    :: DatastoreSummary
mkDatastoreSummary
  = DatastoreSummary'{creationTime = Core.Nothing,
                      datastoreName = Core.Nothing, datastoreStorage = Core.Nothing,
...
{-# INLINEABLE dsLastMessageArrivalTime #-}
...
instance Core.FromJSON DatastoreSummary where
        parseJSON
          = Core.withObject "DatastoreSummary" Core.$
              \ x ->
                DatastoreSummary' Core.<$>
                  (x Core..:? "creationTime") Core.<*> x Core..:? "datastoreName"
