...
data ModifyAccount = ModifyAccount'
  { _maDedicatedTenancySupport :: !(Maybe DedicatedTenancySupportEnum)
...
instance Hashable ModifyAccount where

instance NFData ModifyAccount where
...
instance NFData ModifyAccountResponse where
