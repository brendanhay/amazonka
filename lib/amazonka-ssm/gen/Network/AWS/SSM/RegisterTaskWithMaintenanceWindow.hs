...
instance ToHeaders RegisterTaskWithMaintenanceWindow where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AmazonSSM.RegisterTaskWithMaintenanceWindow" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("ServiceRoleArn" .=) <$> _rtwmwServiceRoleARN,
            ...
            ("TaskInvocationParameters" .=) <$> _rtwmwTaskInvocationParameters,
            ("Name" .=) <$> _rtwmwName,
...
instance NFData RegisterTaskWithMaintenanceWindowResponse
