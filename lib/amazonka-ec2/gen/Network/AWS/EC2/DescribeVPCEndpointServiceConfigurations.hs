...
instance AWSPager DescribeVPCEndpointServiceConfigurations where
  page rq rs
    | stop (rs ^. dvescrsNextToken) = Nothing
    | stop (rs ^. dvescrsServiceConfigurations) = Nothing
    | otherwise = Just $ rq & dvescNextToken .~ rs ^. dvescrsNextToken

instance AWSRequest DescribeVPCEndpointServiceConfigurations where
  type
...
instance Hashable DescribeVPCEndpointServiceConfigurations

instance NFData DescribeVPCEndpointServiceConfigurations

instance ToHeaders DescribeVPCEndpointServiceConfigurations where
  toHeaders = const mempty

instance ToPath DescribeVPCEndpointServiceConfigurations where
  toPath = const "/"

instance ToQuery DescribeVPCEndpointServiceConfigurations where
  toQuery DescribeVPCEndpointServiceConfigurations' {..} =
    mconcat
      [ "Action"
          =: ("DescribeVpcEndpointServiceConfigurations" :: ByteString),
        "Version" =: ("2016-11-15" :: ByteString),
        toQuery (toQueryList "Filter" <$> _dvescFilters),
        toQuery (toQueryList "ServiceId" <$> _dvescServiceIds),
...
instance NFData DescribeVPCEndpointServiceConfigurationsResponse
