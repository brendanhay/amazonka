...
-- unique only for this table. /KeySchema/ - Specifies the key schema for
-- the global secondary index. /Projection/ - Specifies attributes that are
...
-- attributes when determining the total. /ProvisionedThroughput/ - The
...
-- the following: /IndexName/ - The name of the local secondary index. Must
-- be unique only for this table. /KeySchema/ - Specifies the key schema for
-- the local secondary index. The key schema must begin with the same hash
-- key attribute as the table. /Projection/ - Specifies attributes that are
-- copied (projected) from the table into the index. These are in addition
-- to the primary key attributes and index key attributes, which are
-- automatically projected. Each attribute specification is composed of:
-- /ProjectionType/ - One of the following: @KEYS_ONLY@ - Only the index and
