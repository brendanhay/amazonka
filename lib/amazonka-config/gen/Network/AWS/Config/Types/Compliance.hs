...
mkCompliance
    :: Compliance
mkCompliance
  = Compliance'{complianceContributorCount = Core.Nothing,
...
{-# INLINEABLE cComplianceContributorCount #-}
{-# DEPRECATED complianceContributorCount "Use generic-lens or generic-optics with 'complianceContributorCount' instead"  #-}
...
instance Core.FromJSON Compliance where
        parseJSON
          = Core.withObject "Compliance" Core.$
              \ x ->
                Compliance' Core.<$>
                  (x Core..:? "ComplianceContributorCount") Core.<*>
                    x Core..:? "ComplianceType"
