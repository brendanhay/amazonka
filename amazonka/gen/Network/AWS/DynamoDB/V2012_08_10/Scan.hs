...
data Scan = Scan
    ...
    , _siScanFilter :: HashMap Text Condition
      ...
      -- "1"]}.
    , _siExclusiveStartKey :: HashMap Text AttributeValue
...
data ScanResponse = ScanResponse
    ...
    , _soScannedCount :: Maybe Integer
      ...
      -- Guide.
    , _soItems :: [HashMap Text AttributeValue]
      ...
      -- for that attribute.
    , _soLastEvaluatedKey :: HashMap Text AttributeValue
...
instance AWSPager Scan where
    next rq rs = (\x -> rq { _siExclusiveStartKey = Just x })
        <$> (_soLastEvaluatedKey rs)
