...
mkUserIdentityInfo
    :: UserIdentityInfo
mkUserIdentityInfo
  = UserIdentityInfo'{email = Core.Nothing, firstName = Core.Nothing,
...
instance Core.FromJSON UserIdentityInfo where
        parseJSON
          = Core.withObject "UserIdentityInfo" Core.$
              \ x ->
                UserIdentityInfo' Core.<$>
                  (x Core..:? "Email") Core.<*> x Core..:? "FirstName" Core.<*>
                    x Core..:? "LastName"
