...
instance AWSRequest UpdateUserPool where
  ...
  response =
    receiveEmpty
      (\s h x -> UpdateUserPoolResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders UpdateUserPool where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AWSCognitoIdentityProviderService.UpdateUserPool" :: ByteString),
      ...
      ( catMaybes
          [ ("UserPoolTags" .=) <$> _uupUserPoolTags,
            ("VerificationMessageTemplate" .=)
              <$> _uupVerificationMessageTemplate,
            ("EmailVerificationMessage" .=) <$> _uupEmailVerificationMessage,
            ("SmsAuthenticationMessage" .=) <$> _uupSmsAuthenticationMessage,
            ("UserPoolAddOns" .=) <$> _uupUserPoolAddOns,
            ("EmailVerificationSubject" .=) <$> _uupEmailVerificationSubject,
            ("AccountRecoverySetting" .=) <$> _uupAccountRecoverySetting,
            ("EmailConfiguration" .=) <$> _uupEmailConfiguration,
            ("SmsVerificationMessage" .=) <$> _uupSmsVerificationMessage,
            ("MfaConfiguration" .=) <$> _uupMFAConfiguration,
            ("LambdaConfig" .=) <$> _uupLambdaConfig,
            ("SmsConfiguration" .=) <$> _uupSmsConfiguration,
            ("AdminCreateUserConfig" .=) <$> _uupAdminCreateUserConfig,
            ("DeviceConfiguration" .=) <$> _uupDeviceConfiguration,
            ("AutoVerifiedAttributes" .=) <$> _uupAutoVerifiedAttributes,
