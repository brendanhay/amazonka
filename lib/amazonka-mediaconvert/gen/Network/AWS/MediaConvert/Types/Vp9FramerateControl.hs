...
module Network.AWS.MediaConvert.Types.Vp9FramerateControl where
...
instance FromText Vp9FramerateControl where
  parser =
    takeLowerText >>= \case
      "initialize_from_source" -> pure VFCInitializeFromSource
      "specified" -> pure VFCSpecified
      e ->
        fromTextError $
          "Failure parsing Vp9FramerateControl from value: '" <> e
            <> "'. Accepted values: initialize_from_source, specified"

instance ToText Vp9FramerateControl where
  toText = \case
    VFCInitializeFromSource -> "INITIALIZE_FROM_SOURCE"
    VFCSpecified -> "SPECIFIED"
