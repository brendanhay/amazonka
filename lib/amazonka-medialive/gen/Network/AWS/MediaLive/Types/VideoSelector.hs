...
mkVideoSelector
    :: VideoSelector
mkVideoSelector
  = VideoSelector'{colorSpace = Core.Nothing,
                   colorSpaceUsage = Core.Nothing, selectorSettings = Core.Nothing}
...
instance Core.FromJSON VideoSelector where
        parseJSON
          = Core.withObject "VideoSelector" Core.$
              \ x ->
                VideoSelector' Core.<$>
                  (x Core..:? "colorSpace") Core.<*> x Core..:? "colorSpaceUsage"
