...
instance AWSPager ListTransformJobs where
  page rq rs
    | stop (rs ^. ltjsrsNextToken) = Nothing
    | stop (rs ^. ltjsrsTransformJobSummaries) = Nothing
    | otherwise = Just $ rq & ltjNextToken .~ rs ^. ltjsrsNextToken
...
instance ToHeaders ListTransformJobs where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("SageMaker.ListTransformJobs" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("NameContains" .=) <$> _ltjNameContains,
            ("LastModifiedTimeBefore" .=) <$> _ltjLastModifiedTimeBefore,
            ("CreationTimeAfter" .=) <$> _ltjCreationTimeAfter,
            ("NextToken" .=) <$> _ltjNextToken,
            ("SortOrder" .=) <$> _ltjSortOrder,
            ("LastModifiedTimeAfter" .=) <$> _ltjLastModifiedTimeAfter,
...
data ListTransformJobsResponse = ListTransformJobsResponse'
  { _ltjsrsNextToken ::
      !(Maybe Text),
    _ltjsrsResponseStatus :: !Int,
    _ltjsrsTransformJobSummaries ::
      ![TransformJobSummary]
  }
  deriving (Eq, Read, Show, Data, Typeable, Generic)
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ltjsrsNextToken' - If the response is truncated, Amazon SageMaker returns this token. To retrieve the next set of transform jobs, use it in the next request.
--
-- * 'ltjsrsResponseStatus' - -- | The response status code.
--
-- * 'ltjsrsTransformJobSummaries' - An array of @TransformJobSummary@ objects.
listTransformJobsResponse ::
  -- | 'ltjsrsResponseStatus'
  Int ->
  ListTransformJobsResponse
listTransformJobsResponse pResponseStatus_ =
  ListTransformJobsResponse'
    { _ltjsrsNextToken = Nothing,
      _ltjsrsResponseStatus = pResponseStatus_,
      _ltjsrsTransformJobSummaries = mempty
    }

-- | If the response is truncated, Amazon SageMaker returns this token. To retrieve the next set of transform jobs, use it in the next request.
ltjsrsNextToken :: Lens' ListTransformJobsResponse (Maybe Text)
ltjsrsNextToken = lens _ltjsrsNextToken (\s a -> s {_ltjsrsNextToken = a})

-- | -- | The response status code.
ltjsrsResponseStatus :: Lens' ListTransformJobsResponse Int
ltjsrsResponseStatus = lens _ltjsrsResponseStatus (\s a -> s {_ltjsrsResponseStatus = a})

-- | An array of @TransformJobSummary@ objects.
ltjsrsTransformJobSummaries :: Lens' ListTransformJobsResponse [TransformJobSummary]
ltjsrsTransformJobSummaries = lens _ltjsrsTransformJobSummaries (\s a -> s {_ltjsrsTransformJobSummaries = a}) . _Coerce
