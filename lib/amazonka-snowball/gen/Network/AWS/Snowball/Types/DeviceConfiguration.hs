...
{-# INLINEABLE dcSnowconeDeviceConfiguration #-}
{-# DEPRECATED snowconeDeviceConfiguration "Use generic-lens or generic-optics with 'snowconeDeviceConfiguration' instead"  #-}

instance Core.FromJSON DeviceConfiguration where
        toJSON DeviceConfiguration{..}
          = Core.object
              (Core.catMaybes
                 [("SnowconeDeviceConfiguration" Core..=) Core.<$>
          ...
          = Core.withObject "DeviceConfiguration" Core.$
              \ x ->
                DeviceConfiguration' Core.<$>
                  (x Core..:? "SnowconeDeviceConfiguration")
