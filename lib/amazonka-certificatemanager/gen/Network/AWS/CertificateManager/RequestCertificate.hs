...
data RequestCertificate = RequestCertificate'
  { _rcIdempotencyToken ::
    ...
    _rcDomainValidationOptions ::
      !(Maybe (List1 DomainValidationOption)),
...
instance ToHeaders RequestCertificate where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("CertificateManager.RequestCertificate" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("IdempotencyToken" .=) <$> _rcIdempotencyToken,
            ...
            ("SubjectAlternativeNames" .=) <$> _rcSubjectAlternativeNames,
            ("Options" .=) <$> _rcOptions,
            ("DomainValidationOptions" .=) <$> _rcDomainValidationOptions,
            ("CertificateAuthorityArn" .=) <$> _rcCertificateAuthorityARN,
            ("Tags" .=) <$> _rcTags,
