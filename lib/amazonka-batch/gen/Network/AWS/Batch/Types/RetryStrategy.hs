...
mkRetryStrategy
    :: RetryStrategy
mkRetryStrategy
  = RetryStrategy'{attempts = Core.Nothing,
...
{-# INLINEABLE rsEvaluateOnExit #-}
{-# DEPRECATED evaluateOnExit "Use generic-lens or generic-optics with 'evaluateOnExit' instead"  #-}
...
instance Core.FromJSON RetryStrategy where
        parseJSON
          = Core.withObject "RetryStrategy" Core.$
              \ x ->
                RetryStrategy' Core.<$>
                  (x Core..:? "attempts") Core.<*> x Core..:? "evaluateOnExit"
