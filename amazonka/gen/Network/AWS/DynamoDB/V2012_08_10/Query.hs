...
data Query = Query
    ...
    , _qiQueryFilter :: HashMap Text Condition
    ...
    , _qiExclusiveStartKey :: HashMap Text AttributeValue
    ...
    , _qiKeyConditions :: HashMap Text Condition
...
data QueryResponse = QueryResponse
    ...
    , _qoItems :: [HashMap Text AttributeValue]
      ...
      -- for that attribute.
    , _qoLastEvaluatedKey :: HashMap Text AttributeValue
...
instance AWSPager Query where
    next rq rs = (\x -> rq { _qiExclusiveStartKey = Just x })
        <$> (_qoLastEvaluatedKey rs)
