...
mkUpdateJob jobId manifest jobType validateOnly
  = UpdateJob'{jobId, manifest, jobType, validateOnly,
...
{-# INLINEABLE ujAPIVersion #-}
{-# DEPRECATED aPIVersion "Use generic-lens or generic-optics with 'aPIVersion' instead"  #-}

instance Core.ToQuery UpdateJob where
        toQuery UpdateJob{..}
          = Core.toQueryPair "Operation=UpdateJob" ("" :: Core.Text) Core.<>
              Core.toQueryPair "Action" ("UpdateJob" :: Core.Text)
...
mkUpdateJobResponse responseStatus
  = UpdateJobResponse'{artifactList = Core.Nothing,
                       success = Core.Nothing, warningMessage = Core.Nothing,
                       responseStatus}
...
{-# INLINEABLE ujrrsWarningMessage #-}
