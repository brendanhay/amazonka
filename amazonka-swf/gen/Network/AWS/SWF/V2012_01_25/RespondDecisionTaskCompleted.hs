...
module Network.AWS.SWF.V2012_01_25.RespondDecisionTaskCompleted
    ...
    -- ** Request constructor
    , mkRespondDecisionTaskCompletedInput
    -- ** Request lenses
    , rdtciTaskToken
    , rdtciDecisions
...
mkRespondDecisionTaskCompletedInput :: Text -- ^ 'rdtciTaskToken'
                                    -> RespondDecisionTaskCompleted
mkRespondDecisionTaskCompletedInput p1 = RespondDecisionTaskCompleted
    { _rdtciTaskToken = p1
    , _rdtciDecisions = mempty
    , _rdtciExecutionContext = Nothing
    }
{-# INLINE mkRespondDecisionTaskCompletedInput #-}
...
rdtciTaskToken = lens _rdtciTaskToken (\s a -> s { _rdtciTaskToken = a })
...
rdtciDecisions = lens _rdtciDecisions (\s a -> s { _rdtciDecisions = a })
{-# INLINE rdtciDecisions #-}

-- | User defined context to add to workflow execution.
rdtciExecutionContext :: Lens' RespondDecisionTaskCompleted (Maybe Text)
rdtciExecutionContext = lens _rdtciExecutionContext (\s a -> s { _rdtciExecutionContext = a })
