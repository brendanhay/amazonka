...
detectText ::
  ...
  DetectText
detectText pImage_ =
  DetectText' {_dtFilters = Nothing, _dtImage = pImage_}
...
instance ToHeaders DetectText where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("RekognitionService.DetectText" :: ByteString),
...
instance ToJSON DetectText where
  toJSON DetectText' {..} =
    object
      ( catMaybes
          [("Filters" .=) <$> _dtFilters, Just ("Image" .= _dtImage)]
      )
...
data DetectTextResponse = DetectTextResponse'
  { _dtrsTextDetections ::
      !(Maybe [TextDetection]),
    _dtrsTextModelVersion :: !(Maybe Text),
