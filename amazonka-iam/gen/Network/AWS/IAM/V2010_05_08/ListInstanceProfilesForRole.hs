...
{-# INLINE lipfrrsInstanceProfiles #-}
...
instance AWSPager ListInstanceProfilesForRole where
    next rq rs
        | not (_lipfrrsIsTruncated rs) = Nothing
        | otherwise = Just $ rq
            { _lipfrMarker = _lipfrrsMarker rs
            }
