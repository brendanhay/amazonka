...
-- this value to retry the operation starting with the next item to get. For
-- example, if you ask to retrieve 100 items, but each individual item is 300
-- KB in size, the system returns 52 items (so as not to exceed the 16 MB
...
-- own logic to assemble the pages of results into one data set. If /none/ of
...
-- succeed. For more information, go to
...
-- returned in the result. Requests for nonexistent items consume the minimum
-- read capacity units according to the type of read. For more information,
-- see
...
-- table. For each primary key, you must provide /all/ of the key
-- attributes. For example, with a hash type primary key, you only need to
-- specify the hash attribute. For a hash-and-range type primary key, you
-- must specify /both/ the hash attribute and the range attribute.
-- /AttributesToGet/ - One or more attributes to be retrieved from the
-- table. By default, all attributes are returned. If a specified attribute
-- is not found, it does not appear in the result. Note that
...
-- If true, a strongly consistent read is used; if false (the default), an
...
-- Request Parameters section. Each element consists of: /Keys/ - An array
-- of primary key attribute values that define specific items in the table.
