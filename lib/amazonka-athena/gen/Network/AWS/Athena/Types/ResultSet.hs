...
{-# INLINEABLE rsResultSetMetadata #-}
{-# DEPRECATED resultSetMetadata "Use generic-lens or generic-optics with 'resultSetMetadata' instead"  #-}
...
instance Core.FromJSON ResultSet where
        parseJSON
          = Core.withObject "ResultSet" Core.$
              \ x ->
                ResultSet' Core.<$>
                  (x Core..:? "ResultSetMetadata") Core.<*> x Core..:? "Rows"
