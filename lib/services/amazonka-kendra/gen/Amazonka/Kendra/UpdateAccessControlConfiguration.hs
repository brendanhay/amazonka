...
instance
  ...
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "X-Amz-Target"
              Core.=# ( "AWSKendraFrontendService.UpdateAccessControlConfiguration" ::
                          Prelude.ByteString
                      ),
            "Content-Type"
...
instance Core.ToJSON UpdateAccessControlConfiguration where
...
instance Core.ToPath UpdateAccessControlConfiguration where
