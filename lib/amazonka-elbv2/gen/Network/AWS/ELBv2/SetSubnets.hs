...
mkSetSubnets loadBalancerArn
  = SetSubnets'{loadBalancerArn, ipAddressType = Core.Nothing,
                subnetMappings = Core.Nothing, subnets = Core.Nothing}
...
instance Core.AWSRequest SetSubnets where
        ...
        {-# INLINE toRequest #-}
        parseResponse
          = Response.receiveXMLWrapper "SetSubnetsResult"
              (\ s h x ->
                 SetSubnetsResponse' Core.<$>
...
mkSetSubnetsResponse responseStatus
  = SetSubnetsResponse'{availabilityZones = Core.Nothing,
                        ipAddressType = Core.Nothing, responseStatus}
...
{-# INLINEABLE ssrrsAvailabilityZones #-}
