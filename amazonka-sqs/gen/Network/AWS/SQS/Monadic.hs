...
--
-- As an example: using "Network.AWS.SQS" with the smart constructor and
...
-- @
--
-- Versus using "Network.AWS.SQS.Monadic" with the 'State' operator variants from
...
module Network.AWS.SQS.Monadic
    ...
    -- $ChangeMessageVisibility
    , changeMessageVisibility
    , changeMessageVisibilityCatch

    -- * ChangeMessageVisibilityBatch
    -- $ChangeMessageVisibilityBatch
    , changeMessageVisibilityBatch
    , changeMessageVisibilityBatchCatch
    ...
    -- $ListDeadLetterSourceQueues
    , listDeadLetterSourceQueues
    , listDeadLetterSourceQueuesCatch
...
addPermissionCatch :: ( MonadCatch m
    ...
    -> [Text] -- ^ 'apAWSAccountIds'
    -> [Text] -- ^ 'apActions'
    -> m (Either SQSError AddPermissionResponse)
...
changeMessageVisibilityCatch :: ( MonadCatch m
    ...
    -> Text -- ^ 'cmvReceiptHandle'
    -> Integer -- ^ 'cmvVisibilityTimeout'
    -> m (Either SQSError ChangeMessageVisibilityResponse)
...
changeMessageVisibilityBatchCatch :: ( MonadCatch m
    ...
    => Text -- ^ 'cmvbQueueUrl'
    -> [ChangeMessageVisibilityBatchRequestEntry] -- ^ 'cmvbEntries'
    -> m (Either SQSError ChangeMessageVisibilityBatchResponse)
...
createQueueCatch :: ( MonadCatch m
                    , MonadResource m
                    , MonadReader Env m
                    )
    => Text -- ^ 'cqQueueName'
    -> State CreateQueue a
    -> m (Either SQSError CreateQueueResponse)
...
deleteMessageCatch :: ( MonadCatch m
    ...
    => Text -- ^ 'dmQueueUrl'
    -> Text -- ^ 'dmReceiptHandle'
    -> m (Either SQSError DeleteMessageResponse)
...
deleteMessageBatchCatch :: ( MonadCatch m
    ...
    => Text -- ^ 'dmbQueueUrl'
    -> [DeleteMessageBatchRequestEntry] -- ^ 'dmbEntries'
    -> m (Either SQSError DeleteMessageBatchResponse)
...
deleteQueueCatch :: ( MonadCatch m
    ...
    => Text -- ^ 'dqQueueUrl'
    -> m (Either SQSError DeleteQueueResponse)
deleteQueueCatch p1 =
...
getQueueAttributesCatch :: ( MonadCatch m
                           , MonadResource m
                           , MonadReader Env m
                           )
    => Text -- ^ 'gqaQueueUrl'
    -> State GetQueueAttributes a
    -> m (Either SQSError GetQueueAttributesResponse)
...
getQueueUrlCatch :: ( MonadCatch m
                    , MonadResource m
                    , MonadReader Env m
                    )
    => Text -- ^ 'gquQueueName'
    -> State GetQueueUrl a
    -> m (Either SQSError GetQueueUrlResponse)
...
listDeadLetterSourceQueuesCatch :: ( MonadCatch m
    ...
    => Text -- ^ 'ldlsqQueueUrl'
    -> m (Either SQSError ListDeadLetterSourceQueuesResponse)
...
listQueuesCatch :: ( MonadCatch m
                   , MonadResource m
                   , MonadReader Env m
                   )
    => State ListQueues a
    -> m (Either SQSError ListQueuesResponse)
listQueuesCatch s =
...
receiveMessageCatch :: ( MonadCatch m
                       , MonadResource m
                       , MonadReader Env m
                       )
    => Text -- ^ 'rmQueueUrl'
    -> State ReceiveMessage a
    -> m (Either SQSError ReceiveMessageResponse)
...
removePermissionCatch :: ( MonadCatch m
    ...
    => Text -- ^ 'rpQueueUrl'
    -> Text -- ^ 'rpLabel'
    -> m (Either SQSError RemovePermissionResponse)
...
sendMessageCatch :: ( MonadCatch m
    ...
    -> Text -- ^ 'smMessageBody'
    -> State SendMessage a
    -> m (Either SQSError SendMessageResponse)
...
sendMessageBatchCatch :: ( MonadCatch m
    ...
    => Text -- ^ 'smbQueueUrl'
    -> [SendMessageBatchRequestEntry] -- ^ 'smbEntries'
    -> m (Either SQSError SendMessageBatchResponse)
...
setQueueAttributesCatch :: ( MonadCatch m
    ...
    => Text -- ^ 'sqaQueueUrl'
    -> Map QueueAttributeName Text -- ^ 'sqaAttributes'
    -> m (Either SQSError SetQueueAttributesResponse)
