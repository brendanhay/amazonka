...
--
--
-- /Important:/ Policies that are used as permissions boundaries do not provide permissions. You must also attach a permissions policy to the user. To learn how the effective permissions for a user are evaluated, see <http://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_evaluation-logic.html IAM JSON Policy Evaluation Logic> in the IAM User Guide.
--
module Network.AWS.IAM.PutUserPermissionsBoundary
    (
    -- * Creating a Request
...
instance AWSRequest PutUserPermissionsBoundary where
        ...
        response
          = receiveNull PutUserPermissionsBoundaryResponse'

instance Hashable PutUserPermissionsBoundary where

instance NFData PutUserPermissionsBoundary where
...
instance ToQuery PutUserPermissionsBoundary where
        toQuery PutUserPermissionsBoundary'{..}
          = mconcat
              ["Action" =:
                 ("PutUserPermissionsBoundary" :: ByteString),
...
data PutUserPermissionsBoundaryResponse =
  PutUserPermissionsBoundaryResponse'
  deriving (Eq, Read, Show, Data, Typeable, Generic)
