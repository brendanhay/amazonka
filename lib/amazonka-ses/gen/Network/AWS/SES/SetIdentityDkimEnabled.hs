...
instance Core.ToQuery SetIdentityDkimEnabled where
        toQuery SetIdentityDkimEnabled{..}
          = Core.toQueryPair "Action" ("SetIdentityDkimEnabled" :: Core.Text)
              Core.<> Core.toQueryPair "Version" ("2010-12-01" :: Core.Text)
              Core.<> Core.toQueryPair "Identity" identity
              Core.<> Core.toQueryPair "DkimEnabled" dkimEnabled
...
instance Core.AWSRequest SetIdentityDkimEnabled where
        ...
        {-# INLINE toRequest #-}
        parseResponse
          = Response.receiveXMLWrapper "SetIdentityDkimEnabledResult"
              (\ s h x ->
                 SetIdentityDkimEnabledResponse' Core.<$>
...
{-# INLINEABLE siderrsResponseStatus #-}
