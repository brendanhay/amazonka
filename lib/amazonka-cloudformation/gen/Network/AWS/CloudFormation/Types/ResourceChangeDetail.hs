...
mkResourceChangeDetail
  = ResourceChangeDetail'{causingEntity = Core.Nothing,
                          changeSource = Core.Nothing, evaluation = Core.Nothing,
                          target = Core.Nothing}
...
{-# INLINEABLE rcdChangeSource #-}
{-# DEPRECATED changeSource "Use generic-lens or generic-optics with 'changeSource' instead"  #-}
...
instance Core.FromXML ResourceChangeDetail where
        parseXML x
          = ResourceChangeDetail' Core.<$>
              (x Core..@? "CausingEntity") Core.<*> x Core..@? "ChangeSource"
