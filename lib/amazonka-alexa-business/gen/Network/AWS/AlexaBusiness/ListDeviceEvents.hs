...
mkListDeviceEvents deviceArn
  = ListDeviceEvents'{deviceArn, eventType = Core.Nothing,
...
instance Core.AWSRequest ListDeviceEvents where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 ListDeviceEventsResponse' Core.<$>
                   (x Core..:? "DeviceEvents") Core.<*> x Core..:? "NextToken"
...
mkListDeviceEventsResponse responseStatus
  = ListDeviceEventsResponse'{deviceEvents = Core.Nothing,
...
{-# INLINEABLE lderrsDeviceEvents #-}
{-# DEPRECATED deviceEvents "Use generic-lens or generic-optics with 'deviceEvents' instead"  #-}
