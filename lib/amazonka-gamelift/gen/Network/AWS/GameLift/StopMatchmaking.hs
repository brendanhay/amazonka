...
newtype StopMatchmaking = StopMatchmaking' {_smTicketId :: Text}
...
instance AWSRequest StopMatchmaking where
  ...
  response =
    receiveEmpty
      (\s h x -> StopMatchmakingResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders StopMatchmaking where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("GameLift.StopMatchmaking" :: ByteString),
...
instance ToJSON StopMatchmaking where
  toJSON StopMatchmaking' {..} =
    object (catMaybes [Just ("TicketId" .= _smTicketId)])
