...
mkUser path userName userId arn createDate
  = User'{path, userName, userId, arn, createDate,
...
{-# INLINEABLE uPasswordLastUsed #-}
{-# DEPRECATED passwordLastUsed "Use generic-lens or generic-optics with 'passwordLastUsed' instead"  #-}
...
{-# INLINEABLE uPermissionsBoundary #-}
...
instance Core.FromXML User where
        parseXML x
          = User' Core.<$>
              (x Core..@ "Path") Core.<*> x Core..@ "UserName" Core.<*>
                x Core..@ "UserId"
                Core.<*> x Core..@ "Arn"
