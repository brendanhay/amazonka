...
updateDevice ::
  UpdateDevice
updateDevice =
  UpdateDevice' {_udDeviceARN = Nothing, _udDeviceName = Nothing}
...
instance AWSRequest UpdateDevice where
  ...
  response =
    receiveEmpty
      (\s h x -> UpdateDeviceResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders UpdateDevice where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("AlexaForBusiness.UpdateDevice" :: ByteString),
