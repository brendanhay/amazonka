...
instance AWSPager DescribeClusterSubnetGroups where
  page rq rs
    ...
    | otherwise = Just $ rq & dcsgsMarker .~ rs ^. dcsgrsMarker
...
instance ToQuery DescribeClusterSubnetGroups where
  toQuery DescribeClusterSubnetGroups' {..} =
    mconcat
      [ "Action" =: ("DescribeClusterSubnetGroups" :: ByteString),
        "Version" =: ("2012-12-01" :: ByteString),
        "TagValues"
          =: toQuery (toQueryList "TagValue" <$> _dcsgsTagValues),
        "TagKeys" =: toQuery (toQueryList "TagKey" <$> _dcsgsTagKeys),
        "ClusterSubnetGroupName" =: _dcsgsClusterSubnetGroupName,
        "Marker" =: _dcsgsMarker,
