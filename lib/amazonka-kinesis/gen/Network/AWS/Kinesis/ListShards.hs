...
--
-- * 'lsShardFilter' - Undocumented member.
--
...
--
-- * 'lsMaxResults' - The maximum number of shards to return in a single call to @ListShards@ . The minimum value you can specify for this parameter is 1, and the maximum is 10,000, which is also the default. When the number of shards to be listed is greater than the value of @MaxResults@ , the response contains a @NextToken@ value that you can use in a subsequent call to @ListShards@ to list the next set of shards.
listShards ::
  ListShards
listShards =
  ListShards'
    { _lsShardFilter = Nothing,
...
-- | Undocumented member.
lsShardFilter :: Lens' ListShards (Maybe ShardFilter)
lsShardFilter = lens _lsShardFilter (\s a -> s {_lsShardFilter = a})
...
instance AWSPager ListShards where
  page rq rs
    | stop (rs ^. lrsNextToken) = Nothing
    | stop (rs ^. lrsShards) = Nothing
    | otherwise = Just $ rq & lsNextToken .~ rs ^. lrsNextToken
...
instance Hashable ListShards

instance NFData ListShards

instance ToHeaders ListShards where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("Kinesis_20131202.ListShards" :: ByteString),
...
instance ToJSON ListShards where
  toJSON ListShards' {..} =
    object
      ( catMaybes
          [ ("ShardFilter" .=) <$> _lsShardFilter,
            ("NextToken" .=) <$> _lsNextToken,
            ("ExclusiveStartShardId" .=) <$> _lsExclusiveStartShardId,
            ("StreamCreationTimestamp" .=) <$> _lsStreamCreationTimestamp,
...
instance NFData ListShardsResponse
