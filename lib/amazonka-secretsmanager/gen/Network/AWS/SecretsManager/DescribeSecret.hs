...
newtype DescribeSecret = DescribeSecret' {_dSecretId :: Text}
...
instance ToHeaders DescribeSecret where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("secretsmanager.DescribeSecret" :: ByteString),
...
instance ToJSON DescribeSecret where
  toJSON DescribeSecret' {..} =
    object (catMaybes [Just ("SecretId" .= _dSecretId)])
