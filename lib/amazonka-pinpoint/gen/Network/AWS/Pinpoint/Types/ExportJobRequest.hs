...
mkExportJobRequest
    :: Core.Text -- ^ 's3UrlPrefix'
    -> Core.Text -- ^ 'roleArn'
    -> ExportJobRequest
mkExportJobRequest s3UrlPrefix roleArn
  = ExportJobRequest'{s3UrlPrefix, roleArn, segmentId = Core.Nothing,
...
{-# INLINEABLE eS3UrlPrefix #-}
{-# DEPRECATED s3UrlPrefix "Use generic-lens or generic-optics with 's3UrlPrefix' instead"  #-}
