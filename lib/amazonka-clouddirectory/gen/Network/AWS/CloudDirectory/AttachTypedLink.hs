...
instance Hashable AttachTypedLink

instance NFData AttachTypedLink

instance ToHeaders AttachTypedLink where
  toHeaders AttachTypedLink' {..} =
    mconcat ["x-amz-data-partition" =# _atlDirectoryARN]

instance ToJSON AttachTypedLink where
  toJSON AttachTypedLink' {..} =
    object
      ( catMaybes
          [ Just ("SourceObjectReference" .= _atlSourceObjectReference),
            Just ("TargetObjectReference" .= _atlTargetObjectReference),
...
instance ToPath AttachTypedLink where
  toPath = const "/amazonclouddirectory/2017-01-11/typedlink/attach"
...
instance NFData AttachTypedLinkResponse
