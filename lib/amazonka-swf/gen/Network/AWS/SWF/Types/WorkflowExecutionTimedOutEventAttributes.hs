...
{-# INLINEABLE wetoeaChildPolicy #-}
{-# DEPRECATED childPolicy "Use generic-lens or generic-optics with 'childPolicy' instead"  #-}

instance Core.FromJSON WorkflowExecutionTimedOutEventAttributes
         where
        parseJSON
          = Core.withObject "WorkflowExecutionTimedOutEventAttributes" Core.$
              \ x ->
                WorkflowExecutionTimedOutEventAttributes' Core.<$>
                  (x Core..: "timeoutType") Core.<*> x Core..: "childPolicy"
