...
mkMultiConditionalBranch
    :: MultiConditionalBranch
mkMultiConditionalBranch
  = MultiConditionalBranch'{condition = Core.Nothing,
...
instance Core.FromJSON MultiConditionalBranch where
        parseJSON
          = Core.withObject "MultiConditionalBranch" Core.$
              \ x ->
                MultiConditionalBranch' Core.<$>
                  (x Core..:? "Condition") Core.<*> x Core..:? "NextActivity"
