...
mkCarrierGateway
    :: CarrierGateway
mkCarrierGateway
  = CarrierGateway'{carrierGatewayId = Core.Nothing,
...
cgOwnerId :: Lens.Lens' CarrierGateway (Core.Maybe Core.Text)
cgOwnerId = Lens.field @"ownerId"
{-# INLINEABLE cgOwnerId #-}
...
instance Core.FromXML CarrierGateway where
        parseXML x
          = CarrierGateway' Core.<$>
              (x Core..@? "carrierGatewayId") Core.<*> x Core..@? "ownerId"
                Core.<*> x Core..@? "state"
