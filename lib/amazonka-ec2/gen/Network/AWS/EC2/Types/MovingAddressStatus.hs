...
mkMovingAddressStatus
    :: MovingAddressStatus
mkMovingAddressStatus
  = MovingAddressStatus'{moveStatus = Core.Nothing,
...
{-# INLINEABLE masMoveStatus #-}
{-# DEPRECATED moveStatus "Use generic-lens or generic-optics with 'moveStatus' instead"  #-}
...
masPublicIp :: Lens.Lens' MovingAddressStatus (Core.Maybe Core.Text)
masPublicIp = Lens.field @"publicIp"
{-# INLINEABLE masPublicIp #-}
...
instance Core.FromXML MovingAddressStatus where
        parseXML x
          = MovingAddressStatus' Core.<$>
              (x Core..@? "moveStatus") Core.<*> x Core..@? "publicIp"
