...
--
-- * 'smMessageAttributes' - Each message attribute consists of a @Name@ , @Type@ , and @Value@ . For more information, see <http://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-message-attributes.html#message-attributes-items-validation Message Attribute Items and Validation> in the /Amazon SQS Developer Guide/ .
--
-- * 'smDelaySeconds' - The number of seconds to delay a specific message. Valid values: 0 to 900. Maximum: 15 minutes. Messages with a positive @DelaySeconds@ value become available for processing after the delay period is finished. If you don't specify a value, the default value for the queue applies.
--
...
--
-- * 'smMessageBody' - The message to send. The maximum string size is 256 KB. /Important:/ The following list shows the characters (in Unicode) that are allowed in your message, according to the W3C XML specification:     * @#x9@      * @#xA@      * @#xD@      * @#x20@ to @#xD7FF@      * @#xE000@ to @#xFFFD@      * @#x10000@ to @#x10FFFF@  For more information, see <https://www.ietf.org/rfc/rfc1321.txt RFC1321> . If you send any characters that aren't included in this list, your request is rejected.
sendMessage
    :: Text -- ^ 'smQueueURL'
    -> Text -- ^ 'smMessageBody'
    -> SendMessage
...
-- | The message to send. The maximum string size is 256 KB. /Important:/ The following list shows the characters (in Unicode) that are allowed in your message, according to the W3C XML specification:     * @#x9@      * @#xA@      * @#xD@      * @#x20@ to @#xD7FF@      * @#xE000@ to @#xFFFD@      * @#x10000@ to @#x10FFFF@  For more information, see <https://www.ietf.org/rfc/rfc1321.txt RFC1321> . If you send any characters that aren't included in this list, your request is rejected.
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'smrsSequenceNumber' - This parameter applies only to FIFO (first-in-first-out) queues. A large, non-consecutive number that Amazon SQS assigns to each message. The length of @SequenceNumber@ is 128 bits. @SequenceNumber@ continues to increase for a particular @MessageGroupId@ .
--
-- * 'smrsMessageId' - An attribute containing the @MessageId@ of the message sent to the queue. For more information, see <http://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-queue-message-identifiers.html Queue and Message Identifiers> in the /Amazon SQS Developer Guide/ .
--
-- * 'smrsMD5OfMessageBody' - An MD5 digest of the non-URL-encoded message attribute string. You can use this attribute to verify that Amazon SQS received the message correctly. Amazon SQS URL-decodes the message before creating the MD5 digest. For information on MD5, see <https://www.ietf.org/rfc/rfc1321.txt RFC1321> .
--
-- * 'smrsMD5OfMessageAttributes' - An MD5 digest of the non-URL-encoded message attribute string. You can use this attribute to verify that Amazon SQS received the message correctly. Amazon SQS URL-decodes the message before creating the MD5 digest. For information on MD5, see <https://www.ietf.org/rfc/rfc1321.txt RFC1321> .
--
...
-- | An attribute containing the @MessageId@ of the message sent to the queue. For more information, see <http://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-queue-message-identifiers.html Queue and Message Identifiers> in the /Amazon SQS Developer Guide/ .
smrsMessageId :: Lens' SendMessageResponse (Maybe Text)
smrsMessageId = lens _smrsMessageId (\ s a -> s{_smrsMessageId = a});

-- | An MD5 digest of the non-URL-encoded message attribute string. You can use this attribute to verify that Amazon SQS received the message correctly. Amazon SQS URL-decodes the message before creating the MD5 digest. For information on MD5, see <https://www.ietf.org/rfc/rfc1321.txt RFC1321> .
