...
instance AWSRequest AddApplicationReferenceDataSource where
  ...
  response =
    receiveEmpty
      ( \s h x ->
          AddApplicationReferenceDataSourceResponse' <$> (pure (fromEnum s))
      )
    ...
    object
      ( catMaybes
          [ Just ("ApplicationName" .= _aardsApplicationName),
            ...
            Just ("ReferenceDataSource" .= _aardsReferenceDataSource)
          ]
      )
...
instance NFData AddApplicationReferenceDataSourceResponse
