...
instance Core.AWSRequest GetTagValues where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 GetTagValuesResponse' Core.<$>
                   (x Core..:? "PaginationToken") Core.<*> x Core..:? "TagValues"
...
mkGetTagValuesResponse responseStatus
  = GetTagValuesResponse'{paginationToken = Core.Nothing,
                          tagValues = Core.Nothing, responseStatus}
...
{-# INLINEABLE gtvrrsPaginationToken #-}
{-# DEPRECATED paginationToken "Use generic-lens or generic-optics with 'paginationToken' instead"  #-}
