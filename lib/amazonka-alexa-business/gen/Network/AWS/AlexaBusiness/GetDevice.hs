...
newtype GetDevice = GetDevice' {_gdDeviceARN :: Maybe Text}
...
instance AWSRequest GetDevice where
  ...
  response =
    receiveJSON
      ( \s h x ->
          GetDeviceResponse' <$> (x .?> "Device") <*> (pure (fromEnum s))
      )
...
instance ToHeaders GetDevice where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("AlexaForBusiness.GetDevice" :: ByteString),
...
instance ToJSON GetDevice where
  toJSON GetDevice' {..} =
    object (catMaybes [("DeviceArn" .=) <$> _gdDeviceARN])
