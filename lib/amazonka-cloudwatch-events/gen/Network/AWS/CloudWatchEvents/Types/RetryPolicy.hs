...
mkRetryPolicy
    :: RetryPolicy
mkRetryPolicy
  = RetryPolicy'{maximumEventAgeInSeconds = Core.Nothing,
...
{-# INLINEABLE rpMaximumEventAgeInSeconds #-}
...
{-# INLINEABLE rpMaximumRetryAttempts #-}
...
instance Core.FromJSON RetryPolicy where
        parseJSON
          = Core.withObject "RetryPolicy" Core.$
              \ x ->
                RetryPolicy' Core.<$>
                  (x Core..:? "MaximumEventAgeInSeconds") Core.<*>
                    x Core..:? "MaximumRetryAttempts"
