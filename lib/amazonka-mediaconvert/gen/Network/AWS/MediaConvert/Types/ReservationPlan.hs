...
mkReservationPlan
    :: ReservationPlan
mkReservationPlan
  = ReservationPlan'{commitment = Core.Nothing,
                     expiresAt = Core.Nothing, purchasedAt = Core.Nothing,
                     renewalType = Core.Nothing, reservedSlots = Core.Nothing,
                     status = Core.Nothing}
...
{-# INLINEABLE rpPurchasedAt #-}
{-# DEPRECATED purchasedAt "Use generic-lens or generic-optics with 'purchasedAt' instead"  #-}
...
instance Core.FromJSON ReservationPlan where
        parseJSON
          = Core.withObject "ReservationPlan" Core.$
              \ x ->
                ReservationPlan' Core.<$>
                  (x Core..:? "commitment") Core.<*> x Core..:? "expiresAt" Core.<*>
                    x Core..:? "purchasedAt"
