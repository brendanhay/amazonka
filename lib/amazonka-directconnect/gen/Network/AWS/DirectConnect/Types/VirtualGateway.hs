...
mkVirtualGateway
    :: VirtualGateway
mkVirtualGateway
  = VirtualGateway'{virtualGatewayId = Core.Nothing,
...
{-# INLINEABLE vgVirtualGatewayState #-}
{-# DEPRECATED virtualGatewayState "Use generic-lens or generic-optics with 'virtualGatewayState' instead"  #-}
...
instance Core.FromJSON VirtualGateway where
        parseJSON
          = Core.withObject "VirtualGateway" Core.$
              \ x ->
                VirtualGateway' Core.<$>
                  (x Core..:? "virtualGatewayId") Core.<*>
                    x Core..:? "virtualGatewayState"
