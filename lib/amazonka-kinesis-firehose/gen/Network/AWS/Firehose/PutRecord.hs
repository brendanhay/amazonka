...
{-# INLINEABLE prDeliveryStreamName #-}
...
instance Core.ToHeaders PutRecord where
        toHeaders PutRecord{..}
          = Core.pure ("X-Amz-Target", "Firehose_20150804.PutRecord") Core.<>
...
instance Core.AWSRequest PutRecord where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 PutRecordResponse' Core.<$>
                   (x Core..: "RecordId") Core.<*> x Core..:? "Encrypted" Core.<*>
...
mkPutRecordResponse
    :: Types.PutResponseRecordId -- ^ 'recordId'
    -> Core.Int -- ^ 'responseStatus'
    -> PutRecordResponse
mkPutRecordResponse recordId responseStatus
  = PutRecordResponse'{recordId, encrypted = Core.Nothing,
                       responseStatus}
