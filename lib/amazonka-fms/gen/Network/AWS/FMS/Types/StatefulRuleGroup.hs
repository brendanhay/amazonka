...
mkStatefulRuleGroup
    :: StatefulRuleGroup
mkStatefulRuleGroup
  = StatefulRuleGroup'{resourceId = Core.Nothing,
...
instance Core.FromJSON StatefulRuleGroup where
        parseJSON
          = Core.withObject "StatefulRuleGroup" Core.$
              \ x ->
                StatefulRuleGroup' Core.<$>
                  (x Core..:? "ResourceId") Core.<*> x Core..:? "RuleGroupName"
