...
instance AWSRequest DescribeTrustedAdvisorCheckRefreshStatuses where
  ...
  response =
    receiveJSON
      ( \s h x ->
          DescribeTrustedAdvisorCheckRefreshStatusesResponse'
            <$> (pure (fromEnum s)) <*> (x .?> "statuses" .!@ mempty)
      )

instance Hashable DescribeTrustedAdvisorCheckRefreshStatuses

instance NFData DescribeTrustedAdvisorCheckRefreshStatuses

instance ToHeaders DescribeTrustedAdvisorCheckRefreshStatuses where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
...
instance ToJSON DescribeTrustedAdvisorCheckRefreshStatuses where
  toJSON DescribeTrustedAdvisorCheckRefreshStatuses' {..} =
    object (catMaybes [Just ("checkIds" .= _dtacrsCheckIds)])
...
instance NFData DescribeTrustedAdvisorCheckRefreshStatusesResponse
