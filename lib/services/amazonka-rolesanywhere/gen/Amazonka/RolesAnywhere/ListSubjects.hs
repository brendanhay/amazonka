    ...
    Response.receiveJSON
      ( \s h x ->
          ListSubjectsResponse'
            ...
            Prelude.<*> (x Core..?> "subjects" Core..!@ Prelude.mempty)
...
instance Core.ToHeaders ListSubjects where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "Content-Type"
...
instance Core.ToPath ListSubjects where
  toPath = Prelude.const "/subjects"

instance Core.ToQuery ListSubjects where
