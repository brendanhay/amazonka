...
mkJdbcTarget
    :: JdbcTarget
mkJdbcTarget
  = JdbcTarget'{connectionName = Core.Nothing,
                exclusions = Core.Nothing, path = Core.Nothing}
...
instance Core.FromJSON JdbcTarget where
        parseJSON
          = Core.withObject "JdbcTarget" Core.$
              \ x ->
                JdbcTarget' Core.<$>
                  (x Core..:? "ConnectionName") Core.<*> x Core..:? "Exclusions"
                    Core.<*> x Core..:? "Path"
