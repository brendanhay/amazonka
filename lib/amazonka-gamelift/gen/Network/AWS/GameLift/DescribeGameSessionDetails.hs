...
instance AWSPager DescribeGameSessionDetails where
  page rq rs
    ...
    | otherwise = Just $ rq & dgsdNextToken .~ rs ^. dgsdrsNextToken
...
instance ToHeaders DescribeGameSessionDetails where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("GameLift.DescribeGameSessionDetails" :: ByteString),
...
--
-- * 'dgsdrsNextToken' - Token that indicates where to resume retrieving results on the next call to this operation. If no token is returned, these results represent the end of the list.
--
