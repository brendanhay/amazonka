...
--
-- * 'crgAutomaticFailoverEnabled' - Specifies whether a read-only replica will be automatically promoted to read/write primary if the existing primary fails. If @true@ , Multi-AZ is enabled for this replication group. If @false@ , Multi-AZ is disabled for this replication group. Default: false
--
-- * 'crgEngineVersion' - The version number of the cache engine to be used for the cache clusters in this replication group. To view the supported cache engine versions, use the /DescribeCacheEngineVersions/ action. __Important:__ You can upgrade to a newer engine version (see <http://docs.aws.amazon.com/AmazonElastiCache/latest/UserGuide/SelectEngine.html#VersionManagement Selecting a Cache Engine and Version> ) in the /ElastiCache User Guide/ , but you cannot downgrade to an earlier engine version. If you want to use an earlier engine version, you must delete the existing cache cluster or replication group and create it anew with the earlier engine version.
--
...
--
-- * 'crgSecurityGroupIds' - One or more Amazon VPC security groups associated with this replication group. Use this parameter only when you are creating a replication group in an Amazon Virtual Private Cloud (VPC).
--
...
--
-- * 'crgCacheParameterGroupName' - The name of the parameter group to associate with this replication group. If this argument is omitted, the default cache parameter group for the specified engine is used.
--
-- * 'crgSnapshotWindow' - The daily time range (in UTC) during which ElastiCache will begin taking a daily snapshot of your node group. Example: @05:00-09:00@  If you do not specify this parameter, then ElastiCache will automatically choose an appropriate time range.
--
-- * 'crgPrimaryClusterId' - The identifier of the cache cluster that will serve as the primary for this replication group. This cache cluster must already exist and have a status of /available/ . This parameter is not required if /NumCacheClusters/ is specified.
--
-- * 'crgEngine' - The name of the cache engine to be used for the cache clusters in this replication group. Default: redis
--
...
--
-- * 'crgNumCacheClusters' - The number of cache clusters this replication group will initially have. If /Multi-AZ/ is @enabled@ , the value of this parameter must be at least 2. The maximum permitted value for /NumCacheClusters/ is 6 (primary plus 5 replicas). If you need to exceed this limit, please fill out the ElastiCache Limit Increase Request form at <http://aws.amazon.com/contact-us/elasticache-node-limit-request http://aws.amazon.com/contact-us/elasticache-node-limit-request> .
--
...
--
-- * 'crgSnapshotName' - The name of a snapshot from which to restore data into the new node group. The snapshot status changes to @restoring@ while the new node group is being created.
--
-- * 'crgNotificationTopicARN' - The Amazon Resource Name (ARN) of the Amazon Simple Notification Service (SNS) topic to which notifications will be sent.
--
...
--
-- * 'crgPort' - The port number on which each member of the replication group will accept connections.
--
...
-- | Specifies whether a read-only replica will be automatically promoted to read/write primary if the existing primary fails. If @true@ , Multi-AZ is enabled for this replication group. If @false@ , Multi-AZ is disabled for this replication group. Default: false
...
-- | The daily time range (in UTC) during which ElastiCache will begin taking a daily snapshot of your node group. Example: @05:00-09:00@  If you do not specify this parameter, then ElastiCache will automatically choose an appropriate time range.
