...
mkColumnStatisticsError
    :: ColumnStatisticsError
mkColumnStatisticsError
  = ColumnStatisticsError'{columnStatistics = Core.Nothing,
                           error = Core.Nothing}
...
{-# INLINEABLE cseColumnStatistics #-}
{-# DEPRECATED columnStatistics "Use generic-lens or generic-optics with 'columnStatistics' instead"  #-}
...
instance Core.FromJSON ColumnStatisticsError where
        parseJSON
          = Core.withObject "ColumnStatisticsError" Core.$
              \ x ->
                ColumnStatisticsError' Core.<$>
                  (x Core..:? "ColumnStatistics") Core.<*> x Core..:? "Error"
