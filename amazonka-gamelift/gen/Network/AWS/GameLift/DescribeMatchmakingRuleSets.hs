...
--
-- * 'dmrsNames' - List of one or more matchmaking rule set names to retrieve details for. (Note: The rule set name is different from the optional "name" field in the rule set body.)
--
...
data DescribeMatchmakingRuleSetsResponse = DescribeMatchmakingRuleSetsResponse'
  { _dmrssrsNextToken      :: !(Maybe Text)
  , _dmrssrsResponseStatus :: !Int
  , _dmrssrsRuleSets       :: ![MatchmakingRuleSet]
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dmrssrsNextToken' - Token that indicates where to resume retrieving results on the next call to this action. If no token is returned, these results represent the end of the list.
--
-- * 'dmrssrsResponseStatus' - -- | The response status code.
--
-- * 'dmrssrsRuleSets' - Collection of requested matchmaking rule set objects.
describeMatchmakingRuleSetsResponse
