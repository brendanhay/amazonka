...
mkSession
    :: Session
mkSession
  = Session'{details = Core.Nothing, documentName = Core.Nothing,
             endDate = Core.Nothing, outputUrl = Core.Nothing,
             owner = Core.Nothing, sessionId = Core.Nothing,
             startDate = Core.Nothing, status = Core.Nothing,
             target = Core.Nothing}
...
instance Core.FromJSON Session where
        parseJSON
          = Core.withObject "Session" Core.$
              \ x ->
                Session' Core.<$>
                  (x Core..:? "Details") Core.<*> x Core..:? "DocumentName" Core.<*>
                    x Core..:? "EndDate"
