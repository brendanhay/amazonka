...
mkConfigRule
    ...
    -> ConfigRule
mkConfigRule source
  = ConfigRule'{source, configRuleArn = Core.Nothing,
                configRuleId = Core.Nothing, configRuleName = Core.Nothing,
                configRuleState = Core.Nothing, createdBy = Core.Nothing,
                description = Core.Nothing, inputParameters = Core.Nothing,
                maximumExecutionFrequency = Core.Nothing, scope = Core.Nothing}
...
{-# INLINEABLE crConfigRuleState #-}
{-# DEPRECATED configRuleState "Use generic-lens or generic-optics with 'configRuleState' instead"  #-}
...
{-# INLINEABLE crMaximumExecutionFrequency #-}
...
instance Core.FromJSON ConfigRule where
        parseJSON
          = Core.withObject "ConfigRule" Core.$
              \ x ->
                ConfigRule' Core.<$>
                  (x Core..: "Source") Core.<*> x Core..:? "ConfigRuleArn" Core.<*>
                    x Core..:? "ConfigRuleId"
