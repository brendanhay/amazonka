...
mkCreateJob pipelineId
  = CreateJob'{pipelineId, input = Core.Nothing,
               inputs = Core.Nothing, output = Core.Nothing,
               outputKeyPrefix = Core.Nothing, outputs = Core.Nothing,
               playlists = Core.Nothing, userMetadata = Core.Nothing}
...
instance Core.AWSRequest CreateJob where
        ...
        toRequest x@Core.Request{..}
          = Core.Request{Core._rqService = Types.mkServiceConfig,
                         Core._rqMethod = Request.POST, Core._rqPath = "/2012-09-25/jobs",
