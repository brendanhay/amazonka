...
instance AWSPager ListHostedZones where
    next rq rs
        | not (_lhzrsIsTruncated rs) = Nothing
        | otherwise = Just $ rq
            { _lhzMarker = _lhzrsNextMarker rs
            }
