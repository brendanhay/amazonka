...
{-# INLINEABLE ecReplicaKmsKeyID #-}
{-# DEPRECATED replicaKmsKeyID "Use generic-lens or generic-optics with 'replicaKmsKeyID' instead"  #-}

instance Core.ToXML EncryptionConfiguration where
        toXML EncryptionConfiguration{..}
          = Core.maybe Core.mempty (Core.toXMLElement "ReplicaKmsKeyID")
              replicaKmsKeyID
