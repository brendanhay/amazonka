...
mkDelegationSet
    :: Core.NonEmpty Types.DNSName -- ^ 'nameServers'
    -> DelegationSet
mkDelegationSet nameServers
  = DelegationSet'{nameServers, callerReference = Core.Nothing,
                   id = Core.Nothing}
...
instance Core.FromXML DelegationSet where
        parseXML x
          = DelegationSet' Core.<$>
              (x Core..@ "NameServers" Core..<@>
                 Core.parseXMLNonEmpty "NameServer")
