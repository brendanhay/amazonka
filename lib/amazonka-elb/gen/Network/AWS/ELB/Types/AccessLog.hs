...
mkAccessLog
    ...
    -> AccessLog
mkAccessLog enabled
  = AccessLog'{enabled, emitInterval = Core.Nothing,
               s3BucketName = Core.Nothing, s3BucketPrefix = Core.Nothing}
...
{-# INLINEABLE alEmitInterval #-}
{-# DEPRECATED emitInterval "Use generic-lens or generic-optics with 'emitInterval' instead"  #-}
...
{-# INLINEABLE alS3BucketPrefix #-}
{-# DEPRECATED s3BucketPrefix "Use generic-lens or generic-optics with 's3BucketPrefix' instead"  #-}

instance Core.ToQuery AccessLog where
        toQuery AccessLog{..}
          = Core.toQueryPair "Enabled" enabled Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "EmitInterval")
                emitInterval
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "S3BucketName")
                s3BucketName
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "S3BucketPrefix")
                s3BucketPrefix

instance Core.FromXML AccessLog where
        parseXML x
          = AccessLog' Core.<$>
              (x Core..@ "Enabled") Core.<*> x Core..@? "EmitInterval" Core.<*>
                x Core..@? "S3BucketName"
