...
mkJobExecutionSettings
    :: JobExecutionSettings
mkJobExecutionSettings
  = JobExecutionSettings'{allowDeferredExecution = Core.Nothing,
...
{-# INLINEABLE jesAllowDeferredExecution #-}
{-# DEPRECATED allowDeferredExecution "Use generic-lens or generic-optics with 'allowDeferredExecution' instead"  #-}
...
{-# INLINEABLE jesDataAccessRoleArn #-}
...
instance Core.FromJSON JobExecutionSettings where
        toJSON JobExecutionSettings{..}
          = Core.object
              (Core.catMaybes
                 [("AllowDeferredExecution" Core..=) Core.<$>
          ...
          = Core.withObject "JobExecutionSettings" Core.$
              \ x ->
                JobExecutionSettings' Core.<$>
                  (x Core..:? "AllowDeferredExecution") Core.<*>
