...
--
-- * 'laeMaxResults' - The maximum number of results to return in a single call. To retrieve the remaining results, make another call with the returned nextToken value. The action execution history is limited to the most recent 12 months, based on action execution start times. Default value is 100.
--
...
instance Hashable ListActionExecutions where

instance NFData ListActionExecutions where
...
instance NFData ListActionExecutionsResponse where
