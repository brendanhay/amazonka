...
--
--
-- The @export snapshot@ operation supports tag-based access control via resource tags applied to the resource identified by sourceSnapshotName. For more information, see the <https://lightsail.aws.amazon.com/ls/docs/en/articles/amazon-lightsail-controlling-access-using-tags Lightsail Dev Guide> .
--
module Network.AWS.Lightsail.ExportSnapshot
    (
    -- * Creating a Request
...
instance Hashable ExportSnapshot where

instance NFData ExportSnapshot where
...
data ExportSnapshotResponse = ExportSnapshotResponse'
  { _esrsOperations     :: !(Maybe [Operation])
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'esrsOperations' - A list of objects describing the API operation.
--
...
exportSnapshotResponse pResponseStatus_ =
  ExportSnapshotResponse'
    {_esrsOperations = Nothing, _esrsResponseStatus = pResponseStatus_}
...
instance NFData ExportSnapshotResponse where
