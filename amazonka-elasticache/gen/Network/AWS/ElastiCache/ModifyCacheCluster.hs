...
-- Availability Zones. Valid values: @single-az@ | @cross-az@. This option
-- is only supported for Memcached cache clusters.
...
-- cache cluster. If @false@, then changes to the cache cluster are applied
-- on the next maintenance reboot, or the next failure reboot, whichever
-- occurs first. If you perform a @ModifyCacheCluster@ before a pending
...
-- | If @true@, then minor engine upgrades will be applied automatically to
-- the cache cluster during the maintenance window. Valid values: @true@ |
-- @false@ Default: @true@.
...
-- call) and want to add 1 more node. Specify @NumCacheNodes=6@ ((3 + 2) +
-- 1) and optionally specify an Availability Zone for the new node. Scenario
-- 3: You want to cancel all pending actions. Specify @NumCacheNodes=3@ to
-- cancel all pending actions. The Availability Zone placement of nodes
-- pending creation cannot be modified. If you wish to cancel any nodes
-- pending creation, add 0 nodes by setting @NumCacheNodes@ to the number of
-- current nodes. If @cross-az@ is specified, existing Memcached nodes
...
-- may be zero), then more nodes will be added. If the value is less than
-- the number of existing cache nodes, then nodes will be removed. If the
-- value is equal to the number of current cache nodes, then any pending add
-- or remove requests are canceled. If you are removing cache nodes, you
-- must use the @CacheNodeIdsToRemove@ parameter to provide the IDs of the
-- specific cache nodes to remove. For cache clusters running Redis, the
-- value of @NumCacheNodes@must be 1. Note:Adding or removing Memcached
-- cache nodes can be applied immediately or as a pending action. See
-- @ApplyImmediately@. A pending action to modify the number of cache nodes
-- in a cluster during its maintenance window, whether by adding or removing
-- nodes in accordance with the scale out architecture, is not queued. The
-- customer's latest request to add or remove nodes to the cluster overrides
-- any previous pending actions to modify the number of cache nodes in the
-- cluster. For example, a request to remove 2 nodes would override a
-- previous pending action to remove 3 nodes. Similarly, a request to add 2
...
-- nodes does not automatically override a previous pending action to add
-- nodes. The customer can modify the previous pending action to add more
-- nodes or explicitly cancel the pending request and retry the new request.
-- To cancel pending actions to modify the number of cache nodes in a
