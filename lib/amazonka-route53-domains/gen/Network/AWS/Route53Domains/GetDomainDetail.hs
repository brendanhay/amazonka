...
newtype GetDomainDetail = GetDomainDetail' {_gddDomainName :: Text}
...
instance ToHeaders GetDomainDetail where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("Route53Domains_v20140515.GetDomainDetail" :: ByteString),
...
instance ToJSON GetDomainDetail where
  toJSON GetDomainDetail' {..} =
    object (catMaybes [Just ("DomainName" .= _gddDomainName)])
