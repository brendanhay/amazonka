...
instance AWSRequest UpdateStack where
  ...
  response =
    receiveXMLWrapper
      "UpdateStackResult"
      ( \s h x ->
          UpdateStackResponse' <$> (x .@? "StackId") <*> (pure (fromEnum s))
      )
  ...
  toQuery UpdateStack' {..} =
    mconcat
      [ "Action" =: ("UpdateStack" :: ByteString),
        ...
        "NotificationARNs"
          =: toQuery (toQueryList "member" <$> _usNotificationARNs),
        ...
        "Parameters" =: toQuery (toQueryList "member" <$> _usParameters),
