...
instance ToHeaders RunTask where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AmazonEC2ContainerServiceV20141113.RunTask" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("overrides" .=) <$> _rtOverrides,
            ...
            ("enableECSManagedTags" .=) <$> _rtEnableECSManagedTags,
            ("count" .=) <$> _rtCount,
            ("referenceId" .=) <$> _rtReferenceId,
            ("placementConstraints" .=) <$> _rtPlacementConstraints,
            ("placementStrategy" .=) <$> _rtPlacementStrategy,
            ("startedBy" .=) <$> _rtStartedBy,
            ("launchType" .=) <$> _rtLaunchType,
            ("networkConfiguration" .=) <$> _rtNetworkConfiguration,
            ("capacityProviderStrategy" .=) <$> _rtCapacityProviderStrategy,
            ("tags" .=) <$> _rtTags,
