...
data SimulateCustomPolicy = SimulateCustomPolicy'
  { _scpResourcePolicy ::
    ...
    _scpPermissionsBoundaryPolicyInputList ::
      !(Maybe [Text]),
...
--
-- * 'scpPermissionsBoundaryPolicyInputList' - The IAM permissions boundary policy to simulate. The permissions boundary sets the maximum permissions that an IAM entity can have. You can input only one permissions boundary when you pass a policy to this operation. For more information about permissions boundaries, see <https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_boundaries.html Permissions Boundaries for IAM Entities> in the /IAM User Guide/ . The policy input is specified as a string that contains the complete, valid JSON text of a permissions boundary policy. The <http://wikipedia.org/wiki/regex regex pattern> used to validate this parameter is a string of characters consisting of the following:     * Any printable ASCII character ranging from the space character (@\u0020@ ) through the end of the ASCII character range     * The printable characters in the Basic Latin and Latin-1 Supplement character set (through @\u00FF@ )     * The special characters tab (@\u0009@ ), line feed (@\u000A@ ), and carriage return (@\u000D@ )
--
...
--
-- * 'scpMaxItems' - Use this only when paginating results to indicate the maximum number of items you want in the response. If additional items exist beyond the maximum you specify, the @IsTruncated@ response element is @true@ . If you do not include this parameter, the number of items defaults to 100. Note that IAM might return fewer results, even when there are more results available. In that case, the @IsTruncated@ response element returns @true@ , and @Marker@ contains a value to include in the subsequent call that tells the service where to continue from.
--
-- * 'scpContextEntries' - A list of context keys and corresponding values for the simulation to use. Whenever a context key is evaluated in one of the simulated IAM permissions policies, the corresponding value is supplied.
--
-- * 'scpResourceOwner' - An ARN representing the AWS account ID that specifies the owner of any simulated resource that does not identify its owner in the resource ARN. Examples of resource ARNs include an S3 bucket or object. If @ResourceOwner@ is specified, it is also used as the account owner of any @ResourcePolicy@ included in the simulation. If the @ResourceOwner@ parameter is not specified, then the owner of the resources and the resource policy defaults to the account of the identity provided in @CallerArn@ . This parameter is required only if you specify a resource-based policy and account that owns the resource is different from the account that owns the simulated calling user @CallerArn@ . The ARN for an account uses the following syntax: @arn:aws:iam::/AWS-account-ID/ :root@ . For example, to represent the account with the 112233445566 ID, use the following ARN: @arn:aws:iam::112233445566-ID:root@ .
--
-- * 'scpPolicyInputList' - A list of policy documents to include in the simulation. Each document is specified as a string containing the complete, valid JSON text of an IAM policy. Do not include any resource-based policies in this parameter. Any resource-based policy must be submitted with the @ResourcePolicy@ parameter. The policies cannot be "scope-down" policies, such as you could include in a call to <https://docs.aws.amazon.com/IAM/latest/APIReference/API_GetFederationToken.html GetFederationToken> or one of the <https://docs.aws.amazon.com/IAM/latest/APIReference/API_AssumeRole.html AssumeRole> API operations. In other words, do not use policies designed to restrict what a user can do while using the temporary credentials. The <http://wikipedia.org/wiki/regex regex pattern> used to validate this parameter is a string of characters consisting of the following:     * Any printable ASCII character ranging from the space character (@\u0020@ ) through the end of the ASCII character range     * The printable characters in the Basic Latin and Latin-1 Supplement character set (through @\u00FF@ )     * The special characters tab (@\u0009@ ), line feed (@\u000A@ ), and carriage return (@\u000D@ )
--
-- * 'scpActionNames' - A list of names of API operations to evaluate in the simulation. Each operation is evaluated against each resource. Each operation must include the service identifier, such as @iam:CreateUser@ . This operation does not support using wildcards (*) in an action name.
simulateCustomPolicy ::
  SimulateCustomPolicy
simulateCustomPolicy =
  SimulateCustomPolicy'
    { _scpResourcePolicy = Nothing,
      ...
      _scpPermissionsBoundaryPolicyInputList = Nothing,
      _scpMarker = Nothing,
...
scpPermissionsBoundaryPolicyInputList :: Lens' SimulateCustomPolicy [Text]
scpPermissionsBoundaryPolicyInputList = lens _scpPermissionsBoundaryPolicyInputList (\s a -> s {_scpPermissionsBoundaryPolicyInputList = a}) . _Default . _Coerce
  ...
  toQuery SimulateCustomPolicy' {..} =
    mconcat
      [ "Action" =: ("SimulateCustomPolicy" :: ByteString),
        ...
        "ResourceHandlingOption" =: _scpResourceHandlingOption,
        "ResourceArns"
          =: toQuery (toQueryList "member" <$> _scpResourceARNs),
        "PermissionsBoundaryPolicyInputList"
          =: toQuery
            (toQueryList "member" <$> _scpPermissionsBoundaryPolicyInputList),
        "Marker" =: _scpMarker,
        "MaxItems" =: _scpMaxItems,
        "ContextEntries"
          =: toQuery (toQueryList "member" <$> _scpContextEntries),
        "ResourceOwner" =: _scpResourceOwner,
        "PolicyInputList" =: toQueryList "member" _scpPolicyInputList,
        "ActionNames" =: toQueryList "member" _scpActionNames
      ]
