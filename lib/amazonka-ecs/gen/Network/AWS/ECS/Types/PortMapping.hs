...
mkPortMapping
    :: PortMapping
mkPortMapping
  = PortMapping'{containerPort = Core.Nothing,
                 hostPort = Core.Nothing, protocol = Core.Nothing}
...
{-# INLINEABLE pmHostPort #-}
{-# DEPRECATED hostPort "Use generic-lens or generic-optics with 'hostPort' instead"  #-}
...
instance Core.FromJSON PortMapping where
        parseJSON
          = Core.withObject "PortMapping" Core.$
              \ x ->
                PortMapping' Core.<$>
                  (x Core..:? "containerPort") Core.<*> x Core..:? "hostPort"
