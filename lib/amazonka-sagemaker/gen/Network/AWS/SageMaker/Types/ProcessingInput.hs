...
mkProcessingInput
    :: Core.Text -- ^ 'inputName'
    -> Types.ProcessingS3Input -- ^ 's3Input'
    -> ProcessingInput
...
piInputName :: Lens.Lens' ProcessingInput Core.Text
piInputName = Lens.field @"inputName"
{-# INLINEABLE piInputName #-}
...
{-# INLINEABLE piS3Input #-}
{-# DEPRECATED s3Input "Use generic-lens or generic-optics with 's3Input' instead"  #-}
...
instance Core.FromJSON ProcessingInput where
        parseJSON
          = Core.withObject "ProcessingInput" Core.$
              \ x ->
                ProcessingInput' Core.<$>
                  (x Core..: "InputName") Core.<*> x Core..: "S3Input"
