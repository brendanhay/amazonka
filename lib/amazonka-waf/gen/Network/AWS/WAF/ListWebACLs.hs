...
listWebACLs ::
  ListWebACLs
listWebACLs =
  ListWebACLs' {_lwaNextMarker = Nothing, _lwaLimit = Nothing}
...
instance AWSPager ListWebACLs where
  page rq rs
    | stop (rs ^. lwarsNextMarker) = Nothing
    | stop (rs ^. lwarsWebACLs) = Nothing
    | otherwise = Just $ rq & lwaNextMarker .~ rs ^. lwarsNextMarker
...
instance ToHeaders ListWebACLs where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("AWSWAF_20150824.ListWebACLs" :: ByteString),
...
instance ToJSON ListWebACLs where
  toJSON ListWebACLs' {..} =
    object
      ( catMaybes
          [("NextMarker" .=) <$> _lwaNextMarker, ("Limit" .=) <$> _lwaLimit]
      )
