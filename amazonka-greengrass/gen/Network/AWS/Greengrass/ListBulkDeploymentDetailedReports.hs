...
data ListBulkDeploymentDetailedReports = ListBulkDeploymentDetailedReports'
  { _lbddrNextToken        :: !(Maybe Text)
  , _lbddrMaxResults       :: !(Maybe Text)
  , _lbddrBulkDeploymentId :: !Text
...
instance AWSPager ListBulkDeploymentDetailedReports
         where
        page rq rs
          | stop (rs ^. lbddrrsNextToken) = Nothing
          | stop (rs ^. lbddrrsDeployments) = Nothing
          | otherwise =
            Just $ rq & lbddrNextToken .~ rs ^. lbddrrsNextToken

instance AWSRequest ListBulkDeploymentDetailedReports
         where
...
instance ToHeaders ListBulkDeploymentDetailedReports
         where
        toHeaders
          = const
              (mconcat
...
instance ToPath ListBulkDeploymentDetailedReports
         where
        toPath ListBulkDeploymentDetailedReports'{..}
          = mconcat
              ["/greengrass/bulk/deployments/",
               toBS _lbddrBulkDeploymentId, "/detailed-reports"]

instance ToQuery ListBulkDeploymentDetailedReports
         where
...
data ListBulkDeploymentDetailedReportsResponse = ListBulkDeploymentDetailedReportsResponse'
  { _lbddrrsNextToken      :: !(Maybe Text)
  , _lbddrrsDeployments    :: !(Maybe [BulkDeploymentResult])
  , _lbddrrsResponseStatus :: !Int
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
