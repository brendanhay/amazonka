...
mkRtmpOutputSettings destination
  = RtmpOutputSettings'{destination, certificateMode = Core.Nothing,
                        connectionRetryInterval = Core.Nothing, numRetries = Core.Nothing}
...
{-# INLINEABLE rosConnectionRetryInterval #-}
          ...
          = Core.withObject "RtmpOutputSettings" Core.$
              \ x ->
                RtmpOutputSettings' Core.<$>
                  (x Core..: "destination") Core.<*> x Core..:? "certificateMode"
