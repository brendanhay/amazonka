...
data CreateGovCloudAccount = CreateGovCloudAccount'
  { _cgcaIAMUserAccessToBilling ::
    ...
    _cgcaRoleName :: !(Maybe Text),
    _cgcaTags :: !(Maybe [Tag]),
    _cgcaEmail :: !(Sensitive Text),
    _cgcaAccountName :: !(Sensitive Text)
  }
...
instance ToHeaders CreateGovCloudAccount where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AWSOrganizationsV20161128.CreateGovCloudAccount" :: ByteString),
...
instance ToJSON CreateGovCloudAccount where
  toJSON CreateGovCloudAccount' {..} =
    object
      ( catMaybes
          [ ("IamUserAccessToBilling" .=) <$> _cgcaIAMUserAccessToBilling,
