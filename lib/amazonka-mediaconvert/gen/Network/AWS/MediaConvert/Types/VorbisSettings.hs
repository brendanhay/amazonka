...
mkVorbisSettings
    :: VorbisSettings
mkVorbisSettings
  = VorbisSettings'{channels = Core.Nothing,
                    sampleRate = Core.Nothing, vbrQuality = Core.Nothing}
...
instance Core.FromJSON VorbisSettings where
        parseJSON
          = Core.withObject "VorbisSettings" Core.$
              \ x ->
                VorbisSettings' Core.<$>
                  (x Core..:? "channels") Core.<*> x Core..:? "sampleRate" Core.<*>
                    x Core..:? "vbrQuality"
