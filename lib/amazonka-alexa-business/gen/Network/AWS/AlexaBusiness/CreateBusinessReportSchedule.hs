...
instance ToHeaders CreateBusinessReportSchedule where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AlexaForBusiness.CreateBusinessReportSchedule" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("S3KeyPrefix" .=) <$> _cbrsS3KeyPrefix,
            ...
            ("ClientRequestToken" .=) <$> _cbrsClientRequestToken,
...
createBusinessReportScheduleResponse pResponseStatus_ =
  CreateBusinessReportScheduleResponse'
    { _cbrsrsScheduleARN =
        Nothing,
      _cbrsrsResponseStatus = pResponseStatus_
    }
