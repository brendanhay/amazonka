...
mkRotateSecret secretId
  = RotateSecret'{secretId, clientRequestToken = Core.Nothing,
...
{-# INLINEABLE rsClientRequestToken #-}
...
{-# INLINEABLE rsRotationLambdaARN #-}
...
instance Core.ToHeaders RotateSecret where
        toHeaders RotateSecret{..}
          = Core.pure ("X-Amz-Target", "secretsmanager.RotateSecret") Core.<>
...
mkRotateSecretResponse responseStatus
  = RotateSecretResponse'{arn = Core.Nothing, name = Core.Nothing,
