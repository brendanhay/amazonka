...
--
--     * Create and update the predicates that you want to include in the @Rule@ .    * Create the @Rule@ . See 'CreateRule' .    * Use @GetChangeToken@ to get the change token that you provide in the @ChangeToken@ parameter of an 'UpdateRule' request.    * Submit an @UpdateRule@ request to add predicates to the @Rule@ .    * Create and update a @WebACL@ that contains the @Rule@ . See 'CreateWebACL' .
--
