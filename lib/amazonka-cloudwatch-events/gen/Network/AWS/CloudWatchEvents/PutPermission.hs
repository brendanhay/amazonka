...
mkPutPermission
    :: PutPermission
mkPutPermission
  = PutPermission'{action = Core.Nothing, condition = Core.Nothing,
                   eventBusName = Core.Nothing, policy = Core.Nothing,
                   principal = Core.Nothing, statementId = Core.Nothing}
...
instance Core.ToHeaders PutPermission where
        toHeaders PutPermission{..}
          = Core.pure ("X-Amz-Target", "AWSEvents.PutPermission") Core.<>
...
instance Core.AWSRequest PutPermission where
        ...
        {-# INLINE toRequest #-}
        parseResponse = Response.receiveNull PutPermissionResponse'
