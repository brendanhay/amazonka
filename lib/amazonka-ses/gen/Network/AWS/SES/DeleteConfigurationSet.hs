...
{-# INLINEABLE dConfigurationSetName #-}
...
instance Core.ToQuery DeleteConfigurationSet where
        toQuery DeleteConfigurationSet{..}
          = Core.toQueryPair "Action" ("DeleteConfigurationSet" :: Core.Text)
...
instance Core.AWSRequest DeleteConfigurationSet where
        ...
        {-# INLINE toRequest #-}
        parseResponse
          = Response.receiveXMLWrapper "DeleteConfigurationSetResult"
              (\ s h x ->
                 DeleteConfigurationSetResponse' Core.<$>
