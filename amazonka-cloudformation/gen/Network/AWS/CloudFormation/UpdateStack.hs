...
--
-- * 'usUsePreviousTemplate' - Reuse the existing template that is associated with the stack that you are updating.
--
...
--
-- * 'usTemplateBody' - Structure containing the template body with a minimum length of 1 byte and a maximum length of 51,200 bytes. (For more information, go to <http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/template-anatomy.html Template Anatomy> in the AWS CloudFormation User Guide.) Conditional: You must specify either the @TemplateBody@ or the @TemplateURL@ parameter, but not both.
--
