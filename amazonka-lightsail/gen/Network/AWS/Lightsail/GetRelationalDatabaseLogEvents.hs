...
data GetRelationalDatabaseLogEvents = GetRelationalDatabaseLogEvents'
  { _grdleStartTime              :: !(Maybe POSIX)
  , _grdleStartFromHead          :: !(Maybe Bool)
  , _grdleEndTime                :: !(Maybe POSIX)
  , _grdlePageToken              :: !(Maybe Text)
  , _grdleRelationalDatabaseName :: !Text
...
--
-- * 'grdleStartTime' - The start of the time interval from which to get log events. Constraints:     * Specified in Universal Coordinated Time (UTC).     * Specified in the Unix time format. For example, if you wish to use a start time of October 1, 2018, at 8 PM UTC, then you input @1538424000@ as the start time.
--
-- * 'grdleStartFromHead' - Parameter to specify if the log should start from head or tail. If @true@ is specified, the log event starts from the head of the log. If @false@ is specified, the log event starts from the tail of the log. Default: @false@
--
-- * 'grdleEndTime' - The end of the time interval from which to get log events. Constraints:     * Specified in Universal Coordinated Time (UTC).     * Specified in the Unix time format. For example, if you wish to use an end time of October 1, 2018, at 8 PM UTC, then you input @1538424000@ as the end time.
--
-- * 'grdlePageToken' - A token used for advancing to a specific page of results for your @get relational database log events@ request.
--
...
getRelationalDatabaseLogEvents
    :: Text -- ^ 'grdleRelationalDatabaseName'
    -> Text -- ^ 'grdleLogStreamName'
    -> GetRelationalDatabaseLogEvents
getRelationalDatabaseLogEvents pRelationalDatabaseName_ pLogStreamName_ =
  GetRelationalDatabaseLogEvents'
...
instance AWSRequest GetRelationalDatabaseLogEvents
         where
...
instance NFData GetRelationalDatabaseLogEvents where

instance ToHeaders GetRelationalDatabaseLogEvents
         where
        toHeaders
          = const
              (mconcat
...
data GetRelationalDatabaseLogEventsResponse = GetRelationalDatabaseLogEventsResponse'
  { _grdlersNextBackwardToken :: !(Maybe Text)
  , _grdlersResourceLogEvents :: !(Maybe [LogEvent])
  , _grdlersNextForwardToken  :: !(Maybe Text)
  , _grdlersResponseStatus    :: !Int
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
...
getRelationalDatabaseLogEventsResponse
    :: Int -- ^ 'grdlersResponseStatus'
    -> GetRelationalDatabaseLogEventsResponse
getRelationalDatabaseLogEventsResponse pResponseStatus_ =
  GetRelationalDatabaseLogEventsResponse'
    { _grdlersNextBackwardToken = Nothing
