...
instance AWSPager DescribeVPCEndpointConnectionNotifications where
  page rq rs
    | stop (rs ^. dvpcecnrsNextToken) = Nothing
    | stop (rs ^. dvpcecnrsConnectionNotificationSet) = Nothing
    | otherwise =
      Just $ rq & dvpcecnNextToken .~ rs ^. dvpcecnrsNextToken

instance AWSRequest DescribeVPCEndpointConnectionNotifications where
  type
...
instance Hashable DescribeVPCEndpointConnectionNotifications

instance NFData DescribeVPCEndpointConnectionNotifications

instance ToHeaders DescribeVPCEndpointConnectionNotifications where
  toHeaders = const mempty

instance ToPath DescribeVPCEndpointConnectionNotifications where
  toPath = const "/"

instance ToQuery DescribeVPCEndpointConnectionNotifications where
  toQuery DescribeVPCEndpointConnectionNotifications' {..} =
    mconcat
      [ "Action"
          =: ("DescribeVpcEndpointConnectionNotifications" :: ByteString),
        "Version" =: ("2016-11-15" :: ByteString),
        toQuery (toQueryList "Filter" <$> _dvpcecnFilters),
        "NextToken" =: _dvpcecnNextToken,
        "ConnectionNotificationId" =: _dvpcecnConnectionNotificationId,
...
data DescribeVPCEndpointConnectionNotificationsResponse = DescribeVPCEndpointConnectionNotificationsResponse'
  { _dvpcecnrsConnectionNotificationSet ::
      !( Maybe
...
instance NFData DescribeVPCEndpointConnectionNotificationsResponse
