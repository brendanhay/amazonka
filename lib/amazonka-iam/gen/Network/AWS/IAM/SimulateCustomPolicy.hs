  ...
  request x@Core.Request {..} =
    Core.Request
      { Core._rqService = Types.mkServiceConfig,
        ...
        Core._rqBody =
          Core.toFormBody
            ( Core.pure ("Action", "SimulateCustomPolicy")
                ...
                Core.<> (Core.toQueryValue "CallerArn" Core.<$> callerArn)
                Core.<> ( Core.toQueryValue
                            "ContextEntries"
                ...
                Core.<> ( Core.toQueryValue "ResourceHandlingOption"
                            Core.<$> resourceHandlingOption
                        )
                Core.<> (Core.toQueryValue "ResourceOwner" Core.<$> resourceOwner)
                Core.<> (Core.toQueryValue "ResourcePolicy" Core.<$> resourcePolicy)
            )
      }
  response =
