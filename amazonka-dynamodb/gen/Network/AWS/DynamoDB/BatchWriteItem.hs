...
-- items to be written can be as large as 400 KB. The individual /PutItem/ and
-- /DeleteItem/ operations specified in /BatchWriteItem/ are atomic; however
-- /BatchWriteItem/ as a whole is not. If any requested operations fail
...
-- resend the requests. Typically, you would call /BatchWriteItem/ in a loop.
-- Each iteration would check for unprocessed items and submit a new
-- /BatchWriteItem/ request with those unprocessed items until all items have
-- been processed. Note that if /none/ of the items can be processed due to
...
-- Guide/. With /BatchWriteItem/, you can efficiently write or delete large
...
-- /BatchWriteItem/ does not return deleted items in the response. If you use
-- a programming language that supports concurrency, such as Java, you can use
-- threads to write items in parallel. Your application must include the
-- necessary logic to manage the threads. With languages that don't support
-- threading, such as PHP, you must update or delete the specified items one
-- at a time. In both situations, /BatchWriteItem/ provides an alternative
-- where the API performs the specified put and delete operations in parallel,
-- giving you the power of the thread pool approach without having to
-- introduce complexity into your application. Parallel processing reduces
...
-- more of the following is true, DynamoDB rejects the entire batch write
-- operation: One or more tables specified in the /BatchWriteItem/ request
-- does not exist. Primary key attributes specified on an item in the request
-- do not match those in the corresponding table's primary key schema. You try
...
-- item in the same /BatchWriteItem/ request. There are more than 25 requests
-- in the batch. Any individual item in a batch exceeds 400 KB. The total
-- request size exceeds 16 MB.
--
...
-- element in the map consists of the following: /DeleteRequest/ - Perform a
-- /DeleteItem/ operation on the specified item. The item to be deleted is
-- identified by a /Key/ subelement: /Key/ - A map of primary key attribute
-- values that uniquely identify the ! item. Each entry in this map consists
-- of an attribute name and an attribute value. For each primary key, you
-- must provide /all/ of the key attributes. For example, with a hash type
...
-- attribute and the range attribute. /PutRequest/ - Perform a /PutItem/
-- operation on the specified item. The item to be put is identified by an
-- /Item/ subelement: /Item/ - A map of attributes and their values. Each
...
-- individual /DeleteItem/ or /PutItem/ operations. Each entry consists of
-- the following subelements: /ItemCollectionKey/ - The hash key value of
-- the item collection. This is the same as the hash key of the item.
...
-- (/DeleteRequest/ or /PutRequest/). /DeleteRequest/ - Perform a
-- /DeleteItem/ operation on the specified item. The item to be deleted is
-- identified by a /Key/ subelement: /Key/ - A map of primary key attribute
-- values that uniquely identify the item. Each entry in this map consists
-- of an attribute name and an attribute value. /PutRequest/ - Perform a
...
-- rejected with a /ValidationException/ exception. If you specify any
...
-- definition. If there are no unprocessed items remaining, the response
