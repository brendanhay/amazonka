...
mkDescribeImages
    :: DescribeImages
mkDescribeImages
  = DescribeImages'{dryRun = Core.Nothing,
                    executableUsers = Core.Nothing, filters = Core.Nothing,
                    imageIds = Core.Nothing, owners = Core.Nothing}
...
dihExecutableUsers :: Lens.Lens' DescribeImages (Core.Maybe [Core.Text])
dihExecutableUsers = Lens.field @"executableUsers"
{-# INLINEABLE dihExecutableUsers #-}
{-# DEPRECATED executableUsers "Use generic-lens or generic-optics with 'executableUsers' instead"  #-}
...
dihOwners :: Lens.Lens' DescribeImages (Core.Maybe [Core.Text])
dihOwners = Lens.field @"owners"
{-# INLINEABLE dihOwners #-}
...
instance Core.ToQuery DescribeImages where
        toQuery DescribeImages{..}
          = Core.toQueryPair "Action" ("DescribeImages" :: Core.Text) Core.<>
              Core.toQueryPair "Version" ("2016-11-15" :: Core.Text)
              Core.<> Core.maybe Core.mempty (Core.toQueryPair "DryRun") dryRun
              Core.<>
              Core.maybe Core.mempty (Core.toQueryList "ExecutableBy")
                executableUsers
              Core.<> Core.maybe Core.mempty (Core.toQueryList "Filter") filters
              Core.<>
              Core.maybe Core.mempty (Core.toQueryList "ImageId") imageIds
          ...
          = Response.receiveXML
              (\ s h x ->
                 DescribeImagesResponse' Core.<$>
                   (x Core..@? "imagesSet" Core..<@> Core.parseXMLList "item")
