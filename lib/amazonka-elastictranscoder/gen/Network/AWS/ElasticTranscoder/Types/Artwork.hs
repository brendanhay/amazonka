...
mkArtwork
    :: Artwork
mkArtwork
  = Artwork'{albumArtFormat = Core.Nothing,
             encryption = Core.Nothing, inputKey = Core.Nothing,
             maxHeight = Core.Nothing, maxWidth = Core.Nothing,
             paddingPolicy = Core.Nothing, sizingPolicy = Core.Nothing}
...
{-# INLINEABLE aAlbumArtFormat #-}
{-# DEPRECATED albumArtFormat "Use generic-lens or generic-optics with 'albumArtFormat' instead"  #-}
...
{-# INLINEABLE aInputKey #-}
{-# DEPRECATED inputKey "Use generic-lens or generic-optics with 'inputKey' instead"  #-}
...
instance Core.FromJSON Artwork where
        parseJSON
          = Core.withObject "Artwork" Core.$
              \ x ->
                Artwork' Core.<$>
                  (x Core..:? "AlbumArtFormat") Core.<*> x Core..:? "Encryption"
