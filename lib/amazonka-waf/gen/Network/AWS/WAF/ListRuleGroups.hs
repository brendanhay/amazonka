...
instance Page.AWSPager ListRuleGroups where
  page rq rs
    | Page.stop (rs Lens.^. lrgrsNextMarker) = Lude.Nothing
    | Page.stop (rs Lens.^. lrgrsRuleGroups) = Lude.Nothing
    | Lude.otherwise =
      Lude.Just Lude.$
        rq
          Lude.& lrgNextMarker Lens..~ rs Lens.^. lrgrsNextMarker
...
instance Lude.ToHeaders ListRuleGroups where
  toHeaders =
    Lude.const
      ( Lude.mconcat
          [ "X-Amz-Target"
              Lude.=# ("AWSWAF_20150824.ListRuleGroups" :: Lude.ByteString),
            "Content-Type"
