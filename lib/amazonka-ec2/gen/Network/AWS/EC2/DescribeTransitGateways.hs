...
instance AWSPager DescribeTransitGateways where
  page rq rs
    ...
    | otherwise = Just $ rq & dtgsNextToken .~ rs ^. dtgrsNextToken
...
instance ToQuery DescribeTransitGateways where
  toQuery DescribeTransitGateways' {..} =
    mconcat
      [ "Action" =: ("DescribeTransitGateways" :: ByteString),
        "Version" =: ("2016-11-15" :: ByteString),
        toQuery (toQueryList "Filter" <$> _dtgsFilters),
        toQuery
          (toQueryList "TransitGatewayIds" <$> _dtgsTransitGatewayIds),
