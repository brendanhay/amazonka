...
mkPutRecord streamName data' partitionKey
  = PutRecord'{streamName, data', partitionKey,
...
{-# INLINEABLE prSequenceNumberForOrdering #-}
{-# DEPRECATED sequenceNumberForOrdering "Use generic-lens or generic-optics with 'sequenceNumberForOrdering' instead"  #-}
...
instance Core.ToHeaders PutRecord where
        toHeaders PutRecord{..}
          = Core.pure ("X-Amz-Target", "Kinesis_20131202.PutRecord") Core.<>
          ...
          = Core.object
              (Core.catMaybes
                 [Core.Just ("StreamName" Core..= streamName),
                  ...
                  ("SequenceNumberForOrdering" Core..=) Core.<$>
          ...
          = Response.receiveJSON
              (\ s h x ->
                 PutRecordResponse' Core.<$>
                   (x Core..: "ShardId") Core.<*> x Core..: "SequenceNumber" Core.<*>
                     x Core..:? "EncryptionType"
...
mkPutRecordResponse shardId sequenceNumber responseStatus
  = PutRecordResponse'{shardId, sequenceNumber,
                       encryptionType = Core.Nothing, responseStatus}
...
{-# INLINEABLE prrrsSequenceNumber #-}
...
{-# INLINEABLE prrrsEncryptionType #-}
