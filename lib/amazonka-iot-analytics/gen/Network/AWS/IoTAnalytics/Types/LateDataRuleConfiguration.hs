...
mkLateDataRuleConfiguration
    :: LateDataRuleConfiguration
mkLateDataRuleConfiguration
  = LateDataRuleConfiguration'{deltaTimeSessionWindowConfiguration =
                                 Core.Nothing}
...
{-# INLINEABLE ldrcDeltaTimeSessionWindowConfiguration #-}
{-# DEPRECATED deltaTimeSessionWindowConfiguration "Use generic-lens or generic-optics with 'deltaTimeSessionWindowConfiguration' instead"  #-}

instance Core.FromJSON LateDataRuleConfiguration where
        toJSON LateDataRuleConfiguration{..}
          = Core.object
              (Core.catMaybes
                 [("deltaTimeSessionWindowConfiguration" Core..=) Core.<$>
...
instance Core.FromJSON LateDataRuleConfiguration where
        parseJSON
          = Core.withObject "LateDataRuleConfiguration" Core.$
              \ x ->
                LateDataRuleConfiguration' Core.<$>
                  (x Core..:? "deltaTimeSessionWindowConfiguration")
