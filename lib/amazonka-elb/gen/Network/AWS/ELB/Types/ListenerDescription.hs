...
mkListenerDescription
    :: ListenerDescription
mkListenerDescription
  = ListenerDescription'{listener = Core.Nothing,
...
{-# INLINEABLE ldListener #-}
{-# DEPRECATED listener "Use generic-lens or generic-optics with 'listener' instead"  #-}
...
instance Core.FromXML ListenerDescription where
        parseXML x
          = ListenerDescription' Core.<$>
              (x Core..@? "Listener") Core.<*>
                x Core..@? "PolicyNames" Core..<@> Core.parseXMLList "member"
