...
mkProtection
    :: Protection
mkProtection
  = Protection'{healthCheckIds = Core.Nothing, id = Core.Nothing,
                name = Core.Nothing, resourceArn = Core.Nothing}
...
{-# INLINEABLE pHealthCheckIds #-}
{-# DEPRECATED healthCheckIds "Use generic-lens or generic-optics with 'healthCheckIds' instead"  #-}
...
instance Core.FromJSON Protection where
        parseJSON
          = Core.withObject "Protection" Core.$
              \ x ->
                Protection' Core.<$>
                  (x Core..:? "HealthCheckIds") Core.<*> x Core..:? "Id" Core.<*>
                    x Core..:? "Name"
