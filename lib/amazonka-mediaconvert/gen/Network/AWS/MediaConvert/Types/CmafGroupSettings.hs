...
data CmafGroupSettings = CmafGroupSettings'
  { -- | Length of fragments to generate (in seconds). Fragment length must be compatible with GOP size and Framerate. Note that fragments will end on the next keyframe after this number of seconds, so actual fragment length may be longer. When Emit Single File is checked, the fragmentation is internal to a single output file and it does not cause the creation of many output files as in other output types.
    ...
    additionalManifests :: Lude.Maybe [CmafAdditionalManifest],
    ...
    manifestDurationFormat :: Lude.Maybe CmafManifestDurationFormat,
    ...
    writeSegmentTimelineInRepresentation :: Lude.Maybe CmafWriteSegmentTimelineInRepresentation,
    -- | Include or exclude RESOLUTION attribute for video in EXT-X-STREAM-INF tag of variant manifest.
    streamInfResolution :: Lude.Maybe CmafStreamInfResolution,
    -- | When set to GZIP, compresses HLS playlist.
    manifestCompression :: Lude.Maybe CmafManifestCompression
  }
