...
instance AWSPager DescribeLaunchTemplates where
  page rq rs
    ...
    | otherwise = Just $ rq & dltsNextToken .~ rs ^. dltsrsNextToken
...
instance AWSRequest DescribeLaunchTemplates where
  ...
  response =
    receiveXML
      ( \s h x ->
          DescribeLaunchTemplatesResponse'
            <$> (x .@? "launchTemplates" .!@ mempty >>= may (parseXMLList "item"))
            <*> (x .@? "nextToken")
            <*> (pure (fromEnum s))
      )
...
instance ToQuery DescribeLaunchTemplates where
  toQuery DescribeLaunchTemplates' {..} =
    mconcat
      [ "Action" =: ("DescribeLaunchTemplates" :: ByteString),
        "Version" =: ("2016-11-15" :: ByteString),
        toQuery (toQueryList "Filter" <$> _dltsFilters),
        "NextToken" =: _dltsNextToken,
        toQuery
          (toQueryList "LaunchTemplateId" <$> _dltsLaunchTemplateIds),
        "DryRun" =: _dltsDryRun,
        "MaxResults" =: _dltsMaxResults,
        toQuery
          (toQueryList "LaunchTemplateName" <$> _dltsLaunchTemplateNames)
      ]
