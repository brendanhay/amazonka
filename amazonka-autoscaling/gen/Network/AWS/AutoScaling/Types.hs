...
-- the instance. Each mapping is made up of a virtualName and a deviceName.
...
-- second (IOPS) to provision for the volume. Valid values: Range is 100 to
-- 4000. Default: None.
...
-- VolumeSize, VolumeSize must be equal to or larger than the size of the
-- snapshot. Default: If you create a volume from a snapshot and you don't
-- specify a volume size, the default is the size of the snapshot. Required:
...
-- message sent to the target includes the following: Lifecycle action token
-- User account ID Name of the Auto Scaling group Lifecycle hook name EC2
-- instance ID Lifecycle transition Notification metadata.
