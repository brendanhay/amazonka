...
mkSegmentGroup
    :: SegmentGroup
mkSegmentGroup
  = SegmentGroup'{dimensions = Core.Nothing,
                  sourceSegments = Core.Nothing, sourceType = Core.Nothing,
                  type' = Core.Nothing}
...
{-# INLINEABLE sgSourceSegments #-}
{-# DEPRECATED sourceSegments "Use generic-lens or generic-optics with 'sourceSegments' instead"  #-}
...
instance Core.FromJSON SegmentGroup where
        parseJSON
          = Core.withObject "SegmentGroup" Core.$
              \ x ->
                SegmentGroup' Core.<$>
                  (x Core..:? "Dimensions") Core.<*> x Core..:? "SourceSegments"
