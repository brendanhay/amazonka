...
instance Page.AWSPager ListRegexMatchSets where
  page rq rs
    | Page.stop (rs Lens.^. lrmsrsNextMarker) = Lude.Nothing
    | Page.stop (rs Lens.^. lrmsrsRegexMatchSets) = Lude.Nothing
    | Lude.otherwise =
      Lude.Just Lude.$
        rq
          Lude.& lrmsNextMarker Lens..~ rs Lens.^. lrmsrsNextMarker
...
instance Lude.ToHeaders ListRegexMatchSets where
  toHeaders =
    Lude.const
      ( Lude.mconcat
          [ "X-Amz-Target"
              Lude.=# ("AWSWAF_20150824.ListRegexMatchSets" :: Lude.ByteString),
            "Content-Type"
...
data ListRegexMatchSetsResponse = ListRegexMatchSetsResponse'
  { -- | An array of 'RegexMatchSetSummary' objects.
    regexMatchSets :: Lude.Maybe [RegexMatchSetSummary],
