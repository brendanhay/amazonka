...
mkDomainSuggestion
    :: DomainSuggestion
mkDomainSuggestion
  = DomainSuggestion'{availability = Core.Nothing,
...
dAvailability :: Lens.Lens' DomainSuggestion (Core.Maybe Core.Text)
...
instance Core.FromJSON DomainSuggestion where
        parseJSON
          = Core.withObject "DomainSuggestion" Core.$
              \ x ->
                DomainSuggestion' Core.<$>
                  (x Core..:? "Availability") Core.<*> x Core..:? "DomainName"
