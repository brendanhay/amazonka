...
mkDescribeBundleTasks
    :: DescribeBundleTasks
mkDescribeBundleTasks
  = DescribeBundleTasks'{bundleIds = Core.Nothing,
...
{-# INLINEABLE dbtFilters #-}
{-# DEPRECATED filters "Use generic-lens or generic-optics with 'filters' instead"  #-}

instance Core.ToQuery DescribeBundleTasks where
        toQuery DescribeBundleTasks{..}
          = Core.toQueryPair "Action" ("DescribeBundleTasks" :: Core.Text)
              Core.<> Core.toQueryPair "Version" ("2016-11-15" :: Core.Text)
              Core.<>
              Core.maybe Core.mempty (Core.toQueryList "BundleId") bundleIds
...
instance Core.AWSRequest DescribeBundleTasks where
        ...
        parseResponse
          = Response.receiveXML
              (\ s h x ->
                 DescribeBundleTasksResponse' Core.<$>
                   (x Core..@? "bundleInstanceTasksSet" Core..<@>
...
mkDescribeBundleTasksResponse responseStatus
  = DescribeBundleTasksResponse'{bundleTasks = Core.Nothing,
                                 responseStatus}
...
{-# INLINEABLE dbtrrsBundleTasks #-}
{-# DEPRECATED bundleTasks "Use generic-lens or generic-optics with 'bundleTasks' instead"  #-}
...
{-# INLINEABLE dbtrrsResponseStatus #-}
