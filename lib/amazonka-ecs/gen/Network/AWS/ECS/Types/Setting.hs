...
mkSetting
    :: Setting
mkSetting
  = Setting'{name = Core.Nothing, principalArn = Core.Nothing,
             value = Core.Nothing}
...
instance Core.FromJSON Setting where
        parseJSON
          = Core.withObject "Setting" Core.$
              \ x ->
                Setting' Core.<$>
                  (x Core..:? "name") Core.<*> x Core..:? "principalArn" Core.<*>
                    x Core..:? "value"
