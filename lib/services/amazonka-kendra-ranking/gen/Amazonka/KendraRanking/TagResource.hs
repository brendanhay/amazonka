...
-- |
-- Module      : Amazonka.KendraRanking.TagResource
...
-- provisioning the @Rescore@ API. If the tag already exists, the existing
...
--
-- 'resourceARN', 'tagResource_resourceARN' - The Amazon Resource Name (ARN) of the rescore execution plan to tag.
--
-- 'tags', 'tagResource_tags' - A list of tag keys to add to a rescore execution plan. If a tag already
-- exists, the existing value is replaced with the new value.
newTagResource ::
  -- | 'resourceARN'
  Prelude.Text ->
...
instance Data.ToHeaders TagResource where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "X-Amz-Target"
              Data.=# ( "AWSKendraRerankingFrontendService.TagResource" ::
                          Prelude.ByteString
                      ),
            "Content-Type"
