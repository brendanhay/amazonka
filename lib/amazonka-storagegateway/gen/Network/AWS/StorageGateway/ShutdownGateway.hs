...
newtype ShutdownGateway = ShutdownGateway' {_sGatewayARN :: Text}
...
instance ToHeaders ShutdownGateway where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("StorageGateway_20130630.ShutdownGateway" :: ByteString),
...
instance ToJSON ShutdownGateway where
  toJSON ShutdownGateway' {..} =
    object (catMaybes [Just ("GatewayARN" .= _sGatewayARN)])
