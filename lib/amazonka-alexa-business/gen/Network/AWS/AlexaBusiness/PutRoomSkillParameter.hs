...
instance AWSRequest PutRoomSkillParameter where
  ...
  response =
    receiveEmpty
      (\s h x -> PutRoomSkillParameterResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders PutRoomSkillParameter where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AlexaForBusiness.PutRoomSkillParameter" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("RoomArn" .=) <$> _prspRoomARN,
            ...
            Just ("RoomSkillParameter" .= _prspRoomSkillParameter)
          ]
      )
