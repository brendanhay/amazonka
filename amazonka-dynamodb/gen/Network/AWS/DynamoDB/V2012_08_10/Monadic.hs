...
--
-- As an example: using "Network.AWS.DynamoDB" with the smart constructor and
...
-- @
--
-- Versus using "Network.AWS.DynamoDB.Monadic" with the 'State' operator variants from
...
batchGetItemCatch :: ( MonadCatch m
                     , MonadResource m
                     , MonadReader Env m
                     )
    => Map Text KeysAndAttributes -- ^ 'bgiRequestItems'
...
batchWriteItemCatch :: ( MonadCatch m
                       , MonadResource m
                       , MonadReader Env m
                       )
    => Map Text (List1 WriteRequest) -- ^ 'bwiRequestItems'
