...
mkGetItem
    ...
    -> GetItem
mkGetItem tableName
  = GetItem'{tableName, key = Core.mempty,
             attributesToGet = Core.Nothing, consistentRead = Core.Nothing,
...
{-# INLINEABLE giExpressionAttributeNames #-}
...
{-# INLINEABLE giProjectionExpression #-}
...
{-# INLINEABLE giReturnConsumedCapacity #-}
...
instance Core.ToHeaders GetItem where
        toHeaders GetItem{..}
          = Core.pure ("X-Amz-Target", "DynamoDB_20120810.GetItem") Core.<>
          ...
          = Core.object
              (Core.catMaybes
                 [Core.Just ("TableName" Core..= tableName),
                  ...
                  ("ReturnConsumedCapacity" Core..=) Core.<$>
...
instance Core.AWSRequest GetItem where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 GetItemResponse' Core.<$>
                   (x Core..:? "ConsumedCapacity") Core.<*> x Core..:? "Item" Core.<*>
...
mkGetItemResponse responseStatus
  = GetItemResponse'{consumedCapacity = Core.Nothing,
                     item = Core.Nothing, responseStatus}
...
{-# INLINEABLE girrsConsumedCapacity #-}
