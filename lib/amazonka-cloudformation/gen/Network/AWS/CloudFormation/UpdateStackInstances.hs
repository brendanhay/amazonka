...
mkUpdateStackInstances stackSetName
  = UpdateStackInstances'{stackSetName, regions = Core.mempty,
...
{-# INLINEABLE usiDeploymentTargets #-}
...
{-# INLINEABLE usiOperationPreferences #-}
...
{-# INLINEABLE usiParameterOverrides #-}
...
instance Core.ToQuery UpdateStackInstances where
        toQuery UpdateStackInstances{..}
          = Core.toQueryPair "Action" ("UpdateStackInstances" :: Core.Text)
...
instance Core.AWSRequest UpdateStackInstances where
        ...
        {-# INLINE toRequest #-}
        parseResponse
          = Response.receiveXMLWrapper "UpdateStackInstancesResult"
              (\ s h x ->
                 UpdateStackInstancesResponse' Core.<$>
...
mkUpdateStackInstancesResponse responseStatus
  = UpdateStackInstancesResponse'{operationId = Core.Nothing,
                                  responseStatus}
...
{-# INLINEABLE usirrsResponseStatus #-}
