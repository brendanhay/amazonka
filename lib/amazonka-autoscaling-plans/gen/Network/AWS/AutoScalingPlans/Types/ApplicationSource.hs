...
mkApplicationSource
    :: ApplicationSource
mkApplicationSource
  = ApplicationSource'{cloudFormationStackARN = Core.Nothing,
...
{-# INLINEABLE asCloudFormationStackARN #-}
...
instance Core.FromJSON ApplicationSource where
        toJSON ApplicationSource{..}
          = Core.object
              (Core.catMaybes
                 [("CloudFormationStackARN" Core..=) Core.<$>
          ...
          = Core.withObject "ApplicationSource" Core.$
              \ x ->
                ApplicationSource' Core.<$>
                  (x Core..:? "CloudFormationStackARN") Core.<*>
                    x Core..:? "TagFilters"
