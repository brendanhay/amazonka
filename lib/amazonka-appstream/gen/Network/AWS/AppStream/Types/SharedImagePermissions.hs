...
mkSharedImagePermissions
    :: Types.AwsAccountId -- ^ 'sharedAccountId'
...
{-# INLINEABLE sipImagePermissions #-}
...
instance Core.FromJSON SharedImagePermissions where
        parseJSON
          = Core.withObject "SharedImagePermissions" Core.$
              \ x ->
                SharedImagePermissions' Core.<$>
                  (x Core..: "sharedAccountId") Core.<*> x Core..: "imagePermissions"
