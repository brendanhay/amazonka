...
instance AWSPager DescribeReservedInstancesOfferings where
  page rq rs
    ...
    | stop (rs ^. driorsReservedInstancesOfferings) = Nothing
    | otherwise = Just $ rq & drioNextToken .~ rs ^. driorsNextToken
...
instance ToQuery DescribeReservedInstancesOfferings where
  toQuery DescribeReservedInstancesOfferings' {..} =
    mconcat
      [ "Action" =: ("DescribeReservedInstancesOfferings" :: ByteString),
...
instance NFData DescribeReservedInstancesOfferingsResponse
