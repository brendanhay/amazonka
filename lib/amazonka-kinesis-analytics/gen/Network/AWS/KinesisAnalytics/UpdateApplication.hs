...
instance AWSRequest UpdateApplication where
  ...
  response =
    receiveEmpty
      (\s h x -> UpdateApplicationResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders UpdateApplication where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("KinesisAnalytics_20150814.UpdateApplication" :: ByteString),
    ...
    object
      ( catMaybes
          [ Just ("ApplicationName" .= _uaApplicationName),
            Just
              ("CurrentApplicationVersionId" .= _uaCurrentApplicationVersionId),
