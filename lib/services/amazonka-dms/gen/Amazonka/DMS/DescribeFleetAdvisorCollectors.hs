...
instance
  ...
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "X-Amz-Target"
              Core.=# ( "AmazonDMSv20160101.DescribeFleetAdvisorCollectors" ::
                          Prelude.ByteString
                      ),
            "Content-Type"
...
instance Core.ToJSON DescribeFleetAdvisorCollectors where
...
instance Core.ToPath DescribeFleetAdvisorCollectors where
  toPath = Prelude.const "/"

instance Core.ToQuery DescribeFleetAdvisorCollectors where
