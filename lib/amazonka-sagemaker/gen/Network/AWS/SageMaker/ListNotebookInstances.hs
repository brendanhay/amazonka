...
instance AWSPager ListNotebookInstances where
  page rq rs
    ...
    | otherwise = Just $ rq & lniNextToken .~ rs ^. lnirsNextToken
      ...
      ( catMaybes
          [ ("NameContains" .=) <$> _lniNameContains,
            ("DefaultCodeRepositoryContains" .=)
              <$> _lniDefaultCodeRepositoryContains,
            ("LastModifiedTimeBefore" .=) <$> _lniLastModifiedTimeBefore,
            ...
            ("LastModifiedTimeAfter" .=) <$> _lniLastModifiedTimeAfter,
