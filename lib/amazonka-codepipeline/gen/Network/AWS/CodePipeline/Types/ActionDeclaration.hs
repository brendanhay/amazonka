...
mkActionDeclaration name actionTypeId
  = ActionDeclaration'{name, actionTypeId,
                       configuration = Core.Nothing, inputArtifacts = Core.Nothing,
                       namespace = Core.Nothing, outputArtifacts = Core.Nothing,
                       region = Core.Nothing, roleArn = Core.Nothing,
...
{-# INLINEABLE adRunOrder #-}
{-# DEPRECATED runOrder "Use generic-lens or generic-optics with 'runOrder' instead"  #-}
...
instance Core.FromJSON ActionDeclaration where
        parseJSON
          = Core.withObject "ActionDeclaration" Core.$
              \ x ->
                ActionDeclaration' Core.<$>
                  (x Core..: "name") Core.<*> x Core..: "actionTypeId" Core.<*>
