...
fromFilePath n f =
  ...
  where
    ...
    getRegionFromConfigFile = do
      ...
      p <- liftIO (Directory.doesFileExist configPath)
      unless p $ Exception.throwIO (MissingFileError f)
      ini <- either (throwInvalid Nothing) return =<< liftIO (INI.readIniFile configPath)
