...
mkListEntitiesForPolicy policyArn
  = ListEntitiesForPolicy'{policyArn, entityFilter = Core.Nothing,
                           marker = Core.Nothing, maxItems = Core.Nothing,
                           pathPrefix = Core.Nothing, policyUsageFilter = Core.Nothing}
...
{-# INLINEABLE lefpEntityFilter #-}
{-# DEPRECATED entityFilter "Use generic-lens or generic-optics with 'entityFilter' instead"  #-}
...
{-# INLINEABLE lefpPolicyUsageFilter #-}
...
instance Core.ToQuery ListEntitiesForPolicy where
        toQuery ListEntitiesForPolicy{..}
          = Core.toQueryPair "Action" ("ListEntitiesForPolicy" :: Core.Text)
              Core.<> Core.toQueryPair "Version" ("2010-05-08" :: Core.Text)
              Core.<> Core.toQueryPair "PolicyArn" policyArn
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "EntityFilter")
                entityFilter
...
instance Core.AWSRequest ListEntitiesForPolicy where
        ...
        {-# INLINE toRequest #-}
        parseResponse
          = Response.receiveXMLWrapper "ListEntitiesForPolicyResult"
              (\ s h x ->
                 ListEntitiesForPolicyResponse' Core.<$>
                   (x Core..@? "IsTruncated") Core.<*> x Core..@? "Marker" Core.<*>
                     x Core..@? "PolicyGroups" Core..<@> Core.parseXMLList "member"
                     Core.<*>
                     x Core..@? "PolicyRoles" Core..<@> Core.parseXMLList "member"
                     Core.<*>
                     x Core..@? "PolicyUsers" Core..<@> Core.parseXMLList "member"
...
mkListEntitiesForPolicyResponse responseStatus
  = ListEntitiesForPolicyResponse'{isTruncated = Core.Nothing,
                                   marker = Core.Nothing, policyGroups = Core.Nothing,
                                   policyRoles = Core.Nothing, policyUsers = Core.Nothing,
                                   responseStatus}
...
{-# INLINEABLE lefprrsPolicyGroups #-}
{-# DEPRECATED policyGroups "Use generic-lens or generic-optics with 'policyGroups' instead"  #-}
...
{-# INLINEABLE lefprrsPolicyRoles #-}
{-# DEPRECATED policyRoles "Use generic-lens or generic-optics with 'policyRoles' instead"  #-}
...
{-# INLINEABLE lefprrsPolicyUsers #-}
{-# DEPRECATED policyUsers "Use generic-lens or generic-optics with 'policyUsers' instead"  #-}
...
{-# INLINEABLE lefprrsResponseStatus #-}
