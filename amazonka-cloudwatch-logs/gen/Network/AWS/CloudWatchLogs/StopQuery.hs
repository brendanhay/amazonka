...
--
-- * 'sqQueryId' - The ID number of the query to stop. If necessary, you can use @DescribeQueries@ to find this ID number.
stopQuery
    :: Text -- ^ 'sqQueryId'
    -> StopQuery
...
instance Hashable StopQuery where

instance NFData StopQuery where
...
stopQueryResponse
    :: Int -- ^ 'sqrsResponseStatus'
    -> StopQueryResponse
stopQueryResponse pResponseStatus_ =
  StopQueryResponse'
    {_sqrsSuccess = Nothing, _sqrsResponseStatus = pResponseStatus_}
...
instance NFData StopQueryResponse where
