...
{-# INLINEABLE caCacheStaleTimeoutInSeconds #-}
{-# DEPRECATED cacheStaleTimeoutInSeconds "Use generic-lens or generic-optics with 'cacheStaleTimeoutInSeconds' instead"  #-}

instance Core.FromJSON CacheAttributes where
        toJSON CacheAttributes{..}
          = Core.object
              (Core.catMaybes
                 [("CacheStaleTimeoutInSeconds" Core..=) Core.<$>
