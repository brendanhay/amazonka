...
mkGetMetricData instanceId startTime endTime filters
  = GetMetricData'{instanceId, startTime, endTime, filters,
                   historicalMetrics = Core.mempty, groupings = Core.Nothing,
...
{-# INLINEABLE gmdHistoricalMetrics #-}
{-# DEPRECATED historicalMetrics "Use generic-lens or generic-optics with 'historicalMetrics' instead"  #-}
          ...
          = Response.receiveJSON
              (\ s h x ->
                 GetMetricDataResponse' Core.<$>
                   (x Core..:? "MetricResults") Core.<*> x Core..:? "NextToken"
...
mkGetMetricDataResponse responseStatus
  = GetMetricDataResponse'{metricResults = Core.Nothing,
...
{-# INLINEABLE gmdrrsMetricResults #-}
