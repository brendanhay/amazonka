...
--
-- * 'dcvcFilters' - One or more filters. Filter names and values are case-sensitive.
--
...
instance AWSPager DescribeClientVPNConnections where
        page rq rs
          ...
          | otherwise =
            Just $ rq & dcvcNextToken .~ rs ^. dcvcrsNextToken

instance AWSRequest DescribeClientVPNConnections
         where
...
instance Hashable DescribeClientVPNConnections where

instance NFData DescribeClientVPNConnections where
