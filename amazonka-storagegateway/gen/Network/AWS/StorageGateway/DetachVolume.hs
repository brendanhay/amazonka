...
--
-- * 'dvForceDetach' - Set to @true@ to forcibly remove the iSCSI connection of the target volume and detach the volume. The default is @false@ . If this value is set to @false@ , you must manually disconnect the iSCSI connection from the target volume.
--
...
detachVolume pVolumeARN_ =
  DetachVolume' {_dvForceDetach = Nothing, _dvVolumeARN = pVolumeARN_}
...
instance NFData DetachVolumeResponse where
