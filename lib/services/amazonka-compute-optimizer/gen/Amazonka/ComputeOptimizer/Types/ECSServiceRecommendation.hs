...
data ECSServiceRecommendation = ECSServiceRecommendation'
  { -- | The Amazon Web Services account ID of the Amazon ECS service.
    ...
    --     not enough memory or CPU, an Amazon ECS service is considered
    ...
    --     excessive memory or CPU, an Amazon ECS service is considered
    --     over-provisioned. An over-provisioned service might result in
    --     additional infrastructure costs.
    --
    -- -   __@Optimized@__ — When both the CPU and memory of your Amazon ECS
    --     service meet the performance requirements of your workload, the
    ...
    --
    -- -   __@CPUUnderprovisioned@__ — The service CPU configuration can be
    ...
    --
    -- -   __@CPUOverprovisioned@__ — The service CPU configuration can be
    ...
    --
    -- -   __@MemoryUnderprovisioned@__ — The service memory configuration can
    ...
    --
    -- -   __@MemoryOverprovisioned@__ — The service memory configuration can
    --     be sized down while still meeting the performance requirements of
...
--
-- 'accountId', 'eCSServiceRecommendation_accountId' - The Amazon Web Services account ID of the Amazon ECS service.
--
-- 'currentPerformanceRisk', 'eCSServiceRecommendation_currentPerformanceRisk' - The risk of the current Amazon ECS service not meeting the performance
-- needs of its workloads. The higher the risk, the more likely the current
-- service can\'t meet the performance requirements of its workload.
--
-- 'currentServiceConfiguration', 'eCSServiceRecommendation_currentServiceConfiguration' - The configuration of the current Amazon ECS service.
--
-- 'finding', 'eCSServiceRecommendation_finding' - The finding classification of an Amazon ECS service.
--
-- Findings for Amazon ECS services include:
--
-- -   __@Underprovisioned@__ — When Compute Optimizer detects that there’s
...
--     service is considered optimized.
--
-- 'findingReasonCodes', 'eCSServiceRecommendation_findingReasonCodes' - The reason for the finding classification of an Amazon ECS service.
--
-- Finding reason codes for Amazon ECS services include:
--
-- -   __@CPUUnderprovisioned@__ — The service CPU configuration can be
...
--     look-back period.
--
-- 'lastRefreshTimestamp', 'eCSServiceRecommendation_lastRefreshTimestamp' - The timestamp of when the Amazon ECS service recommendation was last
-- generated.
--
-- 'launchType', 'eCSServiceRecommendation_launchType' - The launch type the Amazon ECS service is using.
--
-- Compute Optimizer only supports the Fargate launch type.
--
-- 'lookbackPeriodInDays', 'eCSServiceRecommendation_lookbackPeriodInDays' - The number of days the Amazon ECS service utilization metrics were
-- analyzed.
--
-- 'serviceArn', 'eCSServiceRecommendation_serviceArn' - The Amazon Resource Name (ARN) of the current Amazon ECS service.
--
-- The following is the format of the ARN:
--
-- @arn:aws:ecs:region:aws_account_id:service\/cluster-name\/service-name@
--
-- 'serviceRecommendationOptions', 'eCSServiceRecommendation_serviceRecommendationOptions' - An array of objects that describe the recommendation options for the
-- Amazon ECS service.
--
-- 'tags', 'eCSServiceRecommendation_tags' - A list of tags assigned to your Amazon ECS service recommendations.
--
-- 'utilizationMetrics', 'eCSServiceRecommendation_utilizationMetrics' - An array of objects that describe the utilization metrics of the Amazon
-- ECS service.
...
-- | A list of tags assigned to your Amazon ECS service recommendations.
eCSServiceRecommendation_tags :: Lens.Lens' ECSServiceRecommendation (Prelude.Maybe [Tag])
eCSServiceRecommendation_tags = Lens.lens (\ECSServiceRecommendation' {tags} -> tags) (\s@ECSServiceRecommendation' {} a -> s {tags = a} :: ECSServiceRecommendation) Prelude.. Lens.mapping Lens.coerced
