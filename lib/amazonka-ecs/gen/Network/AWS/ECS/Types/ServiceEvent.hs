...
mkServiceEvent
    :: ServiceEvent
mkServiceEvent
  = ServiceEvent'{createdAt = Core.Nothing, id = Core.Nothing,
...
seId :: Lens.Lens' ServiceEvent (Core.Maybe Core.Text)
seId = Lens.field @"id"
{-# INLINEABLE seId #-}
...
seMessage :: Lens.Lens' ServiceEvent (Core.Maybe Core.Text)
...
instance Core.FromJSON ServiceEvent where
        parseJSON
          = Core.withObject "ServiceEvent" Core.$
              \ x ->
                ServiceEvent' Core.<$>
                  (x Core..:? "createdAt") Core.<*> x Core..:? "id" Core.<*>
                    x Core..:? "message"
