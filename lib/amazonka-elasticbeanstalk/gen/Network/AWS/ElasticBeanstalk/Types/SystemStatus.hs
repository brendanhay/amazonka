...
mkSystemStatus
    :: SystemStatus
mkSystemStatus
  = SystemStatus'{cPUUtilization = Core.Nothing,
...
{-# INLINEABLE ssCPUUtilization #-}
{-# DEPRECATED cPUUtilization "Use generic-lens or generic-optics with 'cPUUtilization' instead"  #-}
...
{-# INLINEABLE ssLoadAverage #-}
{-# DEPRECATED loadAverage "Use generic-lens or generic-optics with 'loadAverage' instead"  #-}

instance Core.FromXML SystemStatus where
        parseXML x
          = SystemStatus' Core.<$>
              (x Core..@? "CPUUtilization") Core.<*>
                x Core..@? "LoadAverage" Core..<@> Core.parseXMLList "member"
