...
mkQuery
    ...
    -> Query
mkQuery tableName
  = Query'{tableName, attributesToGet = Core.Nothing,
           conditionalOperator = Core.Nothing, consistentRead = Core.Nothing,
           exclusiveStartKey = Core.Nothing,
           expressionAttributeNames = Core.Nothing,
           expressionAttributeValues = Core.Nothing,
           filterExpression = Core.Nothing, indexName = Core.Nothing,
           keyConditionExpression = Core.Nothing,
           keyConditions = Core.Nothing, limit = Core.Nothing,
           projectionExpression = Core.Nothing, queryFilter = Core.Nothing,
           returnConsumedCapacity = Core.Nothing,
           scanIndexForward = Core.Nothing, select = Core.Nothing}
...
{-# INLINEABLE qConditionalOperator #-}
...
{-# INLINEABLE qExclusiveStartKey #-}
{-# DEPRECATED exclusiveStartKey "Use generic-lens or generic-optics with 'exclusiveStartKey' instead"  #-}
...
{-# INLINEABLE qExpressionAttributeNames #-}
...
{-# INLINEABLE qExpressionAttributeValues #-}
...
{-# INLINEABLE qKeyConditionExpression #-}
{-# DEPRECATED keyConditionExpression "Use generic-lens or generic-optics with 'keyConditionExpression' instead"  #-}
...
{-# INLINEABLE qKeyConditions #-}
{-# DEPRECATED keyConditions "Use generic-lens or generic-optics with 'keyConditions' instead"  #-}
...
{-# INLINEABLE qProjectionExpression #-}
...
{-# INLINEABLE qQueryFilter #-}
{-# DEPRECATED queryFilter "Use generic-lens or generic-optics with 'queryFilter' instead"  #-}
...
{-# INLINEABLE qReturnConsumedCapacity #-}
...
{-# INLINEABLE qScanIndexForward #-}
{-# DEPRECATED scanIndexForward "Use generic-lens or generic-optics with 'scanIndexForward' instead"  #-}
...
{-# INLINEABLE qSelect #-}
{-# DEPRECATED select "Use generic-lens or generic-optics with 'select' instead"  #-}
...
instance Core.ToHeaders Query where
        toHeaders Query{..}
          = Core.pure ("X-Amz-Target", "DynamoDB_20120810.Query") Core.<>
...
instance Core.AWSRequest Query where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 QueryResponse' Core.<$>
                   (x Core..:? "ConsumedCapacity") Core.<*> x Core..:? "Count"
                     Core.<*> x Core..:? "Items"
...
instance Pager.AWSPager Query where
        page rq rs
          ...
          | Core.otherwise =
            Core.Just
              (rq Core.&
                 Lens.field @"exclusiveStartKey" Lens..~
                   rs Lens.^. Lens.field @"lastEvaluatedKey")
...
mkQueryResponse responseStatus
  = QueryResponse'{consumedCapacity = Core.Nothing,
                   count = Core.Nothing, items = Core.Nothing,
                   lastEvaluatedKey = Core.Nothing, scannedCount = Core.Nothing,
                   responseStatus}
...
{-# INLINEABLE qrrsConsumedCapacity #-}
...
{-# INLINEABLE qrrsLastEvaluatedKey #-}
{-# DEPRECATED lastEvaluatedKey "Use generic-lens or generic-optics with 'lastEvaluatedKey' instead"  #-}
...
{-# INLINEABLE qrrsScannedCount #-}
{-# DEPRECATED scannedCount "Use generic-lens or generic-optics with 'scannedCount' instead"  #-}
