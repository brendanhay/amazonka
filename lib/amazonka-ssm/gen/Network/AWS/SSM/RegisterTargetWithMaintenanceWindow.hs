...
mkRegisterTargetWithMaintenanceWindow
    ...
    -> Types.MaintenanceWindowResourceType -- ^ 'resourceType'
    -> RegisterTargetWithMaintenanceWindow
mkRegisterTargetWithMaintenanceWindow windowId resourceType
  = RegisterTargetWithMaintenanceWindow'{windowId, resourceType,
                                         targets = Core.mempty, clientToken = Core.Nothing,
...
{-# INLINEABLE rOwnerInformation #-}
{-# DEPRECATED ownerInformation "Use generic-lens or generic-optics with 'ownerInformation' instead"  #-}

instance Core.ToQuery RegisterTargetWithMaintenanceWindow where
        toQuery _ = Core.pure Core.mempty

instance Core.ToHeaders RegisterTargetWithMaintenanceWindow where
...
instance Core.AWSRequest RegisterTargetWithMaintenanceWindow where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 RegisterTargetWithMaintenanceWindowResponse' Core.<$>
                   (x Core..:? "WindowTargetId") Core.<*> Core.pure (Core.fromEnum s))
...
mkRegisterTargetWithMaintenanceWindowResponse responseStatus
  = RegisterTargetWithMaintenanceWindowResponse'{windowTargetId =
                                                   Core.Nothing,
                                                 responseStatus}
