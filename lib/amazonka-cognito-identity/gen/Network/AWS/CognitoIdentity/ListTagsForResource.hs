...
--
-- * 'ltfrResourceARN' - The Amazon Resource Name (ARN) of the identity pool that the tags are assigned to.
listTagsForResource ::
  -- | 'ltfrResourceARN'
  Text ->
  ListTagsForResource
...
instance ToHeaders ListTagsForResource where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AWSCognitoIdentityService.ListTagsForResource" :: ByteString),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ltfrrsTags' - The tags that are assigned to the identity pool.
--
