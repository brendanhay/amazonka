...
{-# INLINEABLE chcHealthCheckConfig #-}
...
instance Core.ToXML CreateHealthCheck where
        toXML CreateHealthCheck{..}
          = Core.toXMLElement "CallerReference" callerReference Core.<>
              Core.toXMLElement "HealthCheckConfig" healthCheckConfig
        toXMLDocument
          = Core.newXMLDocument
...
instance Core.AWSRequest CreateHealthCheck where
        ...
        parseResponse
          = Response.receiveXML
              (\ s h x ->
                 CreateHealthCheckResponse' Core.<$>
                   (x Core..@ "HealthCheck") Core.<*> Core.parseHeader "Location" h
