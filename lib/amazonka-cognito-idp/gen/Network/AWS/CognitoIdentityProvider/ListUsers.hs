...
mkListUsers userPoolId
  = ListUsers'{userPoolId, attributesToGet = Core.Nothing,
               filter = Core.Nothing, limit = Core.Nothing,
          ...
          = Response.receiveJSON
              (\ s h x ->
                 ListUsersResponse' Core.<$>
                   (x Core..:? "PaginationToken") Core.<*> x Core..:? "Users" Core.<*>
...
mkListUsersResponse responseStatus
  = ListUsersResponse'{paginationToken = Core.Nothing,
...
{-# INLINEABLE lurrsPaginationToken #-}
{-# DEPRECATED paginationToken "Use generic-lens or generic-optics with 'paginationToken' instead"  #-}
