...
mkLogSettingsResponse
    :: LogSettingsResponse
mkLogSettingsResponse
  = LogSettingsResponse'{destination = Core.Nothing,
                         kmsKeyArn = Core.Nothing, logType = Core.Nothing,
                         resourceArn = Core.Nothing, resourcePrefix = Core.Nothing}
...
{-# INLINEABLE lsrResourcePrefix #-}
{-# DEPRECATED resourcePrefix "Use generic-lens or generic-optics with 'resourcePrefix' instead"  #-}

instance Core.FromJSON LogSettingsResponse where
        parseJSON
          = Core.withObject "LogSettingsResponse" Core.$
              \ x ->
                LogSettingsResponse' Core.<$>
                  (x Core..:? "destination") Core.<*> x Core..:? "kmsKeyArn" Core.<*>
                    x Core..:? "logType"
