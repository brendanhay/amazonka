...
{-# INLINEABLE gsamlpSAMLProviderArn #-}
...
instance Core.ToQuery GetSAMLProvider where
        toQuery GetSAMLProvider{..}
          = Core.toQueryPair "Action" ("GetSAMLProvider" :: Core.Text)
...
instance Core.AWSRequest GetSAMLProvider where
        ...
        {-# INLINE toRequest #-}
        parseResponse
          = Response.receiveXMLWrapper "GetSAMLProviderResult"
              (\ s h x ->
                 GetSAMLProviderResponse' Core.<$>
                   (x Core..@? "CreateDate") Core.<*>
                     x Core..@? "SAMLMetadataDocument"
...
mkGetSAMLProviderResponse responseStatus
  = GetSAMLProviderResponse'{createDate = Core.Nothing,
                             sAMLMetadataDocument = Core.Nothing, validUntil = Core.Nothing,
                             responseStatus}
...
{-# INLINEABLE gsamlprrsCreateDate #-}
...
{-# INLINEABLE gsamlprrsSAMLMetadataDocument #-}
...
{-# INLINEABLE gsamlprrsValidUntil #-}
...
{-# INLINEABLE gsamlprrsResponseStatus #-}
