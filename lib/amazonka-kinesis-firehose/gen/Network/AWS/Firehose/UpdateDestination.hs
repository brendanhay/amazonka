...
instance AWSRequest UpdateDestination where
  ...
  response =
    receiveEmpty
      (\s h x -> UpdateDestinationResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders UpdateDestination where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("Firehose_20150804.UpdateDestination" :: ByteString),
...
instance ToJSON UpdateDestination where
  toJSON UpdateDestination' {..} =
    object
      ( catMaybes
          [ ("SplunkDestinationUpdate" .=) <$> _udSplunkDestinationUpdate,
            ("S3DestinationUpdate" .=) <$> _udS3DestinationUpdate,
            ("RedshiftDestinationUpdate" .=) <$> _udRedshiftDestinationUpdate,
