...
mkCognitoMemberDefinition
    :: Types.UserPool -- ^ 'userPool'
    -> Types.UserGroup -- ^ 'userGroup'
...
{-# INLINEABLE cmdUserGroup #-}
{-# DEPRECATED userGroup "Use generic-lens or generic-optics with 'userGroup' instead"  #-}
          ...
          = Core.withObject "CognitoMemberDefinition" Core.$
              \ x ->
                CognitoMemberDefinition' Core.<$>
                  (x Core..: "UserPool") Core.<*> x Core..: "UserGroup" Core.<*>
                    x Core..: "ClientId"
