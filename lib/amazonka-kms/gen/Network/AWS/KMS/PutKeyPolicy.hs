...
mkPutKeyPolicy
    ...
    -> Types.PolicyType -- ^ 'policy'
    -> PutKeyPolicy
mkPutKeyPolicy keyId policyName policy
  = PutKeyPolicy'{keyId, policyName, policy,
...
{-# INLINEABLE pkpBypassPolicyLockoutSafetyCheck #-}
...
instance Core.ToHeaders PutKeyPolicy where
        toHeaders PutKeyPolicy{..}
          = Core.pure ("X-Amz-Target", "TrentService.PutKeyPolicy") Core.<>
...
instance Core.AWSRequest PutKeyPolicy where
        ...
        {-# INLINE toRequest #-}
        parseResponse = Response.receiveNull PutKeyPolicyResponse'
