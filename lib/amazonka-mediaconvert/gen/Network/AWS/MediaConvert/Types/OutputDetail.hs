...
mkOutputDetail
    :: OutputDetail
mkOutputDetail
  = OutputDetail'{durationInMs = Core.Nothing,
...
{-# INLINEABLE odDurationInMs #-}
{-# DEPRECATED durationInMs "Use generic-lens or generic-optics with 'durationInMs' instead"  #-}
...
{-# INLINEABLE odVideoDetails #-}
{-# DEPRECATED videoDetails "Use generic-lens or generic-optics with 'videoDetails' instead"  #-}
...
instance Core.FromJSON OutputDetail where
        parseJSON
          = Core.withObject "OutputDetail" Core.$
              \ x ->
                OutputDetail' Core.<$>
                  (x Core..:? "durationInMs") Core.<*> x Core..:? "videoDetails"
