...
mkWorkflowExecutionTerminatedEventAttributes childPolicy
  = WorkflowExecutionTerminatedEventAttributes'{childPolicy,
                                                cause = Core.Nothing, details = Core.Nothing,
                                                reason = Core.Nothing}
...
{-# INLINEABLE weteaReason #-}
{-# DEPRECATED reason "Use generic-lens or generic-optics with 'reason' instead"  #-}

instance Core.FromJSON WorkflowExecutionTerminatedEventAttributes
         where
        parseJSON
          = Core.withObject "WorkflowExecutionTerminatedEventAttributes"
              Core.$
              \ x ->
                WorkflowExecutionTerminatedEventAttributes' Core.<$>
                  (x Core..: "childPolicy") Core.<*> x Core..:? "cause" Core.<*>
                    x Core..:? "details"
                    Core.<*> x Core..:? "reason"
