...
instance AWSPager DescribeInstancePatchStatesForPatchGroup where
  page rq rs
    | stop (rs ^. dipsfpgrsNextToken) = Nothing
    | stop (rs ^. dipsfpgrsInstancePatchStates) = Nothing
    | otherwise =
      Just $ rq & dipsfpgNextToken .~ rs ^. dipsfpgrsNextToken

instance AWSRequest DescribeInstancePatchStatesForPatchGroup where
  type
...
instance Hashable DescribeInstancePatchStatesForPatchGroup

instance NFData DescribeInstancePatchStatesForPatchGroup

instance ToHeaders DescribeInstancePatchStatesForPatchGroup where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
...
instance ToJSON DescribeInstancePatchStatesForPatchGroup where
...
instance ToPath DescribeInstancePatchStatesForPatchGroup where
  toPath = const "/"

instance ToQuery DescribeInstancePatchStatesForPatchGroup where
...
data DescribeInstancePatchStatesForPatchGroupResponse = DescribeInstancePatchStatesForPatchGroupResponse'
  { _dipsfpgrsNextToken ::
    ...
    _dipsfpgrsInstancePatchStates ::
      !( Maybe
           ( List1
               InstancePatchState
           )
       ),
    _dipsfpgrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
instance NFData DescribeInstancePatchStatesForPatchGroupResponse
