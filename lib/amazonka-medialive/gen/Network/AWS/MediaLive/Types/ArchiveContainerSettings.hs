...
mkArchiveContainerSettings
    :: ArchiveContainerSettings
mkArchiveContainerSettings
  = ArchiveContainerSettings'{m2tsSettings = Core.Nothing,
...
{-# INLINEABLE acsRawSettings #-}
{-# DEPRECATED rawSettings "Use generic-lens or generic-optics with 'rawSettings' instead"  #-}
...
instance Core.FromJSON ArchiveContainerSettings where
        parseJSON
          = Core.withObject "ArchiveContainerSettings" Core.$
              \ x ->
                ArchiveContainerSettings' Core.<$>
                  (x Core..:? "m2tsSettings") Core.<*> x Core..:? "rawSettings"
