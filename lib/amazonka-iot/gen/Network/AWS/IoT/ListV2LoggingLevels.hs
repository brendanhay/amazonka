...
mkListV2LoggingLevels
    :: ListV2LoggingLevels
mkListV2LoggingLevels
  = ListV2LoggingLevels'{maxResults = Core.Nothing,
                         nextToken = Core.Nothing, targetType = Core.Nothing}
...
instance Core.AWSRequest ListV2LoggingLevels where
        ...
        toRequest x@Core.Request{..}
          = Core.Request{Core._rqService = Types.mkServiceConfig,
                         Core._rqMethod = Request.GET, Core._rqPath = "/v2LoggingLevel",
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 ListV2LoggingLevelsResponse' Core.<$>
                   (x Core..:? "logTargetConfigurations") Core.<*>
                     x Core..:? "nextToken"
...
instance Pager.AWSPager ListV2LoggingLevels where
        page rq rs
          ...
          | Pager.stop
              (rs Lens.^?
                 Lens.field @"logTargetConfigurations" Core.. Lens._Just)
            = Core.Nothing
          | Core.otherwise =
            Core.Just
              (rq Core.&
...
{-# INLINEABLE lvllrrsLogTargetConfigurations #-}
{-# DEPRECATED logTargetConfigurations "Use generic-lens or generic-optics with 'logTargetConfigurations' instead"  #-}
...
{-# INLINEABLE lvllrrsResponseStatus #-}
