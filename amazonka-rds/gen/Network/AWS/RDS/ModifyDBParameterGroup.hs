...
-- modified in a single request. After you modify a DB parameter group, you
-- should wait at least 5 minutes before creating your first DB instance that
-- uses that DB parameter group as the default parameter group. This allows
-- Amazon RDS to fully complete the modify action before the parameter group
-- is used as the default for a new DB instance. This is especially important
-- for parameters that are critical when creating the default database for a
-- DB instance, such as the character set for the default database defined by
