...
-- <http://docs.aws.amazon.com/kinesis/latest/dev/kinesis-using-api-java.html#kinesis-using-api-java-resharding-merge
-- Merge Two Shards> in the /Amazon Kinesis Developer Guide/. If the stream is
-- in the ACTIVE state, you can call MergeShards. If a stream is in the
...
-- returns a ResourceNotFoundException. You can use DescribeStream> to check
-- the state of the stream, which is returned in StreamStatus. MergeShards is
-- an asynchronous operation. Upon receiving a MergeShards request, Amazon
