...
mkAllowedMethods quantity
  = AllowedMethods'{quantity, items = Core.mempty,
...
{-# INLINEABLE amCachedMethods #-}
{-# DEPRECATED cachedMethods "Use generic-lens or generic-optics with 'cachedMethods' instead"  #-}

instance Core.ToXML AllowedMethods where
        toXML AllowedMethods{..}
          = Core.toXMLElement "Quantity" quantity Core.<>
              Core.toXMLElement "Items" (Core.toXMLList "Method" items)
              Core.<>
              Core.maybe Core.mempty (Core.toXMLElement "CachedMethods")
                cachedMethods
