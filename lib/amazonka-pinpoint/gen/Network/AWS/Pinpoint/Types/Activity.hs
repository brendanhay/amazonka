...
mkActivity
    :: Activity
mkActivity
  = Activity'{custom = Core.Nothing, conditionalSplit = Core.Nothing,
              description = Core.Nothing, email = Core.Nothing,
              holdout = Core.Nothing, multiCondition = Core.Nothing,
              push = Core.Nothing, randomSplit = Core.Nothing,
              sms = Core.Nothing, wait = Core.Nothing}
...
{-# INLINEABLE aCUSTOM #-}
{-# DEPRECATED custom "Use generic-lens or generic-optics with 'custom' instead"  #-}
...
{-# INLINEABLE aConditionalSplit #-}
{-# DEPRECATED conditionalSplit "Use generic-lens or generic-optics with 'conditionalSplit' instead"  #-}
...
{-# INLINEABLE aHoldout #-}
{-# DEPRECATED holdout "Use generic-lens or generic-optics with 'holdout' instead"  #-}
...
{-# INLINEABLE aMultiCondition #-}
{-# DEPRECATED multiCondition "Use generic-lens or generic-optics with 'multiCondition' instead"  #-}
...
{-# INLINEABLE aPUSH #-}
{-# DEPRECATED push "Use generic-lens or generic-optics with 'push' instead"  #-}
...
{-# INLINEABLE aRandomSplit #-}
{-# DEPRECATED randomSplit "Use generic-lens or generic-optics with 'randomSplit' instead"  #-}
...
aSMS = Lens.field @"sms"
{-# INLINEABLE aSMS #-}
{-# DEPRECATED sms "Use generic-lens or generic-optics with 'sms' instead"  #-}
...
{-# INLINEABLE aWait #-}
{-# DEPRECATED wait "Use generic-lens or generic-optics with 'wait' instead"  #-}
          ...
          = Core.object
              (Core.catMaybes
                 [("CUSTOM" Core..=) Core.<$> custom,
                  ...
                  ("SMS" Core..=) Core.<$> sms, ("Wait" Core..=) Core.<$> wait])
...
instance Core.FromJSON Activity where
        parseJSON
          = Core.withObject "Activity" Core.$
              \ x ->
                Activity' Core.<$>
                  (x Core..:? "CUSTOM") Core.<*> x Core..:? "ConditionalSplit"
