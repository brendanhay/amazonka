...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gcirsInstanceId' - The ID of the managed instance targeted by the command. A managed instance can be an EC2 instance or an instance in your hybrid environment that is configured for Systems Manager.
--
-- * 'gcirsStatus' - The status of this invocation plugin. This status can be different than StatusDetails.
--
-- * 'gcirsStandardErrorContent' - The first 8,000 characters written by the plugin to stderr. If the command has not finished running, then this string is empty.
--
...
--
-- * 'gcirsDocumentName' - The name of the document that was run. For example, AWS-RunShellScript.
--
-- * 'gcirsStandardErrorURL' - The URL for the complete text written by the plugin to stderr. If the command has not finished running, then this string is empty.
--
...
--
-- * 'gcirsStandardOutputURL' - The URL for the complete text written by the plugin to stdout in Amazon S3. If an S3 bucket was not specified, then this string is empty.
--
...
--
-- * 'gcirsStandardOutputContent' - The first 24,000 characters written by the plugin to stdout. If the command has not finished running, if ExecutionStatus is neither Succeeded nor Failed, then this string is empty.
--
