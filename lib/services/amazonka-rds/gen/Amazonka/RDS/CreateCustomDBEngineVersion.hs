...
data CreateCustomDBEngineVersion = CreateCustomDBEngineVersion'
  { -- | The name of an Amazon S3 bucket that contains database installation
    ...
    -- by calling the
    -- <https://docs.aws.amazon.com/AmazonRDS/latest/APIReference/API_DescribeDBEngineVersions.html DescribeDBEngineVersions>
    -- operation.
...
--
-- 'imageId', 'createCustomDBEngineVersion_imageId' - The ID of the Amazon Machine Image (AMI). For RDS Custom for SQL Server,
-- an AMI ID is required to create a CEV. For RDS Custom for Oracle, the
-- default is the most recent AMI available, but you can specify an AMI ID
-- that was used in a different Oracle CEV. Find the AMIs used by your CEVs
-- by calling the
