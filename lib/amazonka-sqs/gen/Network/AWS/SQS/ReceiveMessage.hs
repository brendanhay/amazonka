...
data ReceiveMessage = ReceiveMessage'
  { -- | The URL of the Amazon SQS queue from which messages are received.
    ...
    receiveRequestAttemptId :: Core.Maybe Types.String,
...
rmQueueUrl :: Lens.Lens' ReceiveMessage Types.String
...
rmReceiveRequestAttemptId :: Lens.Lens' ReceiveMessage (Core.Maybe Types.String)
  ...
  request x@Core.Request {..} =
    Core.Request
      { Core._rqService = Types.mkServiceConfig,
        ...
        Core._rqBody =
          Core.toFormBody
            ( Core.pure ("Action", "ReceiveMessage")
                ...
                Core.<> ( Core.toQueryValue "MaxNumberOfMessages"
                            Core.<$> maxNumberOfMessages
                        )
                Core.<> ( Core.toQueryList "MessageAttributeName"
                            Core.<$> messageAttributeNames
                        )
                Core.<> ( Core.toQueryValue "ReceiveRequestAttemptId"
                            Core.<$> receiveRequestAttemptId
                        )
                Core.<> (Core.toQueryValue "VisibilityTimeout" Core.<$> visibilityTimeout)
                Core.<> (Core.toQueryValue "WaitTimeSeconds" Core.<$> waitTimeSeconds)
            )
      }
  response =
...
{-# DEPRECATED rmrrsMessages "Use generic-lens or generic-optics with 'messages' instead." #-}
...
{-# DEPRECATED rmrrsResponseStatus "Use generic-lens or generic-optics with 'responseStatus' instead." #-}
