...
-- volume (for example, @/dev/sdh@). @block-device-mapping.status@ - The
-- status for the Amazon EBS volume (@attaching@ | @attached@ | @detaching@
-- | @detached@). @block-device-mapping.volume-id@ - The volume ID of the
-- Amazon EBS volume. @client-token@ - The idempotency token you provided
-- when you launched the instance. @dns-name@ - The public DNS name of the
-- instance. @group-id@ - The ID of the security group for the instance. If
-- the instance is in EC2-Classic or a default VPC, you can use @group-name@
-- instead. @group-name@ - The name of the security group for the instance.
...
-- launch the instance. @instance-id@ - The ID of the instance.
...
-- (terminated), 64 (stopping), and 80 (stopped). @instance-state-name@ -
-- The state of the instance (@pending@ | @running@ | @shutting-down@ |
-- @terminated@ | @stopping@ | @stopped@). @instance-type@ - The type of
-- instance (for example, @m1.small@). @instance.group-id@ - The ID of the
-- security group for the instance. If the instance is in EC2-Classic or a
-- default VPC, you can use @instance.group-name@ instead.
-- @instance.group-name@ - The name of the security group for the instance.
-- If the instance is in a nondefault VPC, you must use @instance.group-id@
-- instead. @ip-address@ - The public IP address of the instance.
-- @kernel-id@ - The kernel ID. @key-name@ - The name of the key pair used
-- when the instance was launched. @launch-index@ - When launching multiple
-- instances, this is the index for the instance in the launch group (for
-- example, 0, 1, 2, and so on). @launch-time@ - The time when the instance
-- was launched. @monitoring-state@ - Indicates whether monitoring is
-- enabled for the instance (@disabled@ | @enabled@). @owner-id@ - The AWS
-- account ID of the instance owner. @placement-group-name@ - The name of
-- the placement group for the instance. @platform@ - The platform. Use
...
-- the instance. @product-code.type@ - The type of product code (@devpay@ |
-- @marketplace@). @ramdisk-id@ - The RAM disk ID. @reason@ - The reason for
-- the current state of the instance (for example, shows "User Initiated
-- [date]" when you stop or terminate the instance). Similar to the
-- state-reason-code filter. @requester-id@ - The ID of the entity that
-- launched the instance on your behalf (for example, AWS Management
-- Console, Auto Scaling, and so on). @reservation-id@ - The ID of the
-- instance's reservation. A reservation ID is created any time you launch
-- an instance. A reservation ID has a one-to-one relationship with an
...
-- get one reservation ID. @root-device-name@ - The name of the root device
-- for the instance (for example, @/dev/sda1@). @root-device-type@ - The
...
-- your VPC. @spot-instance-request-id@ - The ID of the Spot Instance
-- request. @state-reason-code@ - The reason code for the state change.
...
-- instance is running in. @network-interface.description@ - The description
-- of the network interface. @network-interface.subnet-id@ - The ID of the
-- subnet for the network interface. @network-interface.vpc-id@ - The ID of
-- the VPC for the network interface.
-- @network-interface.network-interface.id@ - The ID of the network
-- interface. @network-interface.owner-id@ - The ID of the owner of the
-- network interface. @network-interface.availability-zone@ - The
...
-- network interface is being managed by AWS. @network-interface.status@ -
-- The status of the network interface (@available@) | @in-use@).
-- @network-interface.mac-address@ - The MAC address of the network
-- interface. @network-interface-private-dns-name@ - The private DNS name of
-- the network interface. @network-interface.source-destination-check@ -
-- Whether the network interface performs source/destination checking. A
-- value of @true@ means checking is enabled, and @false@ means checking is
...
-- the network interface. @network-interface.group-name@ - The name of a
...
-- - The status of the attachment (@attaching@ | @attached@ | @detaching@ |
-- @detached@). @network-interface.attachment.attach-time@ - The time that
...
-- the network interface. @association.ip-owner-id@ - The owner of the
