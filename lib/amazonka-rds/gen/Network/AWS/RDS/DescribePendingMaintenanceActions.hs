...
instance AWSPager DescribePendingMaintenanceActions where
  page rq rs
    ...
    | stop (rs ^. dpmarsPendingMaintenanceActions) = Nothing
    | otherwise = Just $ rq & dpmaMarker .~ rs ^. dpmarsMarker
      ...
      ( \s h x ->
          DescribePendingMaintenanceActionsResponse'
            <$> ( x .@? "PendingMaintenanceActions" .!@ mempty
                    >>= may (parseXMLList "ResourcePendingMaintenanceActions")
                )
            <*> (x .@? "Marker")
            <*> (pure (fromEnum s))
      )
...
instance ToQuery DescribePendingMaintenanceActions where
  toQuery DescribePendingMaintenanceActions' {..} =
    mconcat
      [ "Action" =: ("DescribePendingMaintenanceActions" :: ByteString),
        "Version" =: ("2014-10-31" :: ByteString),
        "Filters" =: toQuery (toQueryList "Filter" <$> _dpmaFilters),
        "Marker" =: _dpmaMarker,
