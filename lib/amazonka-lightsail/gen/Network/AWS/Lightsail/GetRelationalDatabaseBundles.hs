...
gPageToken :: Lens.Lens' GetRelationalDatabaseBundles (Lude.Maybe Lude.Text)
gPageToken = Lens.lens (pageToken :: GetRelationalDatabaseBundles -> Lude.Maybe Lude.Text) (\s a -> s {pageToken = a} :: GetRelationalDatabaseBundles)
...
instance Page.AWSPager GetRelationalDatabaseBundles where
  page rq rs
    ...
    | Lude.otherwise =
      Lude.Just Lude.$
        rq
          Lude.& gPageToken Lens..~ rs Lens.^. grdbrsNextPageToken
...
data GetRelationalDatabaseBundlesResponse = GetRelationalDatabaseBundlesResponse'
  { -- | The token to advance to the next page of results from your request.
    ...
    bundles :: Lude.Maybe [RelationalDatabaseBundle],
