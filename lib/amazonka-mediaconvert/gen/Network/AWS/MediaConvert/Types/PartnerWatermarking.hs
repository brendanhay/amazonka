...
{-# INLINEABLE pwNexguardFileMarkerSettings #-}
{-# DEPRECATED nexguardFileMarkerSettings "Use generic-lens or generic-optics with 'nexguardFileMarkerSettings' instead"  #-}

instance Core.FromJSON PartnerWatermarking where
        toJSON PartnerWatermarking{..}
          = Core.object
              (Core.catMaybes
                 [("nexguardFileMarkerSettings" Core..=) Core.<$>
          ...
          = Core.withObject "PartnerWatermarking" Core.$
              \ x ->
                PartnerWatermarking' Core.<$>
                  (x Core..:? "nexguardFileMarkerSettings")
