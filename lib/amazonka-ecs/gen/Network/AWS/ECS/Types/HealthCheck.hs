...
mkHealthCheck
    :: HealthCheck
mkHealthCheck
  = HealthCheck'{command = Core.mempty, interval = Core.Nothing,
                 retries = Core.Nothing, startPeriod = Core.Nothing,
...
hcCommand :: Lens.Lens' HealthCheck [Core.Text]
hcCommand = Lens.field @"command"
{-# INLINEABLE hcCommand #-}
...
{-# INLINEABLE hcStartPeriod #-}
{-# DEPRECATED startPeriod "Use generic-lens or generic-optics with 'startPeriod' instead"  #-}
...
instance Core.FromJSON HealthCheck where
        parseJSON
          = Core.withObject "HealthCheck" Core.$
              \ x ->
                HealthCheck' Core.<$>
                  (x Core..:? "command" Core..!= Core.mempty) Core.<*>
                    x Core..:? "interval"
                    Core.<*> x Core..:? "retries"
