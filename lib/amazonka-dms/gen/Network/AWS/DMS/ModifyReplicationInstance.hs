...
instance ToHeaders ModifyReplicationInstance where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AmazonDMSv20160101.ModifyReplicationInstance" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("EngineVersion" .=) <$> _mriEngineVersion,
            ("AutoMinorVersionUpgrade" .=) <$> _mriAutoMinorVersionUpgrade,
            ("AllowMajorVersionUpgrade" .=) <$> _mriAllowMajorVersionUpgrade,
            ("PreferredMaintenanceWindow" .=)
              <$> _mriPreferredMaintenanceWindow,
            ("VpcSecurityGroupIds" .=) <$> _mriVPCSecurityGroupIds,
            ("MultiAZ" .=) <$> _mriMultiAZ,
            ("AllocatedStorage" .=) <$> _mriAllocatedStorage,
            ("ApplyImmediately" .=) <$> _mriApplyImmediately,
            ("ReplicationInstanceClass" .=) <$> _mriReplicationInstanceClass,
            ("ReplicationInstanceIdentifier" .=)
              <$> _mriReplicationInstanceIdentifier,
            Just ("ReplicationInstanceArn" .= _mriReplicationInstanceARN)
          ]
      )
