...
mkRecord
    :: Types.SequenceNumber -- ^ 'sequenceNumber'
    -> Core.Base64 -- ^ 'data\''
...
{-# INLINEABLE rApproximateArrivalTimestamp #-}
{-# DEPRECATED approximateArrivalTimestamp "Use generic-lens or generic-optics with 'approximateArrivalTimestamp' instead"  #-}
...
instance Core.FromJSON Record where
        parseJSON
          = Core.withObject "Record" Core.$
              \ x ->
                Record' Core.<$>
                  (x Core..: "SequenceNumber") Core.<*> x Core..: "Data" Core.<*>
                    x Core..: "PartitionKey"
