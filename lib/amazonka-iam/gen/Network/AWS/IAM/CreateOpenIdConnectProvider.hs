...
instance AWSRequest CreateOpenIdConnectProvider where
  ...
  response =
    receiveXMLWrapper
      ...
      ( \s h x ->
          CreateOpenIdConnectProviderResponse'
            <$> (x .@? "OpenIDConnectProviderArn") <*> (pure (fromEnum s))
      )
...
instance ToQuery CreateOpenIdConnectProvider where
  toQuery CreateOpenIdConnectProvider' {..} =
    mconcat
      [ "Action" =: ("CreateOpenIDConnectProvider" :: ByteString),
        "Version" =: ("2010-05-08" :: ByteString),
        "ClientIDList"
          =: toQuery (toQueryList "member" <$> _coicpClientIdList),
        "Url" =: _coicpURL,
        "ThumbprintList" =: toQueryList "member" _coicpThumbprintList
      ]
