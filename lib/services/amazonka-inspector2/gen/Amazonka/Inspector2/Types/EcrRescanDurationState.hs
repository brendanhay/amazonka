...
newEcrRescanDurationState =
  EcrRescanDurationState'
    { status = Prelude.Nothing,
      rescanDuration = Prelude.Nothing,
...
ecrRescanDurationState_updatedAt = Lens.lens (\EcrRescanDurationState' {updatedAt} -> updatedAt) (\s@EcrRescanDurationState' {} a -> s {updatedAt = a} :: EcrRescanDurationState) Prelude.. Lens.mapping Core._Time

instance Core.FromJSON EcrRescanDurationState where
  parseJSON =
    Core.withObject
      "EcrRescanDurationState"
      ( \x ->
...
instance Prelude.NFData EcrRescanDurationState where
  rnf EcrRescanDurationState' {..} =
    Prelude.rnf status
      `Prelude.seq` Prelude.rnf rescanDuration
