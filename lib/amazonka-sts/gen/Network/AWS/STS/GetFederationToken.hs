...
mkGetFederationToken name
  = GetFederationToken'{name, durationSeconds = Core.Nothing,
...
instance Core.ToQuery GetFederationToken where
        toQuery GetFederationToken{..}
          = Core.toQueryPair "Action" ("GetFederationToken" :: Core.Text)
...
instance Core.AWSRequest GetFederationToken where
        ...
        {-# INLINE toRequest #-}
        parseResponse
          = Response.receiveXMLWrapper "GetFederationTokenResult"
              (\ s h x ->
                 GetFederationTokenResponse' Core.<$>
                   (x Core..@? "Credentials") Core.<*> x Core..@? "FederatedUser"
...
mkGetFederationTokenResponse responseStatus
  = GetFederationTokenResponse'{credentials = Core.Nothing,
                                federatedUser = Core.Nothing, packedPolicySize = Core.Nothing,
                                responseStatus}
...
{-# INLINEABLE gftrrsFederatedUser #-}
{-# DEPRECATED federatedUser "Use generic-lens or generic-optics with 'federatedUser' instead"  #-}
...
{-# INLINEABLE gftrrsPackedPolicySize #-}
