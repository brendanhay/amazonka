...
instance AWSPager DescribeClusterSecurityGroups where
  page rq rs
    ...
    | otherwise = Just $ rq & dcsgMarker .~ rs ^. dcsgsrsMarker
...
instance ToQuery DescribeClusterSecurityGroups where
  toQuery DescribeClusterSecurityGroups' {..} =
    mconcat
      [ "Action" =: ("DescribeClusterSecurityGroups" :: ByteString),
        "Version" =: ("2012-12-01" :: ByteString),
        "TagValues" =: toQuery (toQueryList "TagValue" <$> _dcsgTagValues),
        "TagKeys" =: toQuery (toQueryList "TagKey" <$> _dcsgTagKeys),
