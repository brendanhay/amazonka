...
{-# INLINEABLE deDataURI #-}
{-# DEPRECATED dataURI "Use generic-lens or generic-optics with 'dataURI' instead"  #-}
...
instance Core.FromJSON DatasetEntry where
        parseJSON
          = Core.withObject "DatasetEntry" Core.$
              \ x ->
                DatasetEntry' Core.<$>
                  (x Core..:? "dataURI") Core.<*> x Core..:? "entryName"
