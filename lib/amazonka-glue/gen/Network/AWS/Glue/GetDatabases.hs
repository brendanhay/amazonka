...
gdResourceShareType :: Lens' GetDatabases (Maybe ResourceShareType)
gdResourceShareType = lens _gdResourceShareType (\s a -> s {_gdResourceShareType = a})
...
instance AWSPager GetDatabases where
  page rq rs
    ...
    | otherwise = Just $ rq & gdNextToken .~ rs ^. gdsrsNextToken
...
instance ToHeaders GetDatabases where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("AWSGlue.GetDatabases" :: ByteString),
...
instance ToJSON GetDatabases where
  toJSON GetDatabases' {..} =
    object
      ( catMaybes
          [ ("ResourceShareType" .=) <$> _gdResourceShareType,
