...
-- /ReturnValues/ parameter. Unless you specify conditions, the /DeleteItem/
...
-- operators: = | <> | < | > | <= | >= | BETWEEN | IN Logical operators: NOT
-- | AND | OR.
...
-- parameter does not support lists or maps. A map of attribute/condition
-- pairs. /Expected/ provides a conditional block for the /DeleteItem/
-- operation. Each element of /Expected/ consists of an attribute name, a
...
-- For each /Expected/ element, the result of the evaluation is either true
-- or false. If you specify more than one element in the /Expected/ map,
...
-- this, then at least one of the conditions must evaluate to true, rather
-- than all of them.) If the /Expected/ map evaluates to true, then the
-- conditional operation succeeds; otherwise, it fails. /Expected/ contains
-- the following: /AttributeValueList/ - One or more values to evaluate
...
-- otherwise the condition evaluate to false. If /Exists/ is false, DynamoDB
