...
mkPermission
    :: Permission
mkPermission
  = Permission'{access = Core.Nothing, grantee = Core.Nothing,
...
{-# INLINEABLE pAccess #-}
{-# DEPRECATED access "Use generic-lens or generic-optics with 'access' instead"  #-}
...
instance Core.FromJSON Permission where
        parseJSON
          = Core.withObject "Permission" Core.$
              \ x ->
                Permission' Core.<$>
                  (x Core..:? "Access") Core.<*> x Core..:? "Grantee" Core.<*>
                    x Core..:? "GranteeType"
