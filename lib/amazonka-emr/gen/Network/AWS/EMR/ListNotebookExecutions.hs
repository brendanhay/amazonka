...
mkListNotebookExecutions
    :: ListNotebookExecutions
mkListNotebookExecutions
  = ListNotebookExecutions'{editorId = Core.Nothing,
                            from = Core.Nothing, marker = Core.Nothing, status = Core.Nothing,
                            to = Core.Nothing}
...
instance Core.ToHeaders ListNotebookExecutions where
        toHeaders ListNotebookExecutions{..}
          = Core.pure
          ...
          = Core.object
              (Core.catMaybes
                 [("EditorId" Core..=) Core.<$> editorId,
                  ("From" Core..=) Core.<$> from, ("Marker" Core..=) Core.<$> marker,
                  ("Status" Core..=) Core.<$> status, ("To" Core..=) Core.<$> to])
...
instance Core.AWSRequest ListNotebookExecutions where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 ListNotebookExecutionsResponse' Core.<$>
                   (x Core..:? "Marker") Core.<*> x Core..:? "NotebookExecutions"
...
mkListNotebookExecutionsResponse responseStatus
  = ListNotebookExecutionsResponse'{marker = Core.Nothing,
                                    notebookExecutions = Core.Nothing, responseStatus}
...
{-# INLINEABLE lnerrsNotebookExecutions #-}
{-# DEPRECATED notebookExecutions "Use generic-lens or generic-optics with 'notebookExecutions' instead"  #-}
...
{-# INLINEABLE lnerrsResponseStatus #-}
