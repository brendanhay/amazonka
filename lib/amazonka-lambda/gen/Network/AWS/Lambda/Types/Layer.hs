...
mkLayer
    :: Layer
mkLayer
  = Layer'{arn = Core.Nothing, codeSize = Core.Nothing,
...
{-# INLINEABLE lSigningProfileVersionArn #-}
...
instance Core.FromJSON Layer where
        parseJSON
          = Core.withObject "Layer" Core.$
              \ x ->
                Layer' Core.<$>
                  (x Core..:? "Arn") Core.<*> x Core..:? "CodeSize" Core.<*>
                    x Core..:? "SigningJobArn"
