...
instance AWSPager DescribeClientVPNEndpoints where
  page rq rs
    ...
    | otherwise = Just $ rq & dcveNextToken .~ rs ^. dcversNextToken
...
instance ToQuery DescribeClientVPNEndpoints where
  toQuery DescribeClientVPNEndpoints' {..} =
    mconcat
      [ "Action" =: ("DescribeClientVpnEndpoints" :: ByteString),
        "Version" =: ("2016-11-15" :: ByteString),
        toQuery (toQueryList "Filter" <$> _dcveFilters),
        toQuery
          (toQueryList "ClientVpnEndpointId" <$> _dcveClientVPNEndpointIds),
