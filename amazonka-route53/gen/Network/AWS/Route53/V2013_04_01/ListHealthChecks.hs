...
instance AWSPager ListHealthChecks where
    next rq rs
        | not (_lhcrsIsTruncated rs) = Nothing
        | otherwise = Just $ rq
            { _lhcMarker = _lhcrsNextMarker rs
            }
