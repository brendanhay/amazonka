...
instance AWSRequest PutLifecycleEventHookExecutionStatus where
  type
  ...
  response =
    receiveJSON
      ( \s h x ->
          PutLifecycleEventHookExecutionStatusResponse'
            <$> (x .?> "lifecycleEventHookExecutionId") <*> (pure (fromEnum s))
      )

instance Hashable PutLifecycleEventHookExecutionStatus

instance NFData PutLifecycleEventHookExecutionStatus

instance ToHeaders PutLifecycleEventHookExecutionStatus where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
...
data PutLifecycleEventHookExecutionStatusResponse = PutLifecycleEventHookExecutionStatusResponse'
  { _plehesrsLifecycleEventHookExecutionId ::
      !( Maybe
           Text
       ),
    _plehesrsResponseStatus ::
      !Int
  }
  deriving
    ( Eq,
...
instance NFData PutLifecycleEventHookExecutionStatusResponse
