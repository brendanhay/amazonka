...
mkDeploymentCommand
    :: Types.DeploymentCommandName -- ^ 'name'
    -> DeploymentCommand
...
dcArgs :: Lens.Lens' DeploymentCommand (Core.Maybe (Core.HashMap Core.Text [Core.Text]))
...
instance Core.FromJSON DeploymentCommand where
        parseJSON
          = Core.withObject "DeploymentCommand" Core.$
              \ x ->
                DeploymentCommand' Core.<$>
                  (x Core..: "Name") Core.<*> x Core..:? "Args"
