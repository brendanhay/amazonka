...
--
-- * 'caCloudWatchLoggingOptions' - Use this parameter to configure a CloudWatch log stream to monitor application configuration errors. For more information, see <https://docs.aws.amazon.com/kinesisanalytics/latest/dev/cloudwatch-logs.html Working with Amazon CloudWatch Logs> .
--
...
--
-- * 'caTags' - A list of one or more tags to assign to the application. A tag is a key-value pair that identifies an application. Note that the maximum number of application tags includes system tags. The maximum number of user-defined application tags is 50. For more information, see <https://docs.aws.amazon.com/kinesisanalytics/latest/dev/how-tagging.html Using Tagging> .
--
...
caTags :: Lens' CreateApplication (Maybe (NonEmpty Tag))
caTags = lens _caTags (\s a -> s {_caTags = a}) . mapping _List1
...
instance ToHeaders CreateApplication where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("KinesisAnalytics_20150814.CreateApplication" :: ByteString),
...
instance ToJSON CreateApplication where
  toJSON CreateApplication' {..} =
    object
      ( catMaybes
          [ ("ApplicationDescription" .=) <$> _caApplicationDescription,
            ("Inputs" .=) <$> _caInputs,
            ("CloudWatchLoggingOptions" .=) <$> _caCloudWatchLoggingOptions,
            ("Outputs" .=) <$> _caOutputs,
