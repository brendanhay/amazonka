...
mkUsageRecord
    ...
    -> Types.CustomerIdentifier -- ^ 'customerIdentifier'
    -> Types.UsageDimension -- ^ 'dimension'
    -> UsageRecord
mkUsageRecord timestamp customerIdentifier dimension
  = UsageRecord'{timestamp, customerIdentifier, dimension,
                 quantity = Core.Nothing, usageAllocations = Core.Nothing}
...
{-# INLINEABLE urCustomerIdentifier #-}
...
instance Core.FromJSON UsageRecord where
        parseJSON
          = Core.withObject "UsageRecord" Core.$
              \ x ->
                UsageRecord' Core.<$>
                  (x Core..: "Timestamp") Core.<*> x Core..: "CustomerIdentifier"
