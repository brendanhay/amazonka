...
{-# INLINEABLE plProtectedResourceTypeLimits #-}
{-# DEPRECATED protectedResourceTypeLimits "Use generic-lens or generic-optics with 'protectedResourceTypeLimits' instead"  #-}

instance Core.FromJSON ProtectionLimits where
        parseJSON
          = Core.withObject "ProtectionLimits" Core.$
              \ x ->
                ProtectionLimits' Core.<$>
                  (x Core..:? "ProtectedResourceTypeLimits" Core..!= Core.mempty)
