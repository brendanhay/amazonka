...
kcAdd :: Lens.Lens' KernelCapabilities (Core.Maybe [Core.Text])
...
kcDrop :: Lens.Lens' KernelCapabilities (Core.Maybe [Core.Text])
kcDrop = Lens.field @"drop"
{-# INLINEABLE kcDrop #-}
{-# DEPRECATED drop "Use generic-lens or generic-optics with 'drop' instead"  #-}
...
instance Core.FromJSON KernelCapabilities where
        parseJSON
          = Core.withObject "KernelCapabilities" Core.$
              \ x ->
                KernelCapabilities' Core.<$>
                  (x Core..:? "add") Core.<*> x Core..:? "drop"
