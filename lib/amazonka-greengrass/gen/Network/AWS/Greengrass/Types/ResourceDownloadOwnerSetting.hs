...
mkResourceDownloadOwnerSetting
    :: Core.Text -- ^ 'groupOwner'
    -> Types.Permission -- ^ 'groupPermission'
    -> ResourceDownloadOwnerSetting
...
{-# INLINEABLE rdosGroupPermission #-}
{-# DEPRECATED groupPermission "Use generic-lens or generic-optics with 'groupPermission' instead"  #-}
...
instance Core.FromJSON ResourceDownloadOwnerSetting where
        parseJSON
          = Core.withObject "ResourceDownloadOwnerSetting" Core.$
              \ x ->
                ResourceDownloadOwnerSetting' Core.<$>
                  (x Core..: "GroupOwner") Core.<*> x Core..: "GroupPermission"
