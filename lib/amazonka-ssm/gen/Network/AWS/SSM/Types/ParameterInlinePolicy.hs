...
mkParameterInlinePolicy
    :: ParameterInlinePolicy
mkParameterInlinePolicy
  = ParameterInlinePolicy'{policyStatus = Core.Nothing,
                           policyText = Core.Nothing, policyType = Core.Nothing}
...
pipPolicyStatus :: Lens.Lens' ParameterInlinePolicy (Core.Maybe Core.Text)
pipPolicyStatus = Lens.field @"policyStatus"
{-# INLINEABLE pipPolicyStatus #-}
...
pipPolicyText :: Lens.Lens' ParameterInlinePolicy (Core.Maybe Core.Text)
pipPolicyText = Lens.field @"policyText"
{-# INLINEABLE pipPolicyText #-}
...
pipPolicyType :: Lens.Lens' ParameterInlinePolicy (Core.Maybe Core.Text)
pipPolicyType = Lens.field @"policyType"
{-# INLINEABLE pipPolicyType #-}
...
instance Core.FromJSON ParameterInlinePolicy where
        parseJSON
          = Core.withObject "ParameterInlinePolicy" Core.$
              \ x ->
                ParameterInlinePolicy' Core.<$>
                  (x Core..:? "PolicyStatus") Core.<*> x Core..:? "PolicyText"
