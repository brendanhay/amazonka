...
instance AWSPager ListAWSServiceAccessForOrganization where
  page rq rs
    ...
    | stop (rs ^. lasaforsEnabledServicePrincipals) = Nothing
    | otherwise =
      Just $ rq & lasafoNextToken .~ rs ^. lasaforsNextToken

instance AWSRequest ListAWSServiceAccessForOrganization where
  type
...
instance ToHeaders ListAWSServiceAccessForOrganization where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
...
instance NFData ListAWSServiceAccessForOrganizationResponse
