...
instance ServiceError (Er ELB) where
...
-- back-end connections of your load balancer. If ConnectionSettings attribute
-- is set, Elastic Load Balancing will allow the connections to remain idle
-- (no data is sent over the connection) for the specified duration. For more
-- information, see Configure Idle Connection Timeout.
newtype ConnectionSettings = ConnectionSettings
    { _csIdleTimeout :: Integer
      -- ^ Specifies the maximum time the connection is allowed to be idle
...
-- information to the Amazon S3 bucket you specify. For more information, see
-- Enable Access Logs.
...
data LoadBalancerAttributes = LoadBalancerAttributes
    ...
    , _lbaAccessLog :: Maybe AccessLog
      ...
      -- delivers the information to the Amazon S3 bucket you specify. For
      -- more information, see Enable Access Logs.
