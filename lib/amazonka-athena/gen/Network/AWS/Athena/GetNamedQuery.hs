...
newtype GetNamedQuery = GetNamedQuery' {_gnqNamedQueryId :: Text}
...
instance ToHeaders GetNamedQuery where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("AmazonAthena.GetNamedQuery" :: ByteString),
...
instance ToJSON GetNamedQuery where
  toJSON GetNamedQuery' {..} =
    object (catMaybes [Just ("NamedQueryId" .= _gnqNamedQueryId)])
