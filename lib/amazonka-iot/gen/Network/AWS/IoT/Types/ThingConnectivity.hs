...
mkThingConnectivity
    :: ThingConnectivity
mkThingConnectivity
  = ThingConnectivity'{connected = Core.Nothing,
...
{-# INLINEABLE tcConnected #-}
{-# DEPRECATED connected "Use generic-lens or generic-optics with 'connected' instead"  #-}
...
instance Core.FromJSON ThingConnectivity where
        parseJSON
          = Core.withObject "ThingConnectivity" Core.$
              \ x ->
                ThingConnectivity' Core.<$>
                  (x Core..:? "connected") Core.<*> x Core..:? "timestamp"
