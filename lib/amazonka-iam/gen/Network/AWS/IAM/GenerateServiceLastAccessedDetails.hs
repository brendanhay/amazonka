...
{-# INLINEABLE gsladGranularity #-}
{-# DEPRECATED granularity "Use generic-lens or generic-optics with 'granularity' instead"  #-}

instance Core.ToQuery GenerateServiceLastAccessedDetails where
        toQuery GenerateServiceLastAccessedDetails{..}
          = Core.toQueryPair "Action"
              ("GenerateServiceLastAccessedDetails" :: Core.Text)
              Core.<> Core.toQueryPair "Version" ("2010-05-08" :: Core.Text)
              Core.<> Core.toQueryPair "Arn" arn
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "Granularity") granularity

instance Core.ToHeaders GenerateServiceLastAccessedDetails where
...
instance Core.AWSRequest GenerateServiceLastAccessedDetails where
        ...
        parseResponse
          = Response.receiveXMLWrapper
              ...
              (\ s h x ->
                 GenerateServiceLastAccessedDetailsResponse' Core.<$>
...
mkGenerateServiceLastAccessedDetailsResponse responseStatus
  = GenerateServiceLastAccessedDetailsResponse'{jobId = Core.Nothing,
                                                responseStatus}
...
{-# INLINEABLE gsladrfrsResponseStatus #-}
