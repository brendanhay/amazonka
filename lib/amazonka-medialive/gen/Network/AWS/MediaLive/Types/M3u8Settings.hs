...
mkM3u8Settings
    :: M3u8Settings
mkM3u8Settings
  = M3u8Settings'{audioFramesPerPes = Core.Nothing,
                  audioPids = Core.Nothing, ecmPid = Core.Nothing,
                  nielsenId3Behavior = Core.Nothing, patInterval = Core.Nothing,
                  ...
                  scte35Behavior = Core.Nothing, scte35Pid = Core.Nothing,
...
{-# INLINEABLE msNielsenId3Behavior #-}
...
{-# INLINEABLE msScte35Behavior #-}
{-# DEPRECATED scte35Behavior "Use generic-lens or generic-optics with 'scte35Behavior' instead"  #-}
...
{-# INLINEABLE msTimedMetadataBehavior #-}
...
instance Core.FromJSON M3u8Settings where
        parseJSON
          = Core.withObject "M3u8Settings" Core.$
              \ x ->
                M3u8Settings' Core.<$>
                  (x Core..:? "audioFramesPerPes") Core.<*> x Core..:? "audioPids"
                    Core.<*> x Core..:? "ecmPid"
