...
mkEnvironmentMember
    :: EnvironmentMember
mkEnvironmentMember
  = EnvironmentMember'{environmentId = Core.Nothing,
                       lastAccess = Core.Nothing, permissions = Core.Nothing,
                       userArn = Core.Nothing, userId = Core.Nothing}
...
{-# INLINEABLE emLastAccess #-}
{-# DEPRECATED lastAccess "Use generic-lens or generic-optics with 'lastAccess' instead"  #-}
...
instance Core.FromJSON EnvironmentMember where
        parseJSON
          = Core.withObject "EnvironmentMember" Core.$
              \ x ->
                EnvironmentMember' Core.<$>
                  (x Core..:? "environmentId") Core.<*> x Core..:? "lastAccess"
