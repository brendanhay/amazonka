...
{-# INLINEABLE plpMaximumIndividualPlayerLatencyMilliseconds #-}
{-# DEPRECATED maximumIndividualPlayerLatencyMilliseconds "Use generic-lens or generic-optics with 'maximumIndividualPlayerLatencyMilliseconds' instead"  #-}
...
{-# INLINEABLE plpPolicyDurationSeconds #-}
{-# DEPRECATED policyDurationSeconds "Use generic-lens or generic-optics with 'policyDurationSeconds' instead"  #-}

instance Core.FromJSON PlayerLatencyPolicy where
        toJSON PlayerLatencyPolicy{..}
          = Core.object
              (Core.catMaybes
                 [("MaximumIndividualPlayerLatencyMilliseconds" Core..=) Core.<$>
...
instance Core.FromJSON PlayerLatencyPolicy where
        parseJSON
          = Core.withObject "PlayerLatencyPolicy" Core.$
              \ x ->
                PlayerLatencyPolicy' Core.<$>
                  (x Core..:? "MaximumIndividualPlayerLatencyMilliseconds") Core.<*>
                    x Core..:? "PolicyDurationSeconds"
