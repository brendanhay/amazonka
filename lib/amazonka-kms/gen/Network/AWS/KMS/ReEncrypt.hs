...
data ReEncrypt = ReEncrypt'
  { -- | Specifies that encryption context to use when the reencrypting the data.
    ...
    destinationEncryptionContext :: Lude.Maybe (Lude.HashMap Lude.Text (Lude.Text)),
    ...
    -- An /encryption context/ is a collection of non-secret key-value pairs that represents additional authenticated data. When you use an encryption context to encrypt data, you must specify the same (an exact case-sensitive match) encryption context to decrypt the data. An encryption context is optional when encrypting with a symmetric CMK, but it is highly recommended.
    -- For more information, see <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#encrypt_context Encryption Context> in the /AWS Key Management Service Developer Guide/ .
    sourceEncryptionContext :: Lude.Maybe (Lude.HashMap Lude.Text (Lude.Text)),
    ...
    destinationEncryptionAlgorithm :: Lude.Maybe EncryptionAlgorithmSpec,
