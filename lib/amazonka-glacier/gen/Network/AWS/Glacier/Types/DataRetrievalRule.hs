...
mkDataRetrievalRule
    :: DataRetrievalRule
mkDataRetrievalRule
  = DataRetrievalRule'{bytesPerHour = Core.Nothing,
...
{-# INLINEABLE drrBytesPerHour #-}
{-# DEPRECATED bytesPerHour "Use generic-lens or generic-optics with 'bytesPerHour' instead"  #-}
...
drrStrategy :: Lens.Lens' DataRetrievalRule (Core.Maybe Core.Text)
drrStrategy = Lens.field @"strategy"
{-# INLINEABLE drrStrategy #-}
...
instance Core.FromJSON DataRetrievalRule where
        parseJSON
          = Core.withObject "DataRetrievalRule" Core.$
              \ x ->
                DataRetrievalRule' Core.<$>
                  (x Core..:? "BytesPerHour") Core.<*> x Core..:? "Strategy"
