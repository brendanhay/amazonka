...
mkMaintenanceWindowTaskInvocationParameters
  = MaintenanceWindowTaskInvocationParameters'{automation =
                                                 Core.Nothing,
                                               lambda = Core.Nothing, runCommand = Core.Nothing,
...
{-# INLINEABLE mwtipAutomation #-}
{-# DEPRECATED automation "Use generic-lens or generic-optics with 'automation' instead"  #-}
...
{-# INLINEABLE mwtipRunCommand #-}
{-# DEPRECATED runCommand "Use generic-lens or generic-optics with 'runCommand' instead"  #-}
...
{-# INLINEABLE mwtipStepFunctions #-}
{-# DEPRECATED stepFunctions "Use generic-lens or generic-optics with 'stepFunctions' instead"  #-}

instance Core.FromJSON MaintenanceWindowTaskInvocationParameters
         where
...
instance Core.FromJSON MaintenanceWindowTaskInvocationParameters
         where
        parseJSON
          = Core.withObject "MaintenanceWindowTaskInvocationParameters"
              Core.$
              \ x ->
                MaintenanceWindowTaskInvocationParameters' Core.<$>
                  (x Core..:? "Automation") Core.<*> x Core..:? "Lambda" Core.<*>
                    x Core..:? "RunCommand"
