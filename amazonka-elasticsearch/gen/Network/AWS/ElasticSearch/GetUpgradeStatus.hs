...
instance AWSRequest GetUpgradeStatus where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 GetUpgradeStatusResponse' <$>
                   (x .?> "StepStatus") <*> (x .?> "UpgradeName") <*>
                     (x .?> "UpgradeStep")
                     <*> (pure (fromEnum s)))

instance Hashable GetUpgradeStatus where

instance NFData GetUpgradeStatus where
...
data GetUpgradeStatusResponse = GetUpgradeStatusResponse'
  { _gusrsStepStatus     :: !(Maybe UpgradeStatus)
...
instance NFData GetUpgradeStatusResponse where
