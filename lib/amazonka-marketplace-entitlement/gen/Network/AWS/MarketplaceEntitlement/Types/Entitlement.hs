...
mkEntitlement
    :: Entitlement
mkEntitlement
  = Entitlement'{customerIdentifier = Core.Nothing,
                 dimension = Core.Nothing, expirationDate = Core.Nothing,
                 productCode = Core.Nothing, value = Core.Nothing}
...
{-# INLINEABLE eCustomerIdentifier #-}
...
instance Core.FromJSON Entitlement where
        parseJSON
          = Core.withObject "Entitlement" Core.$
              \ x ->
                Entitlement' Core.<$>
                  (x Core..:? "CustomerIdentifier") Core.<*> x Core..:? "Dimension"
