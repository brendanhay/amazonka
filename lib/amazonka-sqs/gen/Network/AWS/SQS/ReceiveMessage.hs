...
instance AWSRequest ReceiveMessage where
  ...
  response =
    receiveXMLWrapper
      ...
      ( \s h x ->
          ReceiveMessageResponse'
            <$> (may (parseXMLList "Message") x) <*> (pure (fromEnum s))
      )
  ...
  toQuery ReceiveMessage' {..} =
    mconcat
      [ "Action" =: ("ReceiveMessage" :: ByteString),
        ...
        "ReceiveRequestAttemptId" =: _rmReceiveRequestAttemptId,
        "VisibilityTimeout" =: _rmVisibilityTimeout,
        toQuery
          (toQueryList "MessageAttributeName" <$> _rmMessageAttributeNames),
        "WaitTimeSeconds" =: _rmWaitTimeSeconds,
        toQuery (toQueryList "AttributeName" <$> _rmAttributeNames),
