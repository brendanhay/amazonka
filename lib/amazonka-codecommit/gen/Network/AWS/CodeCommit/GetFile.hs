...
--
-- * 'getCommitSpecifier' - The fully quaified reference that identifies the commit that contains the file. For example, you can specify a full commit ID, a tag, a branch name, or a reference such as refs/heads/master. If none is provided, the head commit is used.
--
...
instance ToHeaders GetFile where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("CodeCommit_20150413.GetFile" :: ByteString),
...
--
-- * 'getrsFilePath' - The fully qualified path to the specified file. Returns the name and extension of the file.
--
