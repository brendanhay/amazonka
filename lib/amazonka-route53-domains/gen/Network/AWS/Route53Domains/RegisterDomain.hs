...
instance ToHeaders RegisterDomain where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("Route53Domains_v20140515.RegisterDomain" :: ByteString),
...
instance ToJSON RegisterDomain where
  toJSON RegisterDomain' {..} =
    object
      ( catMaybes
          [ ("PrivacyProtectTechContact" .=) <$> _rPrivacyProtectTechContact,
            ("PrivacyProtectRegistrantContact" .=)
              <$> _rPrivacyProtectRegistrantContact,
            ("AutoRenew" .=) <$> _rAutoRenew,
            ("PrivacyProtectAdminContact" .=) <$> _rPrivacyProtectAdminContact,
