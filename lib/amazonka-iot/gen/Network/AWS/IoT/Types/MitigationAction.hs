...
mkMitigationAction
    :: MitigationAction
mkMitigationAction
  = MitigationAction'{actionParams = Core.Nothing, id = Core.Nothing,
...
instance Core.FromJSON MitigationAction where
        parseJSON
          = Core.withObject "MitigationAction" Core.$
              \ x ->
                MitigationAction' Core.<$>
                  (x Core..:? "actionParams") Core.<*> x Core..:? "id" Core.<*>
                    x Core..:? "name"
                    Core.<*> x Core..:? "roleArn"
