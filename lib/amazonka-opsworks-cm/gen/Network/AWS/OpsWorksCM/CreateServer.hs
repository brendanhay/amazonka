...
data CreateServer = CreateServer'
  { _csEngineVersion ::
    ...
    _csCustomPrivateKey :: !(Maybe (Sensitive Text)),
...
instance AWSRequest CreateServer where
  ...
  response =
    receiveJSON
      ( \s h x ->
          CreateServerResponse' <$> (x .?> "Server") <*> (pure (fromEnum s))
      )
...
instance ToHeaders CreateServer where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("OpsWorksCM_V2016_11_01.CreateServer" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("EngineVersion" .=) <$> _csEngineVersion,
            ...
            ("AssociatePublicIpAddress" .=) <$> _csAssociatePublicIPAddress,
            ...
            ("PreferredBackupWindow" .=) <$> _csPreferredBackupWindow,
            ("CustomCertificate" .=) <$> _csCustomCertificate,
            ("Tags" .=) <$> _csTags,
            ("BackupRetentionCount" .=) <$> _csBackupRetentionCount,
