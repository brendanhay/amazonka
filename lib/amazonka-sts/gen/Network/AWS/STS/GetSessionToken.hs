...
mkGetSessionToken
    :: GetSessionToken
mkGetSessionToken
  = GetSessionToken'{durationSeconds = Core.Nothing,
                     serialNumber = Core.Nothing, tokenCode = Core.Nothing}
...
{-# INLINEABLE gstTokenCode #-}
{-# DEPRECATED tokenCode "Use generic-lens or generic-optics with 'tokenCode' instead"  #-}

instance Core.ToQuery GetSessionToken where
        toQuery GetSessionToken{..}
          = Core.toQueryPair "Action" ("GetSessionToken" :: Core.Text)
              Core.<> Core.toQueryPair "Version" ("2011-06-15" :: Core.Text)
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "DurationSeconds")
                durationSeconds
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "SerialNumber")
                serialNumber
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "TokenCode") tokenCode
...
mkGetSessionTokenResponse responseStatus
  = GetSessionTokenResponse'{credentials = Core.Nothing,
                             responseStatus}
