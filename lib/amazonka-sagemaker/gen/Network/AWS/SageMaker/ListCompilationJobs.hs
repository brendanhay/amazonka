...
instance ToHeaders ListCompilationJobs where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("SageMaker.ListCompilationJobs" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("NameContains" .=) <$> _lcjNameContains,
            ("LastModifiedTimeBefore" .=) <$> _lcjLastModifiedTimeBefore,
            ("CreationTimeAfter" .=) <$> _lcjCreationTimeAfter,
            ("NextToken" .=) <$> _lcjNextToken,
            ("SortOrder" .=) <$> _lcjSortOrder,
            ("LastModifiedTimeAfter" .=) <$> _lcjLastModifiedTimeAfter,
