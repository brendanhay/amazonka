...
data CreateRelationalDatabase = CreateRelationalDatabase'
  { _crdMasterUserPassword            :: !(Maybe (Sensitive Text))
  , _crdPubliclyAccessible            :: !(Maybe Bool)
  , _crdPreferredMaintenanceWindow    :: !(Maybe Text)
  , _crdPreferredBackupWindow         :: !(Maybe Text)
  , _crdAvailabilityZone              :: !(Maybe Text)
  , _crdTags                          :: !(Maybe [Tag])
  , _crdRelationalDatabaseName        :: !Text
  , _crdRelationalDatabaseBlueprintId :: !Text
...
--
-- * 'crdTags' - The tag keys and optional values to add to the resource during create. To tag a resource after it has been created, see the @tag resource@ operation.
--
...
createRelationalDatabase
    :: Text -- ^ 'crdRelationalDatabaseName'
    -> Text -- ^ 'crdRelationalDatabaseBlueprintId'
    -> Text -- ^ 'crdRelationalDatabaseBundleId'
    -> Text -- ^ 'crdMasterDatabaseName'
    -> Text -- ^ 'crdMasterUsername'
    -> CreateRelationalDatabase
createRelationalDatabase pRelationalDatabaseName_ pRelationalDatabaseBlueprintId_ pRelationalDatabaseBundleId_ pMasterDatabaseName_ pMasterUsername_ =
  CreateRelationalDatabase'
    ...
    , _crdRelationalDatabaseBlueprintId = pRelationalDatabaseBlueprintId_
...
instance AWSRequest CreateRelationalDatabase where
        type Rs CreateRelationalDatabase =
             CreateRelationalDatabaseResponse
...
instance Hashable CreateRelationalDatabase where

instance NFData CreateRelationalDatabase where
...
data CreateRelationalDatabaseResponse = CreateRelationalDatabaseResponse'
  { _crdrsOperations     :: !(Maybe [Operation])
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'crdrsOperations' - An object describing the result of your create relational database request.
--
...
createRelationalDatabaseResponse pResponseStatus_ =
  CreateRelationalDatabaseResponse'
    {_crdrsOperations = Nothing, _crdrsResponseStatus = pResponseStatus_}
