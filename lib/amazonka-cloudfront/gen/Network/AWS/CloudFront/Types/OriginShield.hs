...
{-# INLINEABLE osOriginShieldRegion #-}
{-# DEPRECATED originShieldRegion "Use generic-lens or generic-optics with 'originShieldRegion' instead"  #-}

instance Core.ToXML OriginShield where
        toXML OriginShield{..}
          = Core.toXMLElement "Enabled" enabled Core.<>
              Core.maybe Core.mempty (Core.toXMLElement "OriginShieldRegion")
                originShieldRegion

instance Core.FromXML OriginShield where
        parseXML x
          = OriginShield' Core.<$>
              (x Core..@ "Enabled") Core.<*> x Core..@? "OriginShieldRegion"
