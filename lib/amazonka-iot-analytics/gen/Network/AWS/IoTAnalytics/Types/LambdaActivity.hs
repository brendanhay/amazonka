...
mkLambdaActivity
    :: Types.Name -- ^ 'name'
    -> Types.LambdaName -- ^ 'lambdaName'
    -> Core.Natural -- ^ 'batchSize'
    -> LambdaActivity
...
{-# INLINEABLE laLambdaName #-}
{-# DEPRECATED lambdaName "Use generic-lens or generic-optics with 'lambdaName' instead"  #-}
...
instance Core.FromJSON LambdaActivity where
        parseJSON
          = Core.withObject "LambdaActivity" Core.$
              \ x ->
                LambdaActivity' Core.<$>
                  (x Core..: "name") Core.<*> x Core..: "lambdaName" Core.<*>
                    x Core..: "batchSize"
                    Core.<*> x Core..:? "next"
