...
import qualified Network.AWS.Route53.Types.HostedZoneId as Types
...
-- | Creates a 'AliasTarget' value with any optional fields omitted.
mkAliasTarget
    :: Types.HostedZoneId -- ^ 'hostedZoneId'
    -> Types.DNSName -- ^ 'dNSName'
    -> Core.Bool -- ^ 'evaluateTargetHealth'
    -> AliasTarget
mkAliasTarget hostedZoneId dNSName evaluateTargetHealth
...
atHostedZoneId :: Lens.Lens' AliasTarget Types.HostedZoneId
atHostedZoneId = Lens.field @"hostedZoneId"
{-# INLINEABLE atHostedZoneId #-}
...
atDNSName :: Lens.Lens' AliasTarget Types.DNSName
atDNSName = Lens.field @"dNSName"
{-# INLINEABLE atDNSName #-}
...
atEvaluateTargetHealth :: Lens.Lens' AliasTarget Core.Bool
atEvaluateTargetHealth = Lens.field @"evaluateTargetHealth"
{-# INLINEABLE atEvaluateTargetHealth #-}
{-# DEPRECATED evaluateTargetHealth "Use generic-lens or generic-optics with 'evaluateTargetHealth' instead"  #-}

instance Core.ToXML AliasTarget where
        toXML AliasTarget{..}
          = Core.toXMLElement "HostedZoneId" hostedZoneId Core.<>
              Core.toXMLElement "DNSName" dNSName
              Core.<>
              Core.toXMLElement "EvaluateTargetHealth" evaluateTargetHealth

instance Core.FromXML AliasTarget where
        parseXML x
          = AliasTarget' Core.<$>
              (x Core..@ "HostedZoneId") Core.<*> x Core..@ "DNSName" Core.<*>
                x Core..@ "EvaluateTargetHealth"
