...
mkListBuilds
    :: ListBuilds
mkListBuilds
  = ListBuilds'{limit = Core.Nothing, nextToken = Core.Nothing,
                status = Core.Nothing}
...
instance Core.ToHeaders ListBuilds where
        toHeaders ListBuilds{..}
          = Core.pure ("X-Amz-Target", "GameLift.ListBuilds") Core.<>
          ...
          = Response.receiveJSON
              (\ s h x ->
                 ListBuildsResponse' Core.<$>
                   (x Core..:? "Builds") Core.<*> x Core..:? "NextToken" Core.<*>
...
mkListBuildsResponse responseStatus
  = ListBuildsResponse'{builds = Core.Nothing,
