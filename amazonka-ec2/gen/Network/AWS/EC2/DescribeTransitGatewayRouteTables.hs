...
--
-- * 'dtgrtsFilters' - One or more filters. The possible values are:     * @default-association-route-table@ - Indicates whether this is the default association route table for the transit gateway (@true@ | @false@ ).     * @default-propagation-route-table@ - Indicates whether this is the default propagation route table for the transit gateway (@true@ | @false@ ).     * @state@ - The state of the attachment (@available@ | @deleted@ | @deleting@ | @failed@ | @modifying@ | @pendingAcceptance@ | @pending@ | @rollingBack@ | @rejected@ | @rejecting@ ).     * @transit-gateway-id@ - The ID of the transit gateway.     * @transit-gateway-route-table-id@ - The ID of the transit gateway route table.
--
...
instance AWSPager DescribeTransitGatewayRouteTables
         where
        page rq rs
          | stop (rs ^. dtgrtsrsNextToken) = Nothing
          | stop (rs ^. dtgrtsrsTransitGatewayRouteTables) =
            Nothing
          | otherwise =
            Just $ rq &
              dtgrtsNextToken .~ rs ^. dtgrtsrsNextToken

instance AWSRequest DescribeTransitGatewayRouteTables
         where
...
instance ToHeaders DescribeTransitGatewayRouteTables
         where
        toHeaders = const mempty

instance ToPath DescribeTransitGatewayRouteTables
         where
        toPath = const "/"

instance ToQuery DescribeTransitGatewayRouteTables
         where
