...
mkLambdaTarget
    :: LambdaTarget
mkLambdaTarget
  = LambdaTarget'{deploymentId = Core.Nothing,
                  lambdaFunctionInfo = Core.Nothing, lastUpdatedAt = Core.Nothing,
...
{-# INLINEABLE ltLambdaFunctionInfo #-}
{-# DEPRECATED lambdaFunctionInfo "Use generic-lens or generic-optics with 'lambdaFunctionInfo' instead"  #-}
...
instance Core.FromJSON LambdaTarget where
        parseJSON
          = Core.withObject "LambdaTarget" Core.$
              \ x ->
                LambdaTarget' Core.<$>
                  (x Core..:? "deploymentId") Core.<*>
                    x Core..:? "lambdaFunctionInfo"
