...
mkLabelingJobDataSource
    :: LabelingJobDataSource
mkLabelingJobDataSource
  = LabelingJobDataSource'{s3DataSource = Core.Nothing,
...
{-# INLINEABLE ljdsSnsDataSource #-}
{-# DEPRECATED snsDataSource "Use generic-lens or generic-optics with 'snsDataSource' instead"  #-}
...
instance Core.FromJSON LabelingJobDataSource where
        parseJSON
          = Core.withObject "LabelingJobDataSource" Core.$
              \ x ->
                LabelingJobDataSource' Core.<$>
                  (x Core..:? "S3DataSource") Core.<*> x Core..:? "SnsDataSource"
