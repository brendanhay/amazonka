...
mkBudget
    ...
    -> Types.BudgetType -- ^ 'budgetType'
    -> Budget
mkBudget budgetName timeUnit budgetType
  = Budget'{budgetName, timeUnit, budgetType,
            budgetLimit = Core.Nothing, calculatedSpend = Core.Nothing,
            costFilters = Core.Nothing, costTypes = Core.Nothing,
            lastUpdatedTime = Core.Nothing, plannedBudgetLimits = Core.Nothing,
...
{-# INLINEABLE bBudgetLimit #-}
{-# DEPRECATED budgetLimit "Use generic-lens or generic-optics with 'budgetLimit' instead"  #-}
...
{-# INLINEABLE bCalculatedSpend #-}
{-# DEPRECATED calculatedSpend "Use generic-lens or generic-optics with 'calculatedSpend' instead"  #-}
...
{-# INLINEABLE bPlannedBudgetLimits #-}
{-# DEPRECATED plannedBudgetLimits "Use generic-lens or generic-optics with 'plannedBudgetLimits' instead"  #-}
...
instance Core.FromJSON Budget where
        parseJSON
          = Core.withObject "Budget" Core.$
              \ x ->
                Budget' Core.<$>
                  (x Core..: "BudgetName") Core.<*> x Core..: "TimeUnit" Core.<*>
                    x Core..: "BudgetType"
