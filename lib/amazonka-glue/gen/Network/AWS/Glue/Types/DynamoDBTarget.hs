...
mkDynamoDBTarget
    :: DynamoDBTarget
mkDynamoDBTarget
  = DynamoDBTarget'{path = Core.Nothing, scanAll = Core.Nothing,
...
{-# INLINEABLE ddbtScanRate #-}
{-# DEPRECATED scanRate "Use generic-lens or generic-optics with 'scanRate' instead"  #-}
...
instance Core.FromJSON DynamoDBTarget where
        parseJSON
          = Core.withObject "DynamoDBTarget" Core.$
              \ x ->
                DynamoDBTarget' Core.<$>
                  (x Core..:? "Path") Core.<*> x Core..:? "scanAll" Core.<*>
                    x Core..:? "scanRate"
