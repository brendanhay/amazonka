...
{-# INLINEABLE pfgPatchFilters #-}
{-# DEPRECATED patchFilters "Use generic-lens or generic-optics with 'patchFilters' instead"  #-}
...
instance Core.FromJSON PatchFilterGroup where
        parseJSON
          = Core.withObject "PatchFilterGroup" Core.$
              \ x ->
                PatchFilterGroup' Core.<$>
                  (x Core..:? "PatchFilters" Core..!= Core.mempty)
