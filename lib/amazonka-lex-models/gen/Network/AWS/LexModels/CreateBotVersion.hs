...
instance ToJSON CreateBotVersion where
  toJSON CreateBotVersion' {..} =
    object (catMaybes [("checksum" .=) <$> _cbvChecksum])
...
--
-- * 'cbvrsAbortStatement' - The message that Amazon Lex uses to cancel a conversation. For more information, see 'PutBot' .
--
...
--
-- * 'cbvrsEnableModelImprovements' - Indicates whether the bot uses accuracy improvements. @true@ indicates that the bot is using the improvements, otherwise, @false@ .
--
...
createBotVersionResponse pResponseStatus_ =
  CreateBotVersionResponse'
    { _cbvrsFailureReason = Nothing,
      ...
      _cbvrsEnableModelImprovements = Nothing,
...
cbvrsEnableModelImprovements :: Lens' CreateBotVersionResponse (Maybe Bool)
cbvrsEnableModelImprovements = lens _cbvrsEnableModelImprovements (\s a -> s {_cbvrsEnableModelImprovements = a})
