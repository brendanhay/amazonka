...
mkHlsOutputSettings
    :: Types.HlsSettings -- ^ 'hlsSettings'
    -> HlsOutputSettings
mkHlsOutputSettings hlsSettings
  = HlsOutputSettings'{hlsSettings, h265PackagingType = Core.Nothing,
                       nameModifier = Core.Nothing, segmentModifier = Core.Nothing}
...
{-# INLINEABLE hosH265PackagingType #-}
          ...
          = Core.withObject "HlsOutputSettings" Core.$
              \ x ->
                HlsOutputSettings' Core.<$>
                  (x Core..: "hlsSettings") Core.<*> x Core..:? "h265PackagingType"
