...
data Cluster = Cluster'
  { -- | The applications installed on this cluster.
    ...
    -- @TERMINATE_AT_TASK_COMPLETION@ is available only in Amazon EMR releases
    ...
    -- principals in the Amazon Web Services account can perform Amazon EMR
    -- cluster actions on the cluster that their IAM policies allow. When
    -- @false@, only the IAM principal that created the cluster and the Amazon
    -- Web Services account root user can perform Amazon EMR actions,
...
-- configurations that are supplied to the Amazon EMR cluster.
--
-- 'customAmiId', 'cluster_customAmiId' - Available only in Amazon EMR releases 5.7.0 and later. The ID of a
-- custom Amazon EBS-backed Linux AMI if the cluster uses a custom AMI.
--
...
-- releases 4.x and later.
--
-- 'ec2InstanceAttributes', 'cluster_ec2InstanceAttributes' - Provides information about the Amazon EC2 instances in a cluster grouped
-- by category. For example, key name, subnet ID, IAM instance profile, and
-- so on.
--
...
--
-- 'terminationProtected', 'cluster_terminationProtected' - Indicates whether Amazon EMR will lock the cluster to prevent the Amazon
-- EC2 instances from being terminated by an API call or user intervention,
-- or in the event of a cluster error.
--
