...
mkIPSet
    ...
    -> IPSet
mkIPSet iPSetId
  = IPSet'{iPSetId, iPSetDescriptors = Core.mempty,
           name = Core.Nothing}
...
{-# INLINEABLE ipsIPSetDescriptors #-}
{-# DEPRECATED iPSetDescriptors "Use generic-lens or generic-optics with 'iPSetDescriptors' instead"  #-}
        ...
        parseJSON
          = Core.withObject "IPSet" Core.$
              \ x ->
                IPSet' Core.<$>
                  (x Core..: "IPSetId") Core.<*>
                    x Core..:? "IPSetDescriptors" Core..!= Core.mempty
                    Core.<*> x Core..:? "Name"
