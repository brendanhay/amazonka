...
data ComputeLimits = ComputeLimits'
  { -- | The upper boundary of Amazon EC2 units for core node type in a cluster.
    ...
    -- vCPU cores or instances for instance groups and measured through units
    -- for instance fleets. The On-Demand units are not allowed to scale beyond
...
--
-- 'maximumCoreCapacityUnits', 'computeLimits_maximumCoreCapacityUnits' - The upper boundary of Amazon EC2 units for core node type in a cluster.
-- It is measured through vCPU cores or instances for instance groups and
-- measured through units for instance fleets. The core units are not
-- allowed to scale beyond this boundary. The parameter is used to split
-- capacity allocation between core and task nodes.
--
-- 'maximumOnDemandCapacityUnits', 'computeLimits_maximumOnDemandCapacityUnits' - The upper boundary of On-Demand Amazon EC2 units. It is measured through
-- vCPU cores or instances for instance groups and measured through units
-- for instance fleets. The On-Demand units are not allowed to scale beyond
-- this boundary. The parameter is used to split capacity allocation
-- between On-Demand and Spot Instances.
...
--
-- 'minimumCapacityUnits', 'computeLimits_minimumCapacityUnits' - The lower boundary of Amazon EC2 units. It is measured through vCPU
-- cores or instances for instance groups and measured through units for
-- instance fleets. Managed scaling activities are not allowed beyond this
-- boundary. The limit only applies to the core and task nodes. The master
-- node cannot be scaled after initial configuration.
--
-- 'maximumCapacityUnits', 'computeLimits_maximumCapacityUnits' - The upper boundary of Amazon EC2 units. It is measured through vCPU
-- cores or instances for instance groups and measured through units for
-- instance fleets. Managed scaling activities are not allowed beyond this
-- boundary. The limit only applies to the core and task nodes. The master
-- node cannot be scaled after initial configuration.
