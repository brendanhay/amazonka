...
instance Core.ToHeaders StopAutoMLJob where
        toHeaders StopAutoMLJob{..}
          = Core.pure ("X-Amz-Target", "SageMaker.StopAutoMLJob") Core.<>
...
instance Core.AWSRequest StopAutoMLJob where
        ...
        {-# INLINE toRequest #-}
        parseResponse = Response.receiveNull StopAutoMLJobResponse'
