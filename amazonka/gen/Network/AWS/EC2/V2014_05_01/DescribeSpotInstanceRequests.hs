...
-- &amp;AUTHPARAMS.
module Network.AWS.EC2.V2014_05_01.DescribeSpotInstanceRequests where
...
data DescribeSpotInstanceRequests = DescribeSpotInstanceRequests
    ...
    , _dsirrFilters :: [Filter]
      ...
      -- type of the Amazon EBS volume (standard | io1). launch.group-id -
      -- The security group for the instance. launch.image-id - The ID of
      -- the AMI. launch.instance-type - The type of instance (for
      -- example, m1.small). launch.kernel-id - The kernel ID.
      -- launch.key-name - The name of the key pair the instance launched
      -- with. launch.monitoring-enabled - Whether monitoring is enabled
      -- for the Spot Instance. launch.ramdisk-id - The RAM disk ID.
      -- launch.network-interface.network-interface-id - The ID of the
      -- network interface. launch.network-interface.device-index - The
      -- index of the device for the network interface attachment on the
      -- instance. launch.network-interface.subnet-id - The ID of the
      -- subnet for the instance. launch.network-interface.description - A
