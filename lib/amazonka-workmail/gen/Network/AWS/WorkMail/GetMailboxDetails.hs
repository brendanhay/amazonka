...
instance Core.AWSRequest GetMailboxDetails where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 GetMailboxDetailsResponse' Core.<$>
                   (x Core..:? "MailboxQuota") Core.<*> x Core..:? "MailboxSize"
...
mkGetMailboxDetailsResponse responseStatus
  = GetMailboxDetailsResponse'{mailboxQuota = Core.Nothing,
                               mailboxSize = Core.Nothing, responseStatus}
...
{-# INLINEABLE gmdrrsMailboxSize #-}
{-# DEPRECATED mailboxSize "Use generic-lens or generic-optics with 'mailboxSize' instead"  #-}
