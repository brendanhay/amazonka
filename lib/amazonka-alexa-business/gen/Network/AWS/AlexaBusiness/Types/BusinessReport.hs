...
mkBusinessReport
    :: BusinessReport
mkBusinessReport
  = BusinessReport'{deliveryTime = Core.Nothing,
                    downloadUrl = Core.Nothing, failureCode = Core.Nothing,
...
{-# INLINEABLE brDeliveryTime #-}
{-# DEPRECATED deliveryTime "Use generic-lens or generic-optics with 'deliveryTime' instead"  #-}
...
instance Core.FromJSON BusinessReport where
        parseJSON
          = Core.withObject "BusinessReport" Core.$
              \ x ->
                BusinessReport' Core.<$>
                  (x Core..:? "DeliveryTime") Core.<*> x Core..:? "DownloadUrl"
