...
data DBInstance = DBInstance'
  { -- | Indicates whether engine-native audit fields are included in the
    ...
    -- each DB engine, see
    -- <https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_LogAccess.html Monitoring Amazon RDS log files>
    -- in the /Amazon RDS User Guide./
    ...
    -- see
    -- <https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Concepts.RDS_Fea_Regions_DB-eng.Feature.IamDatabaseAuthentication.html IAM database authentication>
    -- in the /Amazon RDS User Guide/ and
    -- <https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Concepts.Aurora_Fea_Regions_DB-eng.Feature.IAMdbauth.html IAM database authentication in Aurora>
    -- in the /Amazon Aurora User Guide/.
    ...
    -- replica.
    readReplicaSourceDBClusterIdentifier :: Prelude.Maybe Prelude.Text,
...
--
-- 'allocatedStorage', 'dbInstance_allocatedStorage' - The amount of storage in gibibytes (GiB) allocated for the DB instance.
--
-- 'associatedRoles', 'dbInstance_associatedRoles' - The Amazon Web Services Identity and Access Management (IAM) roles
-- associated with the DB instance.
--
-- 'autoMinorVersionUpgrade', 'dbInstance_autoMinorVersionUpgrade' - Indicates whether minor version patches are applied automatically.
--
...
-- the duration set by @--resume-full-automation-mode-minutes@.
--
-- 'availabilityZone', 'dbInstance_availabilityZone' - The name of the Availability Zone where the DB instance is located.
--
...
-- Services Backup.
--
-- 'backupRetentionPeriod', 'dbInstance_backupRetentionPeriod' - The number of days for which automatic DB snapshots are retained.
--
-- 'backupTarget', 'dbInstance_backupTarget' - The location where automated backups and manual snapshots are stored:
-- Amazon Web Services Outposts or the Amazon Web Services Region.
--
...
-- is associated with.
--
-- 'copyTagsToSnapshot', 'dbInstance_copyTagsToSnapshot' - Indicates whether tags are copied from the DB instance to snapshots of
-- the DB instance.
--
-- This setting doesn\'t apply to Amazon Aurora DB instances. Copying tags
-- to snapshots is managed by the DB cluster. Setting this value for an
...
-- in the /Amazon RDS User Guide/.
--
-- 'customerOwnedIpEnabled', 'dbInstance_customerOwnedIpEnabled' - Indicates whether a customer-owned IP address (CoIP) is enabled for an
-- RDS on Outposts DB instance.
--
-- A /CoIP/ provides local or external connectivity to resources in your
-- Outpost subnets through your on-premises network. For some use cases, a
...
-- in the /Amazon Web Services Outposts User Guide/.
--
-- 'dbClusterIdentifier', 'dbInstance_dbClusterIdentifier' - If the DB instance is a member of a DB cluster, indicates the name of
-- the DB cluster that the DB instance is a member of.
--
...
-- instance.
--
-- 'dbInstanceClass', 'dbInstance_dbInstanceClass' - The name of the compute and memory capacity class of the DB instance.
--
-- 'dbInstanceIdentifier', 'dbInstance_dbInstanceIdentifier' - The user-supplied database identifier. This identifier is the unique key
-- that identifies a DB instance.
--
-- 'dbInstanceStatus', 'dbInstance_dbInstanceStatus' - The current state of this database.
--
-- For information about DB instance statuses, see
-- <https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/accessing-monitoring.html#Overview.DBInstance.Status Viewing DB instance status>
-- in the /Amazon RDS User Guide./
--
-- 'dbName', 'dbInstance_dbName' - The meaning of this parameter differs depending on the database engine.
--
-- -   For RDS for MariaDB, Microsoft SQL Server, MySQL, and PostgreSQL -
--     The name of the initial database specified for this DB instance when
--     it was created, if one was provided. This same name is returned for
...
--     Oracle DB instance.
--
-- 'dbParameterGroups', 'dbInstance_dbParameterGroups' - The list of DB parameter groups applied to this DB instance.
--
-- 'dbSecurityGroups', 'dbInstance_dbSecurityGroups' - A list of DB security group elements containing @DBSecurityGroup.Name@
-- and @DBSecurityGroup.Status@ subelements.
--
-- 'dbSubnetGroup', 'dbInstance_dbSubnetGroup' - Information about the subnet group associated with the DB instance,
-- including the name, description, and subnets in the subnet group.
--
...
-- RDS Custom DB instances.
--
-- 'dbInstancePort', 'dbInstance_dbInstancePort' - The port that the DB instance listens on. If the DB instance is part of
-- a DB cluster, this can be a different port than the DB cluster port.
--
...
-- accessed.
--
-- 'deletionProtection', 'dbInstance_deletionProtection' - Indicates whether the DB instance has deletion protection enabled. The
-- database can\'t be deleted when deletion protection is enabled. For more
-- information, see
-- <https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_DeleteInstance.html Deleting a DB Instance>.
--
...
-- in the /Amazon RDS User Guide./
--
-- 'endpoint', 'dbInstance_endpoint' - The connection endpoint for the DB instance.
--
-- The endpoint might not be shown for instances with the status of
-- @creating@.
--
-- 'engine', 'dbInstance_engine' - The database engine used for this DB instance.
--
-- 'engineVersion', 'dbInstance_engineVersion' - The version of the database engine.
--
...
-- that receives the Enhanced Monitoring metrics data for the DB instance.
--
-- 'iAMDatabaseAuthenticationEnabled', 'dbInstance_iAMDatabaseAuthenticationEnabled' - Indicates whether mapping of Amazon Web Services Identity and Access
-- Management (IAM) accounts to database accounts is enabled for the DB
-- instance.
--
-- For a list of engine versions that support IAM database authentication,
...
-- in the /Amazon Aurora User Guide/.
--
-- 'instanceCreateTime', 'dbInstance_instanceCreateTime' - The date and time when the DB instance was created.
--
-- 'iops', 'dbInstance_iops' - The Provisioned IOPS (I\/O operations per second) value for the DB
-- instance.
--
-- 'kmsKeyId', 'dbInstance_kmsKeyId' - If @StorageEncrypted@ is enabled, the Amazon Web Services KMS key
-- identifier for the encrypted DB instance.
--
-- The Amazon Web Services KMS key identifier is the key ARN, key ID, alias
-- ARN, or alias name for the KMS key.
--
-- 'latestRestorableTime', 'dbInstance_latestRestorableTime' - The latest time to which a database in this DB instance can be restored
-- with point-in-time restore.
--
-- 'licenseModel', 'dbInstance_licenseModel' - The license model information for this DB instance. This setting
-- doesn\'t apply to RDS Custom DB instances.
--
-- 'listenerEndpoint', 'dbInstance_listenerEndpoint' - The listener connection endpoint for SQL Server Always On.
--
-- 'masterUserSecret', 'dbInstance_masterUserSecret' - The secret managed by RDS in Amazon Web Services Secrets Manager for the
-- master user password.
--
-- For more information, see
-- <https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/rds-secrets-manager.html Password management with Amazon Web Services Secrets Manager>
-- in the /Amazon RDS User Guide./
--
-- 'masterUsername', 'dbInstance_masterUsername' - The master username for the DB instance.
--
...
-- metrics to Amazon CloudWatch Logs.
--
-- 'multiAZ', 'dbInstance_multiAZ' - Indicates whether the DB instance is a Multi-AZ deployment. This setting
-- doesn\'t apply to RDS Custom DB instances.
--
...
-- Valid Values: @IPV4 | DUAL@
--
-- 'optionGroupMemberships', 'dbInstance_optionGroupMemberships' - The list of option group memberships for this DB instance.
--
-- 'pendingModifiedValues', 'dbInstance_pendingModifiedValues' - Information about pending changes to the DB instance. This information
-- is returned only when there are pending changes. Specific changes are
-- identified by subelements.
--
-- 'performanceInsightsEnabled', 'dbInstance_performanceInsightsEnabled' - Indicates whether Performance Insights is enabled for the DB instance.
--
...
-- ARN, or alias name for the KMS key.
--
-- 'performanceInsightsRetentionPeriod', 'dbInstance_performanceInsightsRetentionPeriod' - The number of days to retain Performance Insights data.
--
-- Valid Values:
--
-- -   @7@
...
-- Default: @7@ days
--
-- 'preferredBackupWindow', 'dbInstance_preferredBackupWindow' - The daily time range during which automated backups are created if
-- automated backups are enabled, as determined by the
-- @BackupRetentionPeriod@.
--
-- 'preferredMaintenanceWindow', 'dbInstance_preferredMaintenanceWindow' - The weekly time range during which system maintenance can occur, in
-- Universal Coordinated Time (UTC).
--
-- 'processorFeatures', 'dbInstance_processorFeatures' - The number of CPU cores and the number of threads per core for the DB
-- instance class of the DB instance.
--
-- 'promotionTier', 'dbInstance_promotionTier' - The order of priority in which an Aurora Replica is promoted to the
-- primary instance after a failure of the existing primary instance. For
-- more information, see
-- <https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Concepts.AuroraHighAvailability.html#Aurora.Managing.FaultTolerance Fault Tolerance for an Aurora DB Cluster>
-- in the /Amazon Aurora User Guide/.
--
-- 'publiclyAccessible', 'dbInstance_publiclyAccessible' - Indicates whether the DB instance is publicly accessible.
--
-- When the DB cluster is publicly accessible, its Domain Name System (DNS)
-- endpoint resolves to the private IP address from within the DB
-- cluster\'s virtual private cloud (VPC). It resolves to the public IP
...
-- For more information, see CreateDBInstance.
--
-- 'readReplicaDBClusterIdentifiers', 'dbInstance_readReplicaDBClusterIdentifiers' - The identifiers of Aurora DB clusters to which the RDS DB instance is
-- replicated as a read replica. For example, when you create an Aurora
-- read replica of an RDS for MySQL DB instance, the Aurora MySQL DB
-- cluster for the Aurora read replica is shown. This output doesn\'t
-- contain information about cross-Region Aurora read replicas.
...
-- Currently, each RDS DB instance can have only one Aurora read replica.
--
-- 'readReplicaDBInstanceIdentifiers', 'dbInstance_readReplicaDBInstanceIdentifiers' - The identifiers of the read replicas associated with this DB instance.
--
-- 'readReplicaSourceDBClusterIdentifier', 'dbInstance_readReplicaSourceDBClusterIdentifier' - The identifier of the source DB cluster if this DB instance is a read
-- replica.
--
-- 'readReplicaSourceDBInstanceIdentifier', 'dbInstance_readReplicaSourceDBInstanceIdentifier' - The identifier of the source DB instance if this DB instance is a read
-- replica.
--
...
-- DB instance with multi-AZ support.
--
-- 'statusInfos', 'dbInstance_statusInfos' - The status of a read replica. If the DB instance isn\'t a read replica,
-- the value is blank.
--
-- 'storageEncrypted', 'dbInstance_storageEncrypted' - Indicates whether the DB instance is encrypted.
--
-- 'storageThroughput', 'dbInstance_storageThroughput' - The storage throughput for the DB instance.
--
-- This setting applies only to the @gp3@ storage type.
--
-- 'storageType', 'dbInstance_storageType' - The storage type associated with the DB instance.
--
...
-- instances that were created with a time zone specified.
--
-- 'vpcSecurityGroups', 'dbInstance_vpcSecurityGroups' - The list of Amazon EC2 VPC security groups that the DB instance belongs
-- to.
newDBInstance ::
  DBInstance
newDBInstance =
...
-- | The identifier of the source DB cluster if this DB instance is a read
-- replica.
dbInstance_readReplicaSourceDBClusterIdentifier :: Lens.Lens' DBInstance (Prelude.Maybe Prelude.Text)
dbInstance_readReplicaSourceDBClusterIdentifier = Lens.lens (\DBInstance' {readReplicaSourceDBClusterIdentifier} -> readReplicaSourceDBClusterIdentifier) (\s@DBInstance' {} a -> s {readReplicaSourceDBClusterIdentifier = a} :: DBInstance)
...
instance Data.FromXML DBInstance where
  parseXML x =
    DBInstance'
      ...
      Prelude.<*> (x Data..@? "ReadReplicaSourceDBClusterIdentifier")
...
instance Prelude.Hashable DBInstance where
  hashWithSalt _salt DBInstance' {..} =
    _salt
      ...
      `Prelude.hashWithSalt` readReplicaSourceDBClusterIdentifier
