...
makeSiglessLenses ''DescribeInstances
...
makeSiglessLenses ''DescribeInstancesResponse
...
-- mapped to the instance. block-device-mapping.delete-on-termination - A
-- Boolean that indicates whether the Amazon EBS volume is deleted on instance
-- termination. block-device-mapping.device-name - The device name for the
-- Amazon EBS volume (for example, /dev/sdh). block-device-mapping.status -
-- The status for the Amazon EBS volume (attaching | attached | detaching |
-- detached). block-device-mapping.volume-id - The volume ID of the Amazon EBS
-- volume. client-token - The idempotency token you provided when you launched
-- the instance. dns-name - The public DNS name of the instance. group-id -
...
-- nondefault VPC, you must use group-id instead. hypervisor - The hypervisor
-- type of the instance (ovm | xen). iam-instance-profile.arn - The instance
-- profile associated with the instance. Specified as an ARN. image-id - The
-- ID of the image used to launch the instance. instance-id - The ID of the
-- instance. instance-lifecycle - Indicates whether this is a Spot Instance
-- (spot). instance-state-code - The state of the instance, as a 16-bit
...
-- 64 (stopping), and 80 (stopped). instance-state-name - The state of the
-- instance (pending | running | shutting-down | terminated | stopping |
-- stopped). instance-type - The type of instance (for example, m1.small).
-- instance.group-id - The ID of the security group for the instance. If the
-- instance is in EC2-Classic or a default VPC, you can use
-- instance.group-name instead. instance.group-name - The name of the security
-- group for the instance. If the instance is in a nondefault VPC, you must
-- use instance.group-id instead. ip-address - The public IP address of the
-- instance. kernel-id - The kernel ID. key-name - The name of the key pair
-- used when the instance was launched. launch-index - When launching multiple
-- instances, this is the index for the instance in the launch group (for
-- example, 0, 1, 2, and so on). launch-time - The time when the instance was
-- launched. monitoring-state - Indicates whether monitoring is enabled for
-- the instance (disabled | enabled). owner-id - The AWS account ID of the
-- instance owner. placement-group-name - The name of the placement group for
-- the instance. platform - The platform. Use windows if you have Windows
-- instances; otherwise, leave blank. private-dns-name - The private DNS name
-- of the instance. private-ip-address - The private IP address of the
-- instance. product-code - The product code associated with the AMI used to
-- launch the instance. product-code.type - The type of product code (devpay |
-- marketplace). ramdisk-id - The RAM disk ID. reason - The reason for the
-- current state of the instance (for example, shows "User Initiated [date]"
-- when you stop or terminate the instance). Similar to the state-reason-code
-- filter. requester-id - The ID of the entity that launched the instance on
...
-- launch request. For example, if you launch one instance, you'll get one
-- reservation ID. If you launch ten instances using the same launch request,
-- you'll also get one reservation ID. root-device-name - The name of the root
-- device for the instance (for example, /dev/sda1). root-device-type - The
-- type of root device that the instance uses (ebs | instance-store).
...
-- the instance to perform network address translation (NAT) in your VPC.
-- spot-instance-request-id - The ID of the Spot Instance request.
-- state-reason-code - The reason code for the state change.
-- state-reason-message - A message that describes the state change. subnet-id
-- - The ID of the subnet for the instance. tag:key=value - The key/value
-- combination of a tag assigned to the resource, where tag:key is the tag's
-- key. tag-key - The key of a tag assigned to the resource. This filter is
...
-- filter is independent of the tag-key filter. tenancy - The tenancy of an
-- instance (dedicated | default). virtualization-type - The virtualization
-- type of the instance (paravirtual | hvm). vpc-id - The ID of the VPC that
-- the instance is running in. network-interface.description - The description
-- of the network interface. network-interface.subnet-id - The ID of the
-- subnet for the network interface. network-interface.vpc-id - The ID of the
-- VPC for the network interface. network-interface.network-interface.id - The
-- ID of the network interface. network-interface.owner-id - The ID of the
-- owner of the network interface. network-interface.availability-zone - The
-- Availability Zone for the network interface. network-interface.requester-id
-- - The requester ID for the network interface.
-- network-interface.requester-managed - Indicates whether the network
-- interface is being managed by AWS. network-interface.status - The status of
-- the network interface (available) | in-use). network-interface.mac-address
-- - The MAC address of the network interface.
-- network-interface-private-dns-name - The private DNS name of the network
-- interface. network-interface.source-destination-check - Whether the network
...
-- (NAT) in your VPC. network-interface.group-id - The ID of a security group
...
-- address associated with the network interface. association.allocation-id -
