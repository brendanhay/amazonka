...
instance AWSRequest DeleteThing where
  ...
  response =
    receiveEmpty
      (\s h x -> DeleteThingResponse' <$> (pure (fromEnum s)))
...
instance ToPath DeleteThing where
  toPath DeleteThing' {..} = mconcat ["/things/", toBS _dtThingName]
...
deleteThingResponse pResponseStatus_ =
  DeleteThingResponse' {_delersResponseStatus = pResponseStatus_}

-- | -- | The response status code.
delersResponseStatus :: Lens' DeleteThingResponse Int
