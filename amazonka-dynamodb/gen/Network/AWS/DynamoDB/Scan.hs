...
-- or Binary. No set data types are allowed. In a parallel scan, a /Scan/
-- request that includes /ExclusiveStartKey/ must specify the same segment
-- whose previous /Scan/ returned the corresponding value of
-- /LastEvaluatedKey/.
...
-- than one condition in the /ScanFilter/ map, then by default all of the
...
-- /ScanFilter/ element consists of an attribute name to compare, along with
...
-- to be scanned by an application worker. Segment IDs are zero-based, so
-- the first segment is always 0. For example, if you want to scan a table
-- using four application threads, the first thread specifies a /Segment/
...
-- operation. The value for /Segment/ must be greater than or equal to 0,
-- and less than the value provided for /TotalSegments/. If you specify
...
-- that will perform the parallel scan. For example, if you want to scan a
-- table using four application threads, specify a /TotalSegments/ value of
-- 4. The value for /TotalSegments/ must be greater than or equal to 1, and
-- less than or equal to 1000000. If you specify a /TotalSegments/ value of
-- 1, the /Scan/ operation will be sequential rather than parallel. If you
