...
--
-- * 'luMarker'
data ListUsers = ListUsers'{_luPathPrefix :: Text, _luMaxItems :: Nat, _luMarker :: Text} deriving (Eq, Read, Show)

-- | 'ListUsers' smart constructor.
listUsers :: Text -> Natural -> Text -> ListUsers
listUsers pPathPrefix pMaxItems pMarker = ListUsers'{_luPathPrefix = pPathPrefix, _luMaxItems = _Nat # pMaxItems, _luMarker = pMarker};
...
luPathPrefix :: Lens' ListUsers Text
...
-- it defaults to 100.
luMaxItems :: Lens' ListUsers Natural
luMaxItems = lens _luMaxItems (\ s a -> s{_luMaxItems = a}) . _Nat;

-- | Use this parameter only when paginating results, and only in a
-- subsequent request after you\'ve received a response where the results
-- are truncated. Set it to the value of the @Marker@ element in the
-- response you just received.
luMarker :: Lens' ListUsers Text
...
--
-- * 'lurMarker'
data ListUsersResponse = ListUsersResponse'{_lurIsTruncated :: Maybe Bool, _lurUsers :: [User], _lurMarker :: Text} deriving (Eq, Read, Show)

-- | 'ListUsersResponse' smart constructor.
listUsersResponse :: [User] -> Text -> ListUsersResponse
listUsersResponse pUsers pMarker = ListUsersResponse'{_lurIsTruncated = Nothing, _lurUsers = pUsers, _lurMarker = pMarker};
...
-- request.
lurMarker :: Lens' ListUsersResponse Text
