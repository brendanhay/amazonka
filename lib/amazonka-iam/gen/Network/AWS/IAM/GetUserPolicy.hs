...
instance Core.ToQuery GetUserPolicy where
        toQuery GetUserPolicy{..}
          = Core.toQueryPair "Action" ("GetUserPolicy" :: Core.Text) Core.<>
...
instance Core.AWSRequest GetUserPolicy where
        ...
        {-# INLINE toRequest #-}
        parseResponse
          = Response.receiveXMLWrapper "GetUserPolicyResult"
              (\ s h x ->
                 GetUserPolicyResponse' Core.<$>
                   (x Core..@ "UserName") Core.<*> x Core..@ "PolicyName" Core.<*>
...
mkGetUserPolicyResponse
    ...
    -> GetUserPolicyResponse
mkGetUserPolicyResponse userName policyName policyDocument
  responseStatus
...
{-# INLINEABLE guprrsPolicyDocument #-}
