...
instance AWSPager DescribeClusterSnapshots where
  page rq rs
    ...
    | otherwise = Just $ rq & dMarker .~ rs ^. dcssrsMarker
...
instance ToQuery DescribeClusterSnapshots where
  toQuery DescribeClusterSnapshots' {..} =
    mconcat
      [ "Action" =: ("DescribeClusterSnapshots" :: ByteString),
        "Version" =: ("2012-12-01" :: ByteString),
        "SnapshotIdentifier" =: _dSnapshotIdentifier,
        "TagValues" =: toQuery (toQueryList "TagValue" <$> _dTagValues),
        "ClusterExists" =: _dClusterExists,
        "StartTime" =: _dStartTime,
        "TagKeys" =: toQuery (toQueryList "TagKey" <$> _dTagKeys),
        "ClusterIdentifier" =: _dClusterIdentifier,
        "SnapshotType" =: _dSnapshotType,
        "SortingEntities"
          =: toQuery
            (toQueryList "SnapshotSortingEntity" <$> _dSortingEntities),
        "Marker" =: _dMarker,
