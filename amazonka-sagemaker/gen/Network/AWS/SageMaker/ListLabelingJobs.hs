...
instance AWSPager ListLabelingJobs where
        page rq rs
          ...
          | otherwise =
            Just $ rq & lljNextToken .~ rs ^. lljrsNextToken
...
instance Hashable ListLabelingJobs where

instance NFData ListLabelingJobs where
...
data ListLabelingJobsResponse = ListLabelingJobsResponse'
  { _lljrsLabelingJobSummaryList :: !(Maybe [LabelingJobSummary])
...
instance NFData ListLabelingJobsResponse where
