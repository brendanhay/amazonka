...
data DescribeMaintenanceWindowSchedule = DescribeMaintenanceWindowSchedule'
  { _dmwsResourceType :: !(Maybe MaintenanceWindowResourceType)
  , _dmwsFilters      :: !(Maybe [PatchOrchestratorFilter])
  , _dmwsNextToken    :: !(Maybe Text)
  , _dmwsTargets      :: !(Maybe [Target])
  , _dmwsMaxResults   :: !(Maybe Nat)
  , _dmwsWindowId     :: !(Maybe Text)
...
instance AWSPager DescribeMaintenanceWindowSchedule
         where
        page rq rs
          | stop (rs ^. dmwsrsNextToken) = Nothing
          | stop (rs ^. dmwsrsScheduledWindowExecutions) =
            Nothing
          | otherwise =
            Just $ rq & dmwsNextToken .~ rs ^. dmwsrsNextToken

instance AWSRequest DescribeMaintenanceWindowSchedule
         where
...
instance ToHeaders DescribeMaintenanceWindowSchedule
         where
        toHeaders
          = const
              (mconcat
...
instance ToJSON DescribeMaintenanceWindowSchedule
         where
...
instance ToPath DescribeMaintenanceWindowSchedule
         where
        toPath = const "/"

instance ToQuery DescribeMaintenanceWindowSchedule
         where
...
data DescribeMaintenanceWindowScheduleResponse = DescribeMaintenanceWindowScheduleResponse'
  { _dmwsrsScheduledWindowExecutions :: !(Maybe [ScheduledWindowExecution])
...
describeMaintenanceWindowScheduleResponse pResponseStatus_ =
  DescribeMaintenanceWindowScheduleResponse'
    { _dmwsrsScheduledWindowExecutions = Nothing
