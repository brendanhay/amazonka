...
mkCreateStack stackName
  = CreateStack'{stackName, capabilities = Core.Nothing,
                 clientRequestToken = Core.Nothing, disableRollback = Core.Nothing,
                 enableTerminationProtection = Core.Nothing,
                 notificationARNs = Core.Nothing, onFailure = Core.Nothing,
                 parameters = Core.Nothing, resourceTypes = Core.Nothing,
                 roleARN = Core.Nothing, rollbackConfiguration = Core.Nothing,
                 stackPolicyBody = Core.Nothing, stackPolicyURL = Core.Nothing,
                 tags = Core.Nothing, templateBody = Core.Nothing,
                 templateURL = Core.Nothing, timeoutInMinutes = Core.Nothing}
...
{-# INLINEABLE csClientRequestToken #-}
...
{-# INLINEABLE csDisableRollback #-}
{-# DEPRECATED disableRollback "Use generic-lens or generic-optics with 'disableRollback' instead"  #-}
...
{-# INLINEABLE csEnableTerminationProtection #-}
...
{-# INLINEABLE csRollbackConfiguration #-}
...
instance Core.ToQuery CreateStack where
        toQuery CreateStack{..}
          = Core.toQueryPair "Action" ("CreateStack" :: Core.Text) Core.<>
              ...
              Core.maybe Core.mempty (Core.toQueryPair "ClientRequestToken")
                clientRequestToken
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "DisableRollback")
                disableRollback
              Core.<>
              ...
              Core.toQueryPair "NotificationARNs"
                (Core.maybe Core.mempty (Core.toQueryList "member")
                   notificationARNs)
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "OnFailure") onFailure
              Core.<>
...
instance Core.AWSRequest CreateStack where
        ...
        {-# INLINE toRequest #-}
        parseResponse
          = Response.receiveXMLWrapper "CreateStackResult"
              (\ s h x ->
