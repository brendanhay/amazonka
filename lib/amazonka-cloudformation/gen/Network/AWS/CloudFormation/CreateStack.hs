...
instance AWSRequest CreateStack where
  ...
  response =
    receiveXMLWrapper
      "CreateStackResult"
      ( \s h x ->
          CreateStackResponse' <$> (x .@? "StackId") <*> (pure (fromEnum s))
      )
  ...
  toQuery CreateStack' {..} =
    mconcat
      [ "Action" =: ("CreateStack" :: ByteString),
        ...
        "NotificationARNs"
          =: toQuery (toQueryList "member" <$> _csNotificationARNs),
        "EnableTerminationProtection" =: _csEnableTerminationProtection,
        "StackPolicyBody" =: _csStackPolicyBody,
        "Parameters" =: toQuery (toQueryList "member" <$> _csParameters),
