...
instance AWSPager DescribeInstanceAssociationsStatus where
  page rq rs
    ...
    | stop (rs ^. diasrsInstanceAssociationStatusInfos) = Nothing
    | otherwise = Just $ rq & diasNextToken .~ rs ^. diasrsNextToken

instance AWSRequest DescribeInstanceAssociationsStatus where
  type
...
instance ToHeaders DescribeInstanceAssociationsStatus where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AmazonSSM.DescribeInstanceAssociationsStatus" :: ByteString),
...
instance NFData DescribeInstanceAssociationsStatusResponse
