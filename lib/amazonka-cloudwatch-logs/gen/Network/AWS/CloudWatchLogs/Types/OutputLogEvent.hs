...
mkOutputLogEvent
    :: OutputLogEvent
mkOutputLogEvent
  = OutputLogEvent'{ingestionTime = Core.Nothing,
...
instance Core.FromJSON OutputLogEvent where
        parseJSON
          = Core.withObject "OutputLogEvent" Core.$
              \ x ->
                OutputLogEvent' Core.<$>
                  (x Core..:? "ingestionTime") Core.<*> x Core..:? "message" Core.<*>
                    x Core..:? "timestamp"
