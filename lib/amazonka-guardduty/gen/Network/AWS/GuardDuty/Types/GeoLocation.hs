...
{-# INLINEABLE glLat #-}
{-# DEPRECATED lat "Use generic-lens or generic-optics with 'lat' instead"  #-}
...
{-# INLINEABLE glLon #-}
{-# DEPRECATED lon "Use generic-lens or generic-optics with 'lon' instead"  #-}

instance Core.FromJSON GeoLocation where
        parseJSON
          = Core.withObject "GeoLocation" Core.$
              \ x ->
                GeoLocation' Core.<$> (x Core..:? "lat") Core.<*> x Core..:? "lon"
