...
mkTrace
    :: Trace
mkTrace
  = Trace'{duration = Core.Nothing, id = Core.Nothing,
           limitExceeded = Core.Nothing, segments = Core.Nothing}
...
{-# INLINEABLE tLimitExceeded #-}
{-# DEPRECATED limitExceeded "Use generic-lens or generic-optics with 'limitExceeded' instead"  #-}
...
instance Core.FromJSON Trace where
        parseJSON
          = Core.withObject "Trace" Core.$
              \ x ->
                Trace' Core.<$>
                  (x Core..:? "Duration") Core.<*> x Core..:? "Id" Core.<*>
                    x Core..:? "LimitExceeded"
