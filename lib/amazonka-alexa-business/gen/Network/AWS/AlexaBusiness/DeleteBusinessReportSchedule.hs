...
instance AWSRequest DeleteBusinessReportSchedule where
  ...
  response =
    receiveEmpty
      ( \s h x ->
          DeleteBusinessReportScheduleResponse' <$> (pure (fromEnum s))
      )
...
instance ToHeaders DeleteBusinessReportSchedule where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AlexaForBusiness.DeleteBusinessReportSchedule" :: ByteString),
...
instance ToJSON DeleteBusinessReportSchedule where
  toJSON DeleteBusinessReportSchedule' {..} =
    object (catMaybes [Just ("ScheduleArn" .= _dbrsScheduleARN)])
