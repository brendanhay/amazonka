...
mkExecutionParameter
    :: ExecutionParameter
mkExecutionParameter
  = ExecutionParameter'{defaultValues = Core.Nothing,
...
{-# INLINEABLE epDefaultValues #-}
{-# DEPRECATED defaultValues "Use generic-lens or generic-optics with 'defaultValues' instead"  #-}
...
instance Core.FromJSON ExecutionParameter where
        parseJSON
          = Core.withObject "ExecutionParameter" Core.$
              \ x ->
                ExecutionParameter' Core.<$>
                  (x Core..:? "DefaultValues") Core.<*> x Core..:? "Name" Core.<*>
                    x Core..:? "Type"
