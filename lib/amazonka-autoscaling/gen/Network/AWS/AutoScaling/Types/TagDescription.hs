...
mkTagDescription
    ...
    -> Types.Value -- ^ 'value'
    -> TagDescription
mkTagDescription key propagateAtLaunch resourceId resourceType
  value
  = TagDescription'{key, propagateAtLaunch, resourceId, resourceType,
                    value}
...
{-# INLINEABLE tdPropagateAtLaunch #-}
...
instance Core.FromXML TagDescription where
        parseXML x
          = TagDescription' Core.<$>
              (x Core..@ "Key") Core.<*> x Core..@ "PropagateAtLaunch" Core.<*>
                x Core..@ "ResourceId"
