...
data DescribeTrails = DescribeTrails'
  { -- | Specifies whether to include shadow trails in the response. A shadow trail is the replication in a region of a trail that was created in a different region, or in the case of an organization trail, the replication of an organization trail in member accounts. If you do not include shadow trails, organization trails in a member account and region replication trails will not be returned. The default is true.
    ...
    trailNameList :: Core.Maybe [Types.String]
  }
...
dtTrailNameList :: Lens.Lens' DescribeTrails (Core.Maybe [Types.String])
...
instance Core.AWSRequest DescribeTrails where
  ...
  response =
    Response.receiveJSON
      ( \s h x ->
          DescribeTrailsResponse'
            Core.<$> (x Core..:? "trailList") Core.<*> (Core.pure (Core.fromEnum s))
      )
...
{-# DEPRECATED dtrrsTrailList "Use generic-lens or generic-optics with 'trailList' instead." #-}
