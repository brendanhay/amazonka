...
mkRemediationException
    ...
    -> Types.StringWithCharLimit1024 -- ^ 'resourceId'
    -> RemediationException
mkRemediationException configRuleName resourceType resourceId
  = RemediationException'{configRuleName, resourceType, resourceId,
...
instance Core.FromJSON RemediationException where
        parseJSON
          = Core.withObject "RemediationException" Core.$
              \ x ->
                RemediationException' Core.<$>
                  (x Core..: "ConfigRuleName") Core.<*> x Core..: "ResourceType"
