...
mkPutScalingPolicy name fleetId metricName
  = PutScalingPolicy'{name, fleetId, metricName,
                      ...
                      evaluationPeriods = Core.Nothing, policyType = Core.Nothing,
...
{-# INLINEABLE pspComparisonOperator #-}
...
{-# INLINEABLE pspEvaluationPeriods #-}
...
{-# INLINEABLE pspScalingAdjustmentType #-}
...
{-# INLINEABLE pspTargetConfiguration #-}
...
instance Core.ToHeaders PutScalingPolicy where
        toHeaders PutScalingPolicy{..}
          = Core.pure ("X-Amz-Target", "GameLift.PutScalingPolicy") Core.<>
