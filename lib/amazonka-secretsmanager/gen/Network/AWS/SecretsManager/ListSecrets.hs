...
mkListSecrets
    :: ListSecrets
mkListSecrets
  = ListSecrets'{filters = Core.Nothing, maxResults = Core.Nothing,
...
instance Core.ToHeaders ListSecrets where
        toHeaders ListSecrets{..}
          = Core.pure ("X-Amz-Target", "secretsmanager.ListSecrets") Core.<>
          ...
          = Response.receiveJSON
              (\ s h x ->
                 ListSecretsResponse' Core.<$>
                   (x Core..:? "NextToken") Core.<*> x Core..:? "SecretList" Core.<*>
...
mkListSecretsResponse responseStatus
  = ListSecretsResponse'{nextToken = Core.Nothing,
                         secretList = Core.Nothing, responseStatus}
...
{-# INLINEABLE lsrrsSecretList #-}
{-# DEPRECATED secretList "Use generic-lens or generic-optics with 'secretList' instead"  #-}
