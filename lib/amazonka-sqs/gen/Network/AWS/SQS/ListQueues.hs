...
mkListQueues
    :: ListQueues
mkListQueues
  = ListQueues'{maxResults = Core.Nothing, nextToken = Core.Nothing,
...
lqQueueNamePrefix :: Lens.Lens' ListQueues (Core.Maybe Core.Text)
lqQueueNamePrefix = Lens.field @"queueNamePrefix"
{-# INLINEABLE lqQueueNamePrefix #-}
{-# DEPRECATED queueNamePrefix "Use generic-lens or generic-optics with 'queueNamePrefix' instead"  #-}

instance Core.ToQuery ListQueues where
        toQuery ListQueues{..}
          = Core.toQueryPair "Action" ("ListQueues" :: Core.Text) Core.<>
              ...
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "QueueNamePrefix")
                queueNamePrefix
...
instance Core.AWSRequest ListQueues where
        ...
        {-# INLINE toRequest #-}
        parseResponse
          = Response.receiveXMLWrapper "ListQueuesResult"
              (\ s h x ->
                 ListQueuesResponse' Core.<$>
                   (x Core..@? "NextToken") Core.<*> x Core..@? "QueueUrl" Core.<*>
...
mkListQueuesResponse responseStatus
  = ListQueuesResponse'{nextToken = Core.Nothing,
                        queueUrls = Core.Nothing, responseStatus}
...
lqrrsQueueUrls :: Lens.Lens' ListQueuesResponse (Core.Maybe [Core.Text])
lqrrsQueueUrls = Lens.field @"queueUrls"
{-# INLINEABLE lqrrsQueueUrls #-}
