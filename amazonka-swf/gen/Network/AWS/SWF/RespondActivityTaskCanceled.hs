...
-- optionally provided using the @details@ argument. These @details@ (if
-- provided) appear in the @ActivityTaskCanceled@ event added to the workflow
-- history. Only use this operation if the @canceled@ flag of a
-- 'RecordActivityTaskHeartbeat' request returns @true@ and if the activity
-- can be safely undone or abandoned. A task is considered open from the time
...
-- timed out>. Access Control You can use IAM policies to control this
...
-- the service and should be treated as an opaque value. If the task is
-- passed to another process, its @taskToken@ must also be passed. This
