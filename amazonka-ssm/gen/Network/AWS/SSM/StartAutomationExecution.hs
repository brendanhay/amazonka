...
data StartAutomationExecution = StartAutomationExecution'
  ...
  , _saeTargetLocations     :: !(Maybe (List1 TargetLocation))
  , _saeClientToken         :: !(Maybe Text)
  , _saeMode                :: !(Maybe ExecutionMode)
  , _saeTargetMaps          :: !(Maybe [Map Text [Text]])
...
--
-- * 'saeTargetLocations' - A location is a combination of AWS Regions and/or AWS accounts where you want to execute the Automation. Use this action to start an Automation in multiple Regions and multiple accounts. For more information, see <http://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-automation-multiple-accounts-and-regions.html Concurrently Executing Automations in Multiple AWS Regions and Accounts> in the /AWS Systems Manager User Guide/ .
--
