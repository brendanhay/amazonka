...
mkResourcePolicy
    :: ResourcePolicy
mkResourcePolicy
  = ResourcePolicy'{lastUpdatedTime = Core.Nothing,
...
instance Core.FromJSON ResourcePolicy where
        parseJSON
          = Core.withObject "ResourcePolicy" Core.$
              \ x ->
                ResourcePolicy' Core.<$>
                  (x Core..:? "lastUpdatedTime") Core.<*> x Core..:? "policyDocument"
