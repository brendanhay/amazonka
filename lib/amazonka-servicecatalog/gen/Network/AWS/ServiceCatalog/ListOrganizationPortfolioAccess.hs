...
instance AWSPager ListOrganizationPortfolioAccess where
  page rq rs
    ...
    | otherwise =
      Just $ rq & lopaPageToken .~ rs ^. loparsNextPageToken
    ...
    object
      ( catMaybes
          [ ("AcceptLanguage" .=) <$> _lopaAcceptLanguage,
            ...
            Just ("OrganizationNodeType" .= _lopaOrganizationNodeType)
          ]
      )
...
data ListOrganizationPortfolioAccessResponse = ListOrganizationPortfolioAccessResponse'
  { _loparsNextPageToken ::
      !( Maybe
           Text
       ),
    _loparsOrganizationNodes ::
      !( Maybe
           [OrganizationNode]
       ),
    _loparsResponseStatus ::
      !Int
  }
  deriving (Eq, Read, Show, Data, Typeable, Generic)
...
instance NFData ListOrganizationPortfolioAccessResponse
