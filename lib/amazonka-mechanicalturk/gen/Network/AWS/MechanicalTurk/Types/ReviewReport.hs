...
mkReviewReport
    :: ReviewReport
mkReviewReport
  = ReviewReport'{reviewActions = Core.Nothing,
...
{-# INLINEABLE rrReviewActions #-}
{-# DEPRECATED reviewActions "Use generic-lens or generic-optics with 'reviewActions' instead"  #-}
...
{-# INLINEABLE rrReviewResults #-}
{-# DEPRECATED reviewResults "Use generic-lens or generic-optics with 'reviewResults' instead"  #-}
...
instance Core.FromJSON ReviewReport where
        parseJSON
          = Core.withObject "ReviewReport" Core.$
              \ x ->
                ReviewReport' Core.<$>
                  (x Core..:? "ReviewActions") Core.<*> x Core..:? "ReviewResults"
