...
data MatchmakingConfiguration = MatchmakingConfiguration'
  { -- | Information to attach to all events related to the matchmaking
    ...
    -- configuration uses.
    ruleSetArn :: Core.Maybe Core.Text,
...
-- | Information to attach to all events related to the matchmaking
-- configuration.
matchmakingConfiguration_customEventData :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Text)
...
-- for a successful match. This parameter is not used when @FlexMatchMode@
-- is set to @STANDALONE@.
matchmakingConfiguration_gameProperties :: Lens.Lens' MatchmakingConfiguration (Core.Maybe [GameProperty])
matchmakingConfiguration_gameProperties = Lens.lens (\MatchmakingConfiguration' {gameProperties} -> gameProperties) (\s@MatchmakingConfiguration' {} a -> s {gameProperties = a} :: MatchmakingConfiguration) Core.. Lens.mapping Lens._Coerce
...
-- -   __WITH_QUEUE__ - FlexMatch forms matches and uses the specified
--     GameLift queue to start a game session for the match.
matchmakingConfiguration_flexMatchMode :: Lens.Lens' MatchmakingConfiguration (Core.Maybe FlexMatchMode)
...
-- Automatic backfill is not available when @FlexMatchMode@ is set to
-- @STANDALONE@.
matchmakingConfiguration_backfillMode :: Lens.Lens' MatchmakingConfiguration (Core.Maybe BackfillMode)
...
-- is a number expressed in Unix time as milliseconds (for example
-- \"1469498468.057\").
matchmakingConfiguration_creationTime :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.UTCTime)
matchmakingConfiguration_creationTime = Lens.lens (\MatchmakingConfiguration' {creationTime} -> creationTime) (\s@MatchmakingConfiguration' {} a -> s {creationTime = a} :: MatchmakingConfiguration) Core.. Lens.mapping Core._Time
...
-- only 10 players are initially selected for the match. This parameter is
-- not used when @FlexMatchMode@ is set to @STANDALONE@.
matchmakingConfiguration_additionalPlayerCount :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Natural)
...
-- fails to accept before the timeout, the tickets are returned to the
-- ticket pool and continue to be evaluated for an acceptable match.
matchmakingConfiguration_acceptanceTimeoutSeconds :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Natural)
...
-- for a successful match. This parameter is not used when @FlexMatchMode@
-- is set to @STANDALONE@.
matchmakingConfiguration_gameSessionData :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Text)
...
-- uniquely identifies it. ARNs are unique across all Regions. In a
-- GameLift configuration ARN, the resource ID matches the /Name/ value.
matchmakingConfiguration_configurationArn :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Text)
...
-- configuration. Thais property is not set when @FlexMatchMode@ is set to
-- @STANDALONE@.
matchmakingConfiguration_gameSessionQueueArns :: Lens.Lens' MatchmakingConfiguration (Core.Maybe [Core.Text])
matchmakingConfiguration_gameSessionQueueArns = Lens.lens (\MatchmakingConfiguration' {gameSessionQueueArns} -> gameSessionQueueArns) (\s@MatchmakingConfiguration' {} a -> s {gameSessionQueueArns = a} :: MatchmakingConfiguration) Core.. Lens.mapping Lens._Coerce

-- | A unique identifier for a matchmaking configuration. This name is used
-- to identify the configuration associated with a matchmaking request or
-- ticket.
matchmakingConfiguration_name :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Text)
...
-- | An SNS topic ARN that is set up to receive matchmaking notifications.
matchmakingConfiguration_notificationTarget :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Text)
...
-- in process before timing out. Requests that fail due to timing out can
-- be resubmitted as needed.
matchmakingConfiguration_requestTimeoutSeconds :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Natural)
...
-- associated with the GameLift matchmaking rule set resource that this
-- configuration uses.
matchmakingConfiguration_ruleSetArn :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Text)
...
-- | A descriptive label that is associated with matchmaking configuration.
matchmakingConfiguration_description :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Text)
...
-- configuration. A matchmaking configuration can only use rule sets that
-- are defined in the same Region.
matchmakingConfiguration_ruleSetName :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Text)
...
-- tickets use the status @REQUIRES_ACCEPTANCE@ to indicate when a
-- completed potential match is waiting for player acceptance.
matchmakingConfiguration_acceptanceRequired :: Lens.Lens' MatchmakingConfiguration (Core.Maybe Core.Bool)
    ...
    Core.withObject
      ...
      ( \x ->
          MatchmakingConfiguration'
            ...
            Core.<*> (x Core..:? "AcceptanceTimeoutSeconds")
...
instance Core.Hashable MatchmakingConfiguration

instance Core.NFData MatchmakingConfiguration
