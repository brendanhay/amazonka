...
mkS3DestinationSettings
    :: S3DestinationSettings
mkS3DestinationSettings
  = S3DestinationSettings'{accessControl = Core.Nothing,
...
{-# INLINEABLE sdsAccessControl #-}
{-# DEPRECATED accessControl "Use generic-lens or generic-optics with 'accessControl' instead"  #-}
...
instance Core.FromJSON S3DestinationSettings where
        parseJSON
          = Core.withObject "S3DestinationSettings" Core.$
              \ x ->
                S3DestinationSettings' Core.<$>
                  (x Core..:? "accessControl") Core.<*> x Core..:? "encryption"
