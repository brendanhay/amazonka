...
instance Lude.ToHeaders ListTagsForResource where
  toHeaders =
    Lude.const
      ( Lude.mconcat
          [ "X-Amz-Target"
              Lude.=# ("SimpleWorkflowService.ListTagsForResource" :: Lude.ByteString),
            "Content-Type"
...
-- * 'tags' - An array of tags associated with the domain.
mkListTagsForResourceResponse ::
  -- | 'responseStatus'
  Lude.Int ->
  ListTagsForResourceResponse
...
ltfrrsTags :: Lens.Lens' ListTagsForResourceResponse (Lude.Maybe [ResourceTag])
ltfrrsTags = Lens.lens (tags :: ListTagsForResourceResponse -> Lude.Maybe [ResourceTag]) (\s a -> s {tags = a} :: ListTagsForResourceResponse)
