...
mkCodeHook
    :: Types.LambdaARN -- ^ 'uri'
    -> Types.MessageVersion -- ^ 'messageVersion'
    -> CodeHook
...
{-# INLINEABLE chMessageVersion #-}
{-# DEPRECATED messageVersion "Use generic-lens or generic-optics with 'messageVersion' instead"  #-}
...
instance Core.FromJSON CodeHook where
        parseJSON
          = Core.withObject "CodeHook" Core.$
              \ x ->
                CodeHook' Core.<$>
                  (x Core..: "uri") Core.<*> x Core..: "messageVersion"
