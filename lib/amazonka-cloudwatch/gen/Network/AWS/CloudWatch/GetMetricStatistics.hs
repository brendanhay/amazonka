...
instance AWSRequest GetMetricStatistics where
  ...
  response =
    receiveXMLWrapper
      ...
      ( \s h x ->
          GetMetricStatisticsResponse'
            <$> (x .@? "Datapoints" .!@ mempty >>= may (parseXMLList "member"))
            <*> (x .@? "Label")
            <*> (pure (fromEnum s))
      )
  ...
  toQuery GetMetricStatistics' {..} =
    mconcat
      [ "Action" =: ("GetMetricStatistics" :: ByteString),
        ...
        "ExtendedStatistics"
          =: toQuery (toQueryList "member" <$> _gmsExtendedStatistics),
        "Statistics" =: toQuery (toQueryList "member" <$> _gmsStatistics),
        "Dimensions" =: toQuery (toQueryList "member" <$> _gmsDimensions),
        "Unit" =: _gmsUnit,
