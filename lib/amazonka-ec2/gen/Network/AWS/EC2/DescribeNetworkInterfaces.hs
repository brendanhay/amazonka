...
instance AWSPager DescribeNetworkInterfaces where
  page rq rs
    ...
    | otherwise = Just $ rq & dnisNextToken .~ rs ^. dnirsNextToken
...
instance ToQuery DescribeNetworkInterfaces where
  toQuery DescribeNetworkInterfaces' {..} =
    mconcat
      [ "Action" =: ("DescribeNetworkInterfaces" :: ByteString),
        "Version" =: ("2016-11-15" :: ByteString),
        toQuery
          (toQueryList "NetworkInterfaceId" <$> _dnisNetworkInterfaceIds),
