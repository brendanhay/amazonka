...
data CreateJob = CreateJob'
  { _cjJobType :: !(Maybe JobType),
    ...
    _cjDeviceConfiguration :: !(Maybe DeviceConfiguration),
...
instance AWSRequest CreateJob where
  ...
  response =
    receiveJSON
      ( \s h x ->
          CreateJobResponse' <$> (x .?> "JobId") <*> (pure (fromEnum s))
      )
...
instance ToHeaders CreateJob where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AWSIESnowballJobManagementService.CreateJob" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("JobType" .=) <$> _cjJobType,
            ...
            ("ForwardingAddressId" .=) <$> _cjForwardingAddressId,
            ...
            ("DeviceConfiguration" .=) <$> _cjDeviceConfiguration,
