...
-- 'SplitShard' when a shard appears to be approaching its maximum
...
-- real-time processing. You must specify the shard to be split and the new
-- hash key, which is the position in the shard where the shard gets split in
-- two. In many cases, the new hash key might simply be the average of the
-- beginning and ending hash key, but it can be any hash key value in the
...
-- 'ShardToSplit' and 'NewStartingHashKey' parameters that are specified in
...
-- state. You can use 'DescribeStream' to check the status of the stream,
...
-- 'UPDATING' or 'DELETING' states, 'DescribeStream' returns a
...
-- 'LimitExceededException'. The default limit for an AWS account is 10 shards
...
-- operate on too many streams in parallel using 'CreateStream',
