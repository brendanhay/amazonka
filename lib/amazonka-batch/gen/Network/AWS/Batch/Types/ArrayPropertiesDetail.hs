...
mkArrayPropertiesDetail
    :: ArrayPropertiesDetail
mkArrayPropertiesDetail
  = ArrayPropertiesDetail'{index = Core.Nothing, size = Core.Nothing,
...
apdStatusSummary :: Lens.Lens' ArrayPropertiesDetail (Core.Maybe (Core.HashMap Core.Text Core.Int))
apdStatusSummary = Lens.field @"statusSummary"
{-# INLINEABLE apdStatusSummary #-}
{-# DEPRECATED statusSummary "Use generic-lens or generic-optics with 'statusSummary' instead"  #-}

instance Core.FromJSON ArrayPropertiesDetail where
        parseJSON
          = Core.withObject "ArrayPropertiesDetail" Core.$
              \ x ->
                ArrayPropertiesDetail' Core.<$>
                  (x Core..:? "index") Core.<*> x Core..:? "size" Core.<*>
                    x Core..:? "statusSummary"
