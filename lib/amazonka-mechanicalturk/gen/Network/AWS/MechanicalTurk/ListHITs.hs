...
listHITs ::
  ListHITs
listHITs =
  ListHITs' {_lhitNextToken = Nothing, _lhitMaxResults = Nothing}
...
instance AWSPager ListHITs where
  page rq rs
    ...
    | otherwise = Just $ rq & lhitNextToken .~ rs ^. lhitrsNextToken
...
instance ToHeaders ListHITs where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("MTurkRequesterServiceV20170117.ListHITs" :: ByteString),
