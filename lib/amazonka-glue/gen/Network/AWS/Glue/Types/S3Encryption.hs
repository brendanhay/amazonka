...
mkS3Encryption
    :: S3Encryption
mkS3Encryption
  = S3Encryption'{kmsKeyArn = Core.Nothing,
...
{-# INLINEABLE seS3EncryptionMode #-}
{-# DEPRECATED s3EncryptionMode "Use generic-lens or generic-optics with 's3EncryptionMode' instead"  #-}
...
instance Core.FromJSON S3Encryption where
        parseJSON
          = Core.withObject "S3Encryption" Core.$
              \ x ->
                S3Encryption' Core.<$>
                  (x Core..:? "KmsKeyArn") Core.<*> x Core..:? "S3EncryptionMode"
