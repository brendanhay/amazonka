...
-- Amazon CloudSearch supports four query parsers: 'simple': search all 'text'
-- and 'text-array' fields for the specified string. Search for phrases,
-- individual terms, and prefixes. 'structured': search specific fields,
-- construct compound queries using Boolean operators, and use advanced
-- features such as term boosting and proximity searching. 'lucene': specify
-- search criteria using the Apache Lucene query parser syntax. 'dismax':
...
-- cursor value to 'initial'. In subsequent requests, specify the cursor
-- value returned in the hits section of the response. For more information,
-- see
...
-- 'buckets'. 'size' specifies the maximum number of facets to include in
...
-- option; it cannot be used in conjunction with 'buckets'. 'sort' specifies
-- how you want to sort the facets in the results: 'bucket' or 'count'.
-- Specify 'bucket' to sort alphabetically or numerically by facet value (in
-- ascending order). Specify 'count' to sort by the facet counts computed
-- for each facet value (in descending order). To retrieve facet counts for
-- particular values or ranges of values, use the 'buckets' option instead
-- of 'sort'. If no facet options are specified, facet counts are computed
-- for all field values, the facets are sorted by facet count, and the top
-- 10 facets are returned in the results. For more information, see
...
-- format of the data in the text field: 'text' or 'html'. When data is
-- returned as HTML, all non-alphanumeric characters are encoded. The
-- default is 'html'. 'max_phrases': specifies the maximum number of
-- occurrences of the search term(s) you want to highlight. By default, the
-- first occurrence is highlighted. 'pre_tag': specifies the string to
...
-- text highlights is '*'. If no highlight options are specified for a
...
-- options specified in the 'queryOptions' parameter. By default, the
-- 'simple' query parser is used to process requests. To use the
-- 'structured', 'lucene', or 'dismax' query parser, you must also specify
-- the 'queryParser' parameter. For more information about specifying search
-- criteria, see
-- <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching.html
...
-- you use: 'defaultOperator': The default operator used to combine
-- individual terms in the search string. For example: 'defaultOperator:
-- 'or''. For the 'dismax' parser, you specify a percentage that represents
-- the percentage of terms in the search string (rounded down) that must
-- match, rather than a default operator. A value of '0%' is the equivalent
-- to OR, and a value of '100%' is equivalent to AND. The percentage must be
-- specified as a value in the range 0-100 followed by the percent (%)
-- symbol. For example, 'defaultOperator: 50%'. Valid values: 'and', 'or', a
...
-- text-array fields are searched. You can specify a weight for each field
-- to control the relative importance of each field when Amazon CloudSearch
...
-- importance of the 'title' field over the 'description' field you could
-- specify: '"fields":["title^5","description"]'. Valid values: The name of
...
-- operators or special characters you want to disable for the simple query
-- parser. If you disable the 'and', 'or', or 'not' operators, the
-- corresponding operators ('+', '|', '-') have no special meaning and are
-- dropped from the search string. Similarly, disabling 'prefix' disables
-- the wildcard operator ('*') and disabling 'phrase' disables the ability
...
-- parentheses. Disabling 'near' disables the ability to use the ~ operator
-- to perform a sloppy phrase search. Disabling the 'fuzzy' operator
-- disables the ability to use the ~ operator to perform a fuzzy search.
-- 'escape' disables the ability to use a backslash ('\') to escape special
...
-- Valid for: 'simple'. 'phraseFields': An array of the 'text' or
-- 'text-array' fields you want to use for phrase searches. When the terms
-- in the search string appear in close proximity within a field, the field
...
-- example, to boost phrase matches in the 'title' field over the 'abstract'
-- field, you could specify: '"phraseFields":["title^3", "plot"]' Valid
-- values: The name of any 'text' or 'text-array' field and an optional
-- numeric value greater than zero. Default: No fields. If you don't specify
-- any fields with 'phraseFields', proximity scoring is disabled even if
-- 'phraseSlop' is specified. Valid for: 'dismax'. 'phraseSlop': An integer
-- value that specifies how much matches can deviate from the search phrase
-- and still be boosted according to the weights specified in the
-- 'phraseFields' option; for example, 'phraseSlop: 2'. You must also
-- specify 'phraseFields' to enable proximity scoring. Valid values:
-- positive integers. Default: 0. Valid for: 'dismax'. 'explicitPhraseSlop':
-- An integer value that specifies how much a match can deviate from the
-- search phrase when the phrase is enclosed in double quotes in the search
-- string. (Phrases that exceed this proximity distance are not considered a
-- match.) For example, to specify a slop of three for dismax phrase
-- queries, you would specify '"explicitPhraseSlop":3'. Valid values:
-- positive integers. Default: 0. Valid for: 'dismax'. 'tieBreaker': When a
-- term in the search string is found in a document's field, a score is
-- calculated for that field based on how common the word is in that field
-- compared to other documents. If the term occurs in multiple fields within
-- a document, by default only the highest scoring field contributes to the
-- document's overall score. You can specify a 'tieBreaker' value to enable
...
-- tieBreaker is '(max field score) + (tieBreaker) * (sum of the scores for
-- the rest of the matching fields)'. Set 'tieBreaker' to 0 to disregard all
-- but the highest scoring field (pure max): '"tieBreaker":0'. Set to 1 to
-- sum the scores from all fields (pure sum): '"tieBreaker":1'. Valid
...
-- query parser. Amazon CloudSearch supports four query parsers: 'simple':
-- perform simple searches of 'text' and 'text-array' fields. By default,
-- the 'simple' query parser searches all 'text' and 'text-array' fields.
...
-- unless you configure the default operator with the 'queryOptions'
-- parameter.) You can use the '-' (NOT), '|' (OR), and '*' (wildcard)
...
-- ID ('_id asc') and version ('_version desc'). For more information, see
...
-- are mutually exclusive. For more information, see
-- <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/paginating-results.html
-- Paginating Results> in the /Amazon CloudSearch Developer Guide/.
