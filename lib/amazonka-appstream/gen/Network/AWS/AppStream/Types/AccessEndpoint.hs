...
mkAccessEndpoint
    :: Types.AccessEndpointType -- ^ 'endpointType'
    -> AccessEndpoint
...
aeVpceId :: Lens.Lens' AccessEndpoint (Core.Maybe Core.Text)
aeVpceId = Lens.field @"vpceId"
{-# INLINEABLE aeVpceId #-}
{-# DEPRECATED vpceId "Use generic-lens or generic-optics with 'vpceId' instead"  #-}
...
instance Core.FromJSON AccessEndpoint where
        parseJSON
          = Core.withObject "AccessEndpoint" Core.$
              \ x ->
                AccessEndpoint' Core.<$>
                  (x Core..: "EndpointType") Core.<*> x Core..:? "VpceId"
