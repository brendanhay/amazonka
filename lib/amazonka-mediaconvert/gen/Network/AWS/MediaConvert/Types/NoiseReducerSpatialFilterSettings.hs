...
mkNoiseReducerSpatialFilterSettings
    :: NoiseReducerSpatialFilterSettings
mkNoiseReducerSpatialFilterSettings
  = NoiseReducerSpatialFilterSettings'{postFilterSharpenStrength =
                                         Core.Nothing,
                                       speed = Core.Nothing, strength = Core.Nothing}
...
{-# INLINEABLE nrsfsPostFilterSharpenStrength #-}
{-# DEPRECATED postFilterSharpenStrength "Use generic-lens or generic-optics with 'postFilterSharpenStrength' instead"  #-}
...
instance Core.FromJSON NoiseReducerSpatialFilterSettings where
        toJSON NoiseReducerSpatialFilterSettings{..}
          = Core.object
              (Core.catMaybes
                 [("postFilterSharpenStrength" Core..=) Core.<$>
...
instance Core.FromJSON NoiseReducerSpatialFilterSettings where
        parseJSON
          = Core.withObject "NoiseReducerSpatialFilterSettings" Core.$
              \ x ->
                NoiseReducerSpatialFilterSettings' Core.<$>
                  (x Core..:? "postFilterSharpenStrength") Core.<*>
                    x Core..:? "speed"
