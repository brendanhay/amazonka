...
data CreateRelationalDatabaseFromSnapshot = CreateRelationalDatabaseFromSnapshot'
  { _crdfsSourceRelationalDatabaseName   :: !(Maybe Text)
  , _crdfsRelationalDatabaseBundleId     :: !(Maybe Text)
  , _crdfsPubliclyAccessible             :: !(Maybe Bool)
  , _crdfsUseLatestRestorableTime        :: !(Maybe Bool)
  , _crdfsRestoreTime                    :: !(Maybe POSIX)
  , _crdfsAvailabilityZone               :: !(Maybe Text)
  , _crdfsRelationalDatabaseSnapshotName :: !(Maybe Text)
  , _crdfsTags                           :: !(Maybe [Tag])
  , _crdfsRelationalDatabaseName         :: !Text
...
--
-- * 'crdfsTags' - The tag keys and optional values to add to the resource during create. To tag a resource after it has been created, see the @tag resource@ operation.
--
...
createRelationalDatabaseFromSnapshot
    :: Text -- ^ 'crdfsRelationalDatabaseName'
    -> CreateRelationalDatabaseFromSnapshot
createRelationalDatabaseFromSnapshot pRelationalDatabaseName_ =
  CreateRelationalDatabaseFromSnapshot'
    { _crdfsSourceRelationalDatabaseName = Nothing
...
instance ToJSON CreateRelationalDatabaseFromSnapshot
         where
...
instance ToPath CreateRelationalDatabaseFromSnapshot
         where
        toPath = const "/"

instance ToQuery CreateRelationalDatabaseFromSnapshot
         where
...
data CreateRelationalDatabaseFromSnapshotResponse = CreateRelationalDatabaseFromSnapshotResponse'
  { _crdfsrsOperations     :: !(Maybe [Operation])
  , _crdfsrsResponseStatus :: !Int
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'crdfsrsOperations' - An object describing the result of your create relational database from snapshot request.
--
-- * 'crdfsrsResponseStatus' - -- | The response status code.
createRelationalDatabaseFromSnapshotResponse
    :: Int -- ^ 'crdfsrsResponseStatus'
    -> CreateRelationalDatabaseFromSnapshotResponse
createRelationalDatabaseFromSnapshotResponse pResponseStatus_ =
  CreateRelationalDatabaseFromSnapshotResponse'
    {_crdfsrsOperations = Nothing, _crdfsrsResponseStatus = pResponseStatus_}
