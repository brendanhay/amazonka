...
mkCreateSecret
    ...
    -> CreateSecret
mkCreateSecret name
  = CreateSecret'{name, clientRequestToken = Core.Nothing,
...
instance Core.ToHeaders CreateSecret where
        toHeaders CreateSecret{..}
          = Core.pure ("X-Amz-Target", "secretsmanager.CreateSecret") Core.<>
...
mkCreateSecretResponse responseStatus
  = CreateSecretResponse'{arn = Core.Nothing, name = Core.Nothing,
