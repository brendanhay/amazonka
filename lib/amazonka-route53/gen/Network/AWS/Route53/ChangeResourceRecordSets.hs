...
{-# INLINEABLE crrsChangeBatch #-}
{-# DEPRECATED changeBatch "Use generic-lens or generic-optics with 'changeBatch' instead"  #-}
...
instance Core.ToXML ChangeResourceRecordSets where
        toXML ChangeResourceRecordSets{..}
          = Core.toXMLElement "ChangeBatch" changeBatch
        toXMLDocument
          = Core.newXMLDocument
...
instance Core.AWSRequest ChangeResourceRecordSets where
        ...
        parseResponse
          = Response.receiveXML
              (\ s h x ->
                 ChangeResourceRecordSetsResponse' Core.<$>
