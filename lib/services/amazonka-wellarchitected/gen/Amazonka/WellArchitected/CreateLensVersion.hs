...
instance Core.ToHeaders CreateLensVersion where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "Content-Type"
...
instance Core.ToJSON CreateLensVersion where
  toJSON CreateLensVersion' {..} =
    Core.object
      ( Prelude.catMaybes
          [ ("IsMajorVersion" Core..=)
              Prelude.<$> isMajorVersion,
            Prelude.Just ("LensVersion" Core..= lensVersion),
            Prelude.Just
              ("ClientRequestToken" Core..= clientRequestToken)
          ]
      )

instance Core.ToPath CreateLensVersion where
  toPath CreateLensVersion' {..} =
    Prelude.mconcat
      ["/lenses/", Core.toBS lensAlias, "/versions"]

instance Core.ToQuery CreateLensVersion where
