...
mkBehavior
    :: Types.BehaviorName -- ^ 'name'
    -> Behavior
mkBehavior name
  = Behavior'{name, criteria = Core.Nothing, metric = Core.Nothing,
...
{-# INLINEABLE bCriteria #-}
{-# DEPRECATED criteria "Use generic-lens or generic-optics with 'criteria' instead"  #-}
...
{-# INLINEABLE bMetricDimension #-}
{-# DEPRECATED metricDimension "Use generic-lens or generic-optics with 'metricDimension' instead"  #-}
...
instance Core.FromJSON Behavior where
        parseJSON
          = Core.withObject "Behavior" Core.$
              \ x ->
                Behavior' Core.<$>
                  (x Core..: "name") Core.<*> x Core..:? "criteria" Core.<*>
                    x Core..:? "metric"
