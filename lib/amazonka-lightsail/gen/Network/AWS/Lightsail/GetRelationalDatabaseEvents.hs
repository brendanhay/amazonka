...
instance AWSPager GetRelationalDatabaseEvents where
  page rq rs
    ...
    | stop (rs ^. grdersRelationalDatabaseEvents) = Nothing
    | otherwise =
      Just $ rq & grdePageToken .~ rs ^. grdersNextPageToken
...
instance ToHeaders GetRelationalDatabaseEvents where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("Lightsail_20161128.GetRelationalDatabaseEvents" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("durationInMinutes" .=) <$> _grdeDurationInMinutes,
            ...
            Just ("relationalDatabaseName" .= _grdeRelationalDatabaseName)
          ]
      )
...
data GetRelationalDatabaseEventsResponse = GetRelationalDatabaseEventsResponse'
  { _grdersNextPageToken ::
      !(Maybe Text),
    _grdersRelationalDatabaseEvents ::
      !( Maybe
           [RelationalDatabaseEvent]
       ),
    _grdersResponseStatus ::
      !Int
  }
  deriving (Eq, Read, Show, Data, Typeable, Generic)
