...
mkWaitActivity
    :: WaitActivity
mkWaitActivity
  = WaitActivity'{nextActivity = Core.Nothing,
...
{-# INLINEABLE waWaitTime #-}
{-# DEPRECATED waitTime "Use generic-lens or generic-optics with 'waitTime' instead"  #-}
...
instance Core.FromJSON WaitActivity where
        parseJSON
          = Core.withObject "WaitActivity" Core.$
              \ x ->
                WaitActivity' Core.<$>
                  (x Core..:? "NextActivity") Core.<*> x Core..:? "WaitTime"
