...
instance Core.AWSRequest DetectSyntax where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 DetectSyntaxResponse' Core.<$>
                   (x Core..:? "SyntaxTokens") Core.<*> Core.pure (Core.fromEnum s))
...
mkDetectSyntaxResponse responseStatus
  = DetectSyntaxResponse'{syntaxTokens = Core.Nothing,
                          responseStatus}
