...
instance ToHeaders CreateStorediSCSIVolume where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("StorageGateway_20130630.CreateStorediSCSIVolume" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("KMSKey" .=) <$> _csscsivKMSKey,
            ...
            Just ("PreserveExistingData" .= _csscsivPreserveExistingData),
            Just ("TargetName" .= _csscsivTargetName),
            Just ("NetworkInterfaceId" .= _csscsivNetworkInterfaceId)
          ]
      )
