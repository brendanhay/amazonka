...
mkResultByTime
    :: ResultByTime
mkResultByTime
  = ResultByTime'{estimated = Core.Nothing, groups = Core.Nothing,
...
{-# INLINEABLE rbtEstimated #-}
{-# DEPRECATED estimated "Use generic-lens or generic-optics with 'estimated' instead"  #-}
...
instance Core.FromJSON ResultByTime where
        parseJSON
          = Core.withObject "ResultByTime" Core.$
              \ x ->
                ResultByTime' Core.<$>
                  (x Core..:? "Estimated") Core.<*> x Core..:? "Groups" Core.<*>
                    x Core..:? "TimePeriod"
                    Core.<*> x Core..:? "Total"
