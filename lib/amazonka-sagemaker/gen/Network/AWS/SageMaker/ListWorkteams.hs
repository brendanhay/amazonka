...
mkListWorkteams
    :: ListWorkteams
mkListWorkteams
  = ListWorkteams'{maxResults = Core.Nothing,
...
instance Core.ToHeaders ListWorkteams where
        toHeaders ListWorkteams{..}
          = Core.pure ("X-Amz-Target", "SageMaker.ListWorkteams") Core.<>
...
instance Core.AWSRequest ListWorkteams where
        ...
        parseResponse
          = Response.receiveJSON
              (\ s h x ->
                 ListWorkteamsResponse' Core.<$>
                   (x Core..:? "Workteams" Core..!= Core.mempty) Core.<*>
                     x Core..:? "NextToken"
...
mkListWorkteamsResponse responseStatus
  = ListWorkteamsResponse'{workteams = Core.mempty,
...
{-# INLINEABLE lrsWorkteams #-}
{-# DEPRECATED workteams "Use generic-lens or generic-optics with 'workteams' instead"  #-}
