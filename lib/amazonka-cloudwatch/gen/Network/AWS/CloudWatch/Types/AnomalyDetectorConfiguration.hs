...
mkAnomalyDetectorConfiguration
    :: AnomalyDetectorConfiguration
mkAnomalyDetectorConfiguration
  = AnomalyDetectorConfiguration'{excludedTimeRanges = Core.Nothing,
...
{-# INLINEABLE adcExcludedTimeRanges #-}
{-# DEPRECATED excludedTimeRanges "Use generic-lens or generic-optics with 'excludedTimeRanges' instead"  #-}
...
{-# INLINEABLE adcMetricTimezone #-}
{-# DEPRECATED metricTimezone "Use generic-lens or generic-optics with 'metricTimezone' instead"  #-}

instance Core.ToQuery AnomalyDetectorConfiguration where
        toQuery AnomalyDetectorConfiguration{..}
          = Core.toQueryPair "ExcludedTimeRanges"
              (Core.maybe Core.mempty (Core.toQueryList "member")
                 excludedTimeRanges)
              Core.<>
              Core.maybe Core.mempty (Core.toQueryPair "MetricTimezone")
                metricTimezone

instance Core.FromXML AnomalyDetectorConfiguration where
        parseXML x
          = AnomalyDetectorConfiguration' Core.<$>
              (x Core..@? "ExcludedTimeRanges" Core..<@>
