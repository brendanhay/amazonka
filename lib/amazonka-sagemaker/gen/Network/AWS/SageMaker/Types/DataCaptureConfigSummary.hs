...
mkDataCaptureConfigSummary
    :: Core.Bool -- ^ 'enableCapture'
    -> Types.CaptureStatus -- ^ 'captureStatus'
    -> Core.Natural -- ^ 'currentSamplingPercentage'
    -> Types.DestinationS3Uri -- ^ 'destinationS3Uri'
    -> Types.KmsKeyId -- ^ 'kmsKeyId'
    -> DataCaptureConfigSummary
mkDataCaptureConfigSummary enableCapture captureStatus
  currentSamplingPercentage destinationS3Uri kmsKeyId
  = DataCaptureConfigSummary'{enableCapture, captureStatus,
                              currentSamplingPercentage, destinationS3Uri, kmsKeyId}
...
{-# INLINEABLE dccsCaptureStatus #-}
{-# DEPRECATED captureStatus "Use generic-lens or generic-optics with 'captureStatus' instead"  #-}
...
{-# INLINEABLE dccsCurrentSamplingPercentage #-}
{-# DEPRECATED currentSamplingPercentage "Use generic-lens or generic-optics with 'currentSamplingPercentage' instead"  #-}
...
{-# INLINEABLE dccsDestinationS3Uri #-}
...
instance Core.FromJSON DataCaptureConfigSummary where
        parseJSON
          = Core.withObject "DataCaptureConfigSummary" Core.$
              \ x ->
                DataCaptureConfigSummary' Core.<$>
                  (x Core..: "EnableCapture") Core.<*> x Core..: "CaptureStatus"
