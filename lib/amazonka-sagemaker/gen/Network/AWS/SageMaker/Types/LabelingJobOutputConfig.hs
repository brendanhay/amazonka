...
mkLabelingJobOutputConfig s3OutputPath
  = LabelingJobOutputConfig'{s3OutputPath, kmsKeyId = Core.Nothing,
...
instance Core.FromJSON LabelingJobOutputConfig where
        parseJSON
          = Core.withObject "LabelingJobOutputConfig" Core.$
              \ x ->
                LabelingJobOutputConfig' Core.<$>
                  (x Core..: "S3OutputPath") Core.<*> x Core..:? "KmsKeyId" Core.<*>
                    x Core..:? "SnsTopicArn"
