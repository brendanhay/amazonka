...
instance AWSRequest DeleteConfigurationAggregator where
  type
...
instance Hashable DeleteConfigurationAggregator

instance NFData DeleteConfigurationAggregator

instance ToHeaders DeleteConfigurationAggregator where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
