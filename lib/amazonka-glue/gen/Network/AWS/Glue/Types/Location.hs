...
mkLocation
    :: Location
mkLocation
  = Location'{dynamoDB = Core.Nothing, jdbc = Core.Nothing,
              s3 = Core.Nothing}
...
{-# INLINEABLE lJdbc #-}
{-# DEPRECATED jdbc "Use generic-lens or generic-optics with 'jdbc' instead"  #-}
          ...
          = Core.object
              (Core.catMaybes
                 [("DynamoDB" Core..=) Core.<$> dynamoDB,
                  ("Jdbc" Core..=) Core.<$> jdbc, ("S3" Core..=) Core.<$> s3])
