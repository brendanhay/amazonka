...
data ResultConfiguration = ResultConfiguration'
  { -- | Indicates that an Amazon S3 canned ACL should be set to control
    ...
    -- stored, such as @s3:\/\/path\/to\/query\/bucket\/@. To run the query,
    ...
    -- or in the workgroup, using WorkGroupConfiguration. If none of them is
    -- set, Athena issues an error that no output location is provided. For
    -- more information, see
...
-- <https://docs.aws.amazon.com/athena/latest/ug/workgroups-settings-override.html Workgroup Settings Override Client-Side Settings>.
--
-- 'encryptionConfiguration', 'resultConfiguration_encryptionConfiguration' - If query and calculation results are encrypted in Amazon S3, indicates
-- the encryption option used (for example, @SSE_KMS@ or @CSE_KMS@) and key
-- information. This is a client-side setting. If workgroup settings
-- override client-side settings, then the query uses the encryption
-- configuration that is specified for the workgroup, and also uses the
...
-- <https://docs.aws.amazon.com/athena/latest/ug/workgroups-settings-override.html Workgroup Settings Override Client-Side Settings>.
--
-- 'outputLocation', 'resultConfiguration_outputLocation' - The location in Amazon S3 where your query and calculation results are
-- stored, such as @s3:\/\/path\/to\/query\/bucket\/@. To run the query,
-- you must specify the query results location using one of the ways:
-- either for individual queries using either this setting (client-side),
-- or in the workgroup, using WorkGroupConfiguration. If none of them is
