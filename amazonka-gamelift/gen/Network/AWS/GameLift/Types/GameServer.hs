...
data GameServer = GameServer'
  { -- | The unique identifier for the instance where the game server is running.
    ...
    claimStatus :: Core.Maybe GameServerClaimStatus,
    ...
    lastClaimTime :: Core.Maybe Core.POSIX,
    ...
    lastHealthCheckTime :: Core.Maybe Core.POSIX
  }
...
-- This ID is available in the instance metadata. EC2 instance IDs use a
-- 17-character format, for example: @i-1234567890abcdef0@.
gameServer_instanceId :: Lens.Lens' GameServer (Core.Maybe Core.Text)
...
-- -   @UTILIZED@ - The game server is currently hosting a game session
--     with players.
gameServer_utilizationStatus :: Lens.Lens' GameServer (Core.Maybe GameServerUtilizationStatus)
...
-- trigger the game server to update its utilization status. After one
-- minute, the game server claim status reverts to null.
gameServer_claimStatus :: Lens.Lens' GameServer (Core.Maybe GameServerClaimStatus)
gameServer_claimStatus = Lens.lens (\GameServer' {claimStatus} -> claimStatus) (\s@GameServer' {} a -> s {claimStatus = a} :: GameServer)

-- | A set of custom game server properties, formatted as a single string
-- value. This data is passed to a game client or service when it requests
-- information on game servers using ListGameServers or ClaimGameServer.
gameServer_gameServerData :: Lens.Lens' GameServer (Core.Maybe Core.Text)
...
-- as milliseconds (for example @\"1469498468.057\"@). This value is used
-- to calculate when a claimed game server\'s status should revert to null.
gameServer_lastClaimTime :: Lens.Lens' GameServer (Core.Maybe Core.UTCTime)
gameServer_lastClaimTime = Lens.lens (\GameServer' {lastClaimTime} -> lastClaimTime) (\s@GameServer' {} a -> s {lastClaimTime = a} :: GameServer) Core.. Lens.mapping Core._Time

-- | Timestamp that indicates when the game server was created with a
-- RegisterGameServer request. The format is a number expressed in Unix
-- time as milliseconds (for example @\"1469498468.057\"@).
gameServer_registrationTime :: Lens.Lens' GameServer (Core.Maybe Core.UTCTime)
gameServer_registrationTime = Lens.lens (\GameServer' {registrationTime} -> registrationTime) (\s@GameServer' {} a -> s {registrationTime = a} :: GameServer) Core.. Lens.mapping Core._Time

-- | The ARN identifier for the game server group where the game server is
-- located.
gameServer_gameServerGroupArn :: Lens.Lens' GameServer (Core.Maybe Core.Text)
...
-- IDs are developer-defined and are unique across all game server groups
-- in an AWS account.
gameServer_gameServerId :: Lens.Lens' GameServer (Core.Maybe Core.Text)
gameServer_gameServerId = Lens.lens (\GameServer' {gameServerId} -> gameServerId) (\s@GameServer' {} a -> s {gameServerId = a} :: GameServer)

-- | A unique identifier for the game server group where the game server is
-- running. Use either the GameServerGroup name or ARN value.
gameServer_gameServerGroupName :: Lens.Lens' GameServer (Core.Maybe Core.Text)
...
-- | The port and IP address that must be used to establish a client
-- connection to the game server.
gameServer_connectionInfo :: Lens.Lens' GameServer (Core.Maybe Core.Text)
...
-- @\"1469498468.057\"@). After game server registration, this property is
-- only changed when a game server update specifies a health check value.
gameServer_lastHealthCheckTime :: Lens.Lens' GameServer (Core.Maybe Core.UTCTime)
gameServer_lastHealthCheckTime = Lens.lens (\GameServer' {lastHealthCheckTime} -> lastHealthCheckTime) (\s@GameServer' {} a -> s {lastHealthCheckTime = a} :: GameServer) Core.. Lens.mapping Core._Time
...
instance Core.Hashable GameServer

instance Core.NFData GameServer
