...
{-# INLINEABLE iaiAccelerators #-}
{-# DEPRECATED accelerators "Use generic-lens or generic-optics with 'accelerators' instead"  #-}

instance Core.FromXML InferenceAcceleratorInfo where
        parseXML x
          = InferenceAcceleratorInfo' Core.<$>
              (x Core..@? "accelerators" Core..<@> Core.parseXMLList "member")
