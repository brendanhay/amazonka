...
instance ToHeaders CreateLabelingJob where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("SageMaker.CreateLabelingJob" :: ByteString),
    ...
    object
      ( catMaybes
          [ ("LabelingJobAlgorithmsConfig" .=)
              <$> _cljLabelingJobAlgorithmsConfig,
            ("LabelCategoryConfigS3Uri" .=) <$> _cljLabelCategoryConfigS3URI,
            ("StoppingConditions" .=) <$> _cljStoppingConditions,
            ("Tags" .=) <$> _cljTags,
            Just ("LabelingJobName" .= _cljLabelingJobName),
            Just ("LabelAttributeName" .= _cljLabelAttributeName),
