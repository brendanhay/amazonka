...
-- storage class that you want to assign to the files. If you specify values
-- for @ContentConfig@, you must also specify values for @ThumbnailConfig@.
...
-- the @Grantee@ object: Canonical: The value in the @Grantee@ object is
...
-- ID is not the same as an AWS account number. Email: The value in the
-- @Grantee@ object is the registered email address of an AWS account.
...
-- access to transcoded files and playlists. To identify the user or group,
-- you can specify the canonical user ID for an AWS account, an origin
...
-- @Grantee@. Permissions are granted on the files that Elastic Transcoder
-- adds to the bucket, including playlists and video files. Valid values
-- include: @READ@: The grantee can read the objects and metadata for
...
-- bucket. StorageClass: The Amazon S3 storage class, @Standard@ or
...
-- assign to the files. If you specify values for @ContentConfig@, you must
-- also specify values for @ThumbnailConfig@ even if you don't want to
-- create thumbnails. If you specify values for @ContentConfig@ and
-- @ThumbnailConfig@, omit the @OutputBucket@ object. Bucket: The Amazon S3
...
-- object: Canonical: The value in the @Grantee@ object is either the
...
-- AWS account number. Email: The value in the @Grantee@ object is the
-- registered email address of an AWS account. Group: The value in the
...
-- or group that you want to have access to thumbnail files. To identify the
-- user or group, you can specify the canonical user ID for an AWS account,
-- an origin access identity for a CloudFront distribution, the registered
-- email address of an AWS account, or a predefined Amazon S3 group. Access:
-- The permission that you want to give to the AWS user that you specified
-- in @Grantee@. Permissions are granted on the thumbnail files that Elastic
-- Transcoder adds to the bucket. Valid values include: @READ@: The grantee
...
-- ACL for thumbnails that Elastic Transcoder adds to the Amazon S3 bucket.
