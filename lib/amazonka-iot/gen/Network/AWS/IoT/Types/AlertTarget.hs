...
mkAlertTarget
    :: Types.AlertTargetArn -- ^ 'alertTargetArn'
...
{-# INLINEABLE atAlertTargetArn #-}
{-# DEPRECATED alertTargetArn "Use generic-lens or generic-optics with 'alertTargetArn' instead"  #-}
...
instance Core.FromJSON AlertTarget where
        parseJSON
          = Core.withObject "AlertTarget" Core.$
              \ x ->
                AlertTarget' Core.<$>
                  (x Core..: "alertTargetArn") Core.<*> x Core..: "roleArn"
