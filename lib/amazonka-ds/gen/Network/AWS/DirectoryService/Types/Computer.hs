...
mkComputer
    :: Computer
mkComputer
  = Computer'{computerAttributes = Core.Nothing,
              computerId = Core.Nothing, computerName = Core.Nothing}
...
{-# INLINEABLE cComputerAttributes #-}
...
{-# INLINEABLE cComputerId #-}
{-# DEPRECATED computerId "Use generic-lens or generic-optics with 'computerId' instead"  #-}
...
instance Core.FromJSON Computer where
        parseJSON
          = Core.withObject "Computer" Core.$
              \ x ->
                Computer' Core.<$>
                  (x Core..:? "ComputerAttributes") Core.<*> x Core..:? "ComputerId"
