...
data CreateChangeSet = CreateChangeSet'
  { _ccsChangeSetType ::
    ...
    _ccsIncludeNestedStacks :: !(Maybe Bool),
...
instance AWSRequest CreateChangeSet where
  ...
  response =
    receiveXMLWrapper
      ...
      ( \s h x ->
          CreateChangeSetResponse'
            <$> (x .@? "Id") <*> (x .@? "StackId") <*> (pure (fromEnum s))
      )
  ...
  toQuery CreateChangeSet' {..} =
    mconcat
      [ "Action" =: ("CreateChangeSet" :: ByteString),
        ...
        "NotificationARNs"
          =: toQuery (toQueryList "member" <$> _ccsNotificationARNs),
        "IncludeNestedStacks" =: _ccsIncludeNestedStacks,
        "ResourcesToImport"
          =: toQuery (toQueryList "member" <$> _ccsResourcesToImport),
        "Parameters" =: toQuery (toQueryList "member" <$> _ccsParameters),
        ...
        "Tags" =: toQuery (toQueryList "member" <$> _ccsTags),
        "RoleARN" =: _ccsRoleARN,
