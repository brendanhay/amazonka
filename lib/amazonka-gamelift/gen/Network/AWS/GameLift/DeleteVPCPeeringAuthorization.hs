...
instance AWSRequest DeleteVPCPeeringAuthorization where
  type
  ...
  response =
    receiveEmpty
      ( \s h x ->
          DeleteVPCPeeringAuthorizationResponse' <$> (pure (fromEnum s))
      )

instance Hashable DeleteVPCPeeringAuthorization

instance NFData DeleteVPCPeeringAuthorization

instance ToHeaders DeleteVPCPeeringAuthorization where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("GameLift.DeleteVpcPeeringAuthorization" :: ByteString),
...
instance ToJSON DeleteVPCPeeringAuthorization where
  toJSON DeleteVPCPeeringAuthorization' {..} =
    object
      ( catMaybes
          [ Just ("GameLiftAwsAccountId" .= _dvpaGameLiftAWSAccountId),
