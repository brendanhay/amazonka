...
newtype ListLexicons = ListLexicons' {_llNextToken :: Maybe Text}
...
instance AWSPager ListLexicons where
  page rq rs
    ...
    | otherwise = Just $ rq & llNextToken .~ rs ^. llrsNextToken
...
instance ToQuery ListLexicons where
  toQuery ListLexicons' {..} = mconcat ["NextToken" =: _llNextToken]
