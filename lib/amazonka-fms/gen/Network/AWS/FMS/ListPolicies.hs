...
mkListPolicies
    :: ListPolicies
mkListPolicies
  = ListPolicies'{maxResults = Core.Nothing,
          ...
          = Response.receiveJSON
              (\ s h x ->
                 ListPoliciesResponse' Core.<$>
                   (x Core..:? "NextToken") Core.<*> x Core..:? "PolicyList" Core.<*>
...
mkListPoliciesResponse responseStatus
  = ListPoliciesResponse'{nextToken = Core.Nothing,
                          policyList = Core.Nothing, responseStatus}
...
{-# INLINEABLE lprrsPolicyList #-}
{-# DEPRECATED policyList "Use generic-lens or generic-optics with 'policyList' instead"  #-}
