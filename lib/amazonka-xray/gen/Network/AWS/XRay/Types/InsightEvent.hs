...
mkInsightEvent
    :: InsightEvent
mkInsightEvent
  = InsightEvent'{clientRequestImpactStatistics = Core.Nothing,
...
{-# INLINEABLE ieClientRequestImpactStatistics #-}
...
{-# INLINEABLE ieRootCauseServiceRequestImpactStatistics #-}
...
{-# INLINEABLE ieTopAnomalousServices #-}
...
instance Core.FromJSON InsightEvent where
        parseJSON
          = Core.withObject "InsightEvent" Core.$
              \ x ->
                InsightEvent' Core.<$>
                  (x Core..:? "ClientRequestImpactStatistics") Core.<*>
                    x Core..:? "EventTime"
