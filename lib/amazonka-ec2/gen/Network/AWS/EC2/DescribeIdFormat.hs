...
difResource :: Lens.Lens' DescribeIdFormat (Core.Maybe Core.Text)
difResource = Lens.field @"resource"
{-# INLINEABLE difResource #-}
...
instance Core.ToQuery DescribeIdFormat where
        toQuery DescribeIdFormat{..}
          = Core.toQueryPair "Action" ("DescribeIdFormat" :: Core.Text)
...
mkDescribeIdFormatResponse responseStatus
  = DescribeIdFormatResponse'{statuses = Core.Nothing,
                              responseStatus}
