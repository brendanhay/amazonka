...
data AddTags = AddTags' {_atPipelineId :: !Text, _atTags :: ![Tag]}
...
addTags pPipelineId_ =
  AddTags' {_atPipelineId = pPipelineId_, _atTags = mempty}
...
instance ToHeaders AddTags where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("DataPipeline.AddTags" :: ByteString),
...
instance ToJSON AddTags where
  toJSON AddTags' {..} =
    object
      ( catMaybes
          [Just ("pipelineId" .= _atPipelineId), Just ("tags" .= _atTags)]
      )
