...
instance AWSRequest StopDBCluster where
        ...
        response
          = receiveXMLWrapper "StopDBClusterResult"
              (\ s h x ->
                 StopDBClusterResponse' <$>
                   (x .@? "DBCluster") <*> (pure (fromEnum s)))

instance Hashable StopDBCluster where

instance NFData StopDBCluster where
...
data StopDBClusterResponse = StopDBClusterResponse'
  { _sdbcrsDBCluster      :: !(Maybe DBCluster)
...
stopDBClusterResponse
    :: Int -- ^ 'sdbcrsResponseStatus'
    -> StopDBClusterResponse
stopDBClusterResponse pResponseStatus_ =
  StopDBClusterResponse'
    {_sdbcrsDBCluster = Nothing, _sdbcrsResponseStatus = pResponseStatus_}
...
instance NFData StopDBClusterResponse where
