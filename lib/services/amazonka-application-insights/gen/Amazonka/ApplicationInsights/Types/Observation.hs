...
instance Prelude.Hashable Observation where
  hashWithSalt salt' Observation' {..} =
    salt' `Prelude.hashWithSalt` healthService
...
instance Prelude.NFData Observation where
  rnf Observation' {..} =
    Prelude.rnf codeDeployApplication
      ...
      `Prelude.seq` Prelude.rnf statesExecutionArn
