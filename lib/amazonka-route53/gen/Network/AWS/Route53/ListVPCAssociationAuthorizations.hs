...
instance AWSPager ListVPCAssociationAuthorizations where
  page rq rs
    ...
    | otherwise = Just $ rq & lvaaNextToken .~ rs ^. lvaarsNextToken
...
instance ToQuery ListVPCAssociationAuthorizations where
  toQuery ListVPCAssociationAuthorizations' {..} =
    mconcat
      ["nexttoken" =: _lvaaNextToken, "maxresults" =: _lvaaMaxResults]
...
instance NFData ListVPCAssociationAuthorizationsResponse
