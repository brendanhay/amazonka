...
mkEventCondition
    :: EventCondition
mkEventCondition
  = EventCondition'{dimensions = Core.Nothing,
...
{-# INLINEABLE ecMessageActivity #-}
{-# DEPRECATED messageActivity "Use generic-lens or generic-optics with 'messageActivity' instead"  #-}
...
instance Core.FromJSON EventCondition where
        parseJSON
          = Core.withObject "EventCondition" Core.$
              \ x ->
                EventCondition' Core.<$>
                  (x Core..:? "Dimensions") Core.<*> x Core..:? "MessageActivity"
