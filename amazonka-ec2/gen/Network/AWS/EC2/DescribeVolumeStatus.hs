...
-- the specified volumes: /Status/: Reflects the current status of the volume.
-- The possible values are @ok@, @impaired@ , @warning@, or
...
-- status is @insufficient-data@, then the checks may still be taking place on
-- your volume at the time. We recommend that you retry the request. For more
-- information on volume status, see
...
-- disabled, and may have inconsistent data. /Actions/: Reflect the actions
-- you may have to take in response to an event. For example, if the status of
...
-- @stalled@). @volume-status.status@ - The status of the volume (@ok@ |
