...
mkDataSource
    :: DataSource
mkDataSource
  = DataSource'{fileSystemDataSource = Core.Nothing,
...
{-# INLINEABLE dsFileSystemDataSource #-}
{-# DEPRECATED fileSystemDataSource "Use generic-lens or generic-optics with 'fileSystemDataSource' instead"  #-}
...
instance Core.FromJSON DataSource where
        parseJSON
          = Core.withObject "DataSource" Core.$
              \ x ->
                DataSource' Core.<$>
                  (x Core..:? "FileSystemDataSource") Core.<*>
                    x Core..:? "S3DataSource"
