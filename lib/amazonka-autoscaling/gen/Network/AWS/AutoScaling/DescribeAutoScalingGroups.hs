...
instance AWSPager DescribeAutoScalingGroups where
  page rq rs
    ...
    | otherwise = Just $ rq & dasgNextToken .~ rs ^. dasgrsNextToken
    ...
    receiveXMLWrapper
      ...
      ( \s h x ->
          DescribeAutoScalingGroupsResponse'
            <$> (x .@? "NextToken")
            <*> (pure (fromEnum s))
            <*> (x .@? "AutoScalingGroups" .!@ mempty >>= parseXMLList "member")
      )
...
instance ToQuery DescribeAutoScalingGroups where
  toQuery DescribeAutoScalingGroups' {..} =
    mconcat
      [ "Action" =: ("DescribeAutoScalingGroups" :: ByteString),
        "Version" =: ("2011-01-01" :: ByteString),
        "AutoScalingGroupNames"
          =: toQuery (toQueryList "member" <$> _dasgAutoScalingGroupNames),
