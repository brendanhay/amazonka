...
instance AWSPager ListCompatibleImages where
  page rq rs
    ...
    | otherwise = Just $ rq & lciNextToken .~ rs ^. lcirsNextToken
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lcirsCompatibleImages' - A JSON-formatted object that describes a compatible AMI, including the ID and name for a Snow device AMI.
--
