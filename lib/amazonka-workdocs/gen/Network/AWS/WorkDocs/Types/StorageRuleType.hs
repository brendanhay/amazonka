...
mkStorageRuleType
    :: StorageRuleType
mkStorageRuleType
  = StorageRuleType'{storageAllocatedInBytes = Core.Nothing,
...
{-# INLINEABLE srtStorageAllocatedInBytes #-}
{-# DEPRECATED storageAllocatedInBytes "Use generic-lens or generic-optics with 'storageAllocatedInBytes' instead"  #-}
...
instance Core.FromJSON StorageRuleType where
        toJSON StorageRuleType{..}
          = Core.object
              (Core.catMaybes
                 [("StorageAllocatedInBytes" Core..=) Core.<$>
...
instance Core.FromJSON StorageRuleType where
        parseJSON
          = Core.withObject "StorageRuleType" Core.$
              \ x ->
                StorageRuleType' Core.<$>
                  (x Core..:? "StorageAllocatedInBytes") Core.<*>
                    x Core..:? "StorageType"
