...
mkHopDestination
    :: HopDestination
mkHopDestination
  = HopDestination'{priority = Core.Nothing, queue = Core.Nothing,
...
{-# INLINEABLE hdWaitMinutes #-}
{-# DEPRECATED waitMinutes "Use generic-lens or generic-optics with 'waitMinutes' instead"  #-}
...
instance Core.FromJSON HopDestination where
        parseJSON
          = Core.withObject "HopDestination" Core.$
              \ x ->
                HopDestination' Core.<$>
                  (x Core..:? "priority") Core.<*> x Core..:? "queue" Core.<*>
                    x Core..:? "waitMinutes"
