...
instance AWSPager ListResourceComplianceSummaries where
  page rq rs
    ...
    | stop (rs ^. lrcsrsResourceComplianceSummaryItems) = Nothing
    | otherwise = Just $ rq & lrcsNextToken .~ rs ^. lrcsrsNextToken
...
instance ToHeaders ListResourceComplianceSummaries where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("AmazonSSM.ListResourceComplianceSummaries" :: ByteString),
...
data ListResourceComplianceSummariesResponse = ListResourceComplianceSummariesResponse'
  { _lrcsrsResourceComplianceSummaryItems ::
      !( Maybe
...
instance NFData ListResourceComplianceSummariesResponse
