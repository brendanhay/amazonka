...
mkChapInfo
    :: ChapInfo
mkChapInfo
  = ChapInfo'{initiatorName = Core.Nothing,
...
{-# INLINEABLE ciSecretToAuthenticateInitiator #-}
...
{-# INLINEABLE ciSecretToAuthenticateTarget #-}
...
instance Core.FromJSON ChapInfo where
        parseJSON
          = Core.withObject "ChapInfo" Core.$
              \ x ->
                ChapInfo' Core.<$>
                  (x Core..:? "InitiatorName") Core.<*>
                    x Core..:? "SecretToAuthenticateInitiator"
