...
data DescribeAssociationExecutionTargets = DescribeAssociationExecutionTargets'
  { _daetFilters       :: !(Maybe (List1 AssociationExecutionTargetsFilter))
  , _daetNextToken     :: !(Maybe Text)
  , _daetMaxResults    :: !(Maybe Nat)
  , _daetAssociationId :: !Text
  , _daetExecutionId   :: !Text
...
describeAssociationExecutionTargets
    :: Text -- ^ 'daetAssociationId'
    -> Text -- ^ 'daetExecutionId'
    -> DescribeAssociationExecutionTargets
...
instance AWSPager DescribeAssociationExecutionTargets
         where
        page rq rs
          | stop (rs ^. daetrsNextToken) = Nothing
          | stop (rs ^. daetrsAssociationExecutionTargets) =
            Nothing
          | otherwise =
            Just $ rq & daetNextToken .~ rs ^. daetrsNextToken
...
instance ToJSON DescribeAssociationExecutionTargets
         where
...
instance ToPath DescribeAssociationExecutionTargets
         where
        toPath = const "/"

instance ToQuery DescribeAssociationExecutionTargets
         where
...
data DescribeAssociationExecutionTargetsResponse = DescribeAssociationExecutionTargetsResponse'
  { _daetrsNextToken                   :: !(Maybe Text)
  , _daetrsAssociationExecutionTargets :: !(Maybe [AssociationExecutionTarget])
  , _daetrsResponseStatus              :: !Int
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
