...
instance AWSPager DescribeClientVPNAuthorizationRules where
  page rq rs
    ...
    | otherwise = Just $ rq & dcvarNextToken .~ rs ^. dcvarrsNextToken

instance AWSRequest DescribeClientVPNAuthorizationRules where
  type
...
instance ToHeaders DescribeClientVPNAuthorizationRules where
...
instance ToQuery DescribeClientVPNAuthorizationRules where
  toQuery DescribeClientVPNAuthorizationRules' {..} =
    mconcat
      [ "Action" =: ("DescribeClientVpnAuthorizationRules" :: ByteString),
...
instance NFData DescribeClientVPNAuthorizationRulesResponse
