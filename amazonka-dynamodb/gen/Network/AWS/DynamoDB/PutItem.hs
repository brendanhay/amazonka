...
-- be rejected with a /ValidationException/ exception. You can request that
-- /PutItem/ return either a copy of the original item (before the update) or
-- a copy of the updated item (after the update). For more information, see
-- the /ReturnValues/ description below. For more information about using this
-- API, see
-- <http://docs.aws.amazon.com/amazondynamodb/latest/developerguide/WorkingWithItems.html
-- Working with Items> in the /Amazon DynamoDB Developer Guide/.
--
...
-- pairs. /Expected/ provides a conditional block for the /PutItem/
...
-- Primary Key> in the /Amazon DynamoDB Developer Guide/. Each element in
...
-- setting is the default for /ReturnValues/.) ALL_OLD - If /PutItem/
