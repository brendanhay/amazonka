...
-- @AssumeRoleWithWebIdentity@. The @GetFederationToken@ action must be called
-- by using the long-term AWS security credentials of an IAM user. You can
-- also call @GetFederationToken@ using the security credentials of an AWS
-- account (root), but this is not recommended. Instead, we recommend that you
-- create an IAM user for the purpose of the proxy application and then attach
-- a policy to the IAM user that limits federated users to only the actions
-- and resources they need access to. For more information, see
...
-- by @GetFederationToken@ are determined by a combination of the following:
...
-- policy cannot grant more permissions than those that are defined in the IAM
-- user policy. A typical use case is that the permissions of the IAM user
-- whose credentials are used to call @GetFederationToken@ are designed to
-- allow access to all the actions and resources that any federated user will
-- need. Then, for individual users, you pass a policy to the operation that
-- scopes down the permissions to a level that's appropriate to that
-- individual user, using a policy that allows only a subset of permissions
-- that are granted to the IAM user. If you do not pass a policy, the
-- resulting temporary security credentials have no effective permissions. The
-- only exception is when the temporary security credentials are used to
-- access a resource that has a resource-based policy that specifically allows
-- the federated user to access the resource. For more information about how
...
-- You can use the federated user's ARN in your resource-based policies,
