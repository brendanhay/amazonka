...
data StartBulkDeployment = StartBulkDeployment'
  { _sbdAmznClientToken  :: !(Maybe Text)
  , _sbdExecutionRoleARN :: !(Maybe Text)
  , _sbdInputFileURI     :: !(Maybe Text)
...
startBulkDeployment =
  StartBulkDeployment'
    ...
    , _sbdExecutionRoleARN = Nothing
    , _sbdInputFileURI = Nothing
    }
...
sbdExecutionRoleARN :: Lens' StartBulkDeployment (Maybe Text)
sbdExecutionRoleARN = lens _sbdExecutionRoleARN (\ s a -> s{_sbdExecutionRoleARN = a})

-- | The URI of the input file contained in the S3 bucket. The execution role must have ''getObject'' permissions on this bucket to access the input file. The input file is a JSON-serialized, line delimited file with UTF-8 encoding that provides a list of group and version IDs and the deployment type. This file must be less than 100 MB. Currently, AWS IoT Greengrass supports only ''NewDeployment'' deployment types.
sbdInputFileURI :: Lens' StartBulkDeployment (Maybe Text)
...
instance ToJSON StartBulkDeployment where
        toJSON StartBulkDeployment'{..}
          = object
              (catMaybes
                 [("ExecutionRoleArn" .=) <$> _sbdExecutionRoleARN,
                  ("InputFileUri" .=) <$> _sbdInputFileURI])
...
data StartBulkDeploymentResponse = StartBulkDeploymentResponse'
  { _srsBulkDeploymentARN :: !(Maybe Text)
  , _srsBulkDeploymentId  :: !(Maybe Text)
