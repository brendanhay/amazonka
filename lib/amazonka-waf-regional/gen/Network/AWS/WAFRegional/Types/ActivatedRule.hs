...
mkActivatedRule priority ruleId
  = ActivatedRule'{priority, ruleId, action = Core.Nothing,
                   excludedRules = Core.Nothing, overrideAction = Core.Nothing,
                   type' = Core.Nothing}
...
{-# INLINEABLE arExcludedRules #-}
{-# DEPRECATED excludedRules "Use generic-lens or generic-optics with 'excludedRules' instead"  #-}
...
{-# INLINEABLE arOverrideAction #-}
{-# DEPRECATED overrideAction "Use generic-lens or generic-optics with 'overrideAction' instead"  #-}
...
instance Core.FromJSON ActivatedRule where
        parseJSON
          = Core.withObject "ActivatedRule" Core.$
              \ x ->
                ActivatedRule' Core.<$>
                  (x Core..: "Priority") Core.<*> x Core..: "RuleId" Core.<*>
                    x Core..:? "Action"
