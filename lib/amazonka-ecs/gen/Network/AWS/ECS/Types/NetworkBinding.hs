...
mkNetworkBinding
    :: NetworkBinding
mkNetworkBinding
  = NetworkBinding'{bindIP = Core.Nothing,
                    containerPort = Core.Nothing, hostPort = Core.Nothing,
...
nbBindIP :: Lens.Lens' NetworkBinding (Core.Maybe Core.Text)
nbBindIP = Lens.field @"bindIP"
{-# INLINEABLE nbBindIP #-}
{-# DEPRECATED bindIP "Use generic-lens or generic-optics with 'bindIP' instead"  #-}
...
instance Core.FromJSON NetworkBinding where
        parseJSON
          = Core.withObject "NetworkBinding" Core.$
              \ x ->
                NetworkBinding' Core.<$>
                  (x Core..:? "bindIP") Core.<*> x Core..:? "containerPort" Core.<*>
                    x Core..:? "hostPort"
