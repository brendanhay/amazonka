...
-- | The description for the status code.
spotInstanceStatus_message :: Lens.Lens' SpotInstanceStatus (Core.Maybe Core.Text)
...
-- <https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/spot-bid-status.html#spot-instance-bid-status-understand Spot status codes>
-- in the /Amazon EC2 User Guide for Linux Instances/.
spotInstanceStatus_code :: Lens.Lens' SpotInstanceStatus (Core.Maybe Core.Text)
spotInstanceStatus_code = Lens.lens (\SpotInstanceStatus' {code} -> code) (\s@SpotInstanceStatus' {} a -> s {code = a} :: SpotInstanceStatus)

-- | The date and time of the most recent status update, in UTC format (for
-- example, /YYYY/-/MM/-/DD/T/HH/:/MM/:/SS/Z).
spotInstanceStatus_updateTime :: Lens.Lens' SpotInstanceStatus (Core.Maybe Core.UTCTime)
spotInstanceStatus_updateTime = Lens.lens (\SpotInstanceStatus' {updateTime} -> updateTime) (\s@SpotInstanceStatus' {} a -> s {updateTime = a} :: SpotInstanceStatus) Core.. Lens.mapping Core._Time
...
instance Core.Hashable SpotInstanceStatus

instance Core.NFData SpotInstanceStatus
