...
mkResourceTagMapping
    :: ResourceTagMapping
mkResourceTagMapping
  = ResourceTagMapping'{complianceDetails = Core.Nothing,
                        resourceARN = Core.Nothing, tags = Core.Nothing}
...
{-# INLINEABLE rtmComplianceDetails #-}
{-# DEPRECATED complianceDetails "Use generic-lens or generic-optics with 'complianceDetails' instead"  #-}
...
instance Core.FromJSON ResourceTagMapping where
        parseJSON
          = Core.withObject "ResourceTagMapping" Core.$
              \ x ->
                ResourceTagMapping' Core.<$>
                  (x Core..:? "ComplianceDetails") Core.<*> x Core..:? "ResourceARN"
                    Core.<*> x Core..:? "Tags"
