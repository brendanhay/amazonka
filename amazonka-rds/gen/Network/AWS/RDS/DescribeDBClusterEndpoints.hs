...
data DescribeDBClusterEndpoints = DescribeDBClusterEndpoints'
  { _ddbceDBClusterIdentifier         :: !(Maybe Text)
  , _ddbceFilters                     :: !(Maybe [Filter])
  , _ddbceDBClusterEndpointIdentifier :: !(Maybe Text)
...
--
-- * 'ddbceMaxRecords' - The maximum number of records to include in the response. If more records exist than the specified @MaxRecords@ value, a pagination token called a marker is included in the response so that the remaining results can be retrieved.  Default: 100 Constraints: Minimum 20, maximum 100.
describeDBClusterEndpoints
    :: DescribeDBClusterEndpoints
describeDBClusterEndpoints =
  DescribeDBClusterEndpoints'
...
instance AWSPager DescribeDBClusterEndpoints where
        page rq rs
          ...
          | otherwise =
            Just $ rq & ddbceMarker .~ rs ^. ddcersMarker
...
instance Hashable DescribeDBClusterEndpoints where

instance NFData DescribeDBClusterEndpoints where
        ...
        toQuery DescribeDBClusterEndpoints'{..}
          = mconcat
              ["Action" =:
               ...
               "Filters" =:
                 toQuery (toQueryList "Filter" <$> _ddbceFilters),
...
data DescribeDBClusterEndpointsResponse = DescribeDBClusterEndpointsResponse'
  { _ddcersDBClusterEndpoints :: !(Maybe [DBClusterEndpoint])
  , _ddcersMarker             :: !(Maybe Text)
  , _ddcersResponseStatus     :: !Int
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
...
describeDBClusterEndpointsResponse
    :: Int -- ^ 'ddcersResponseStatus'
    -> DescribeDBClusterEndpointsResponse
describeDBClusterEndpointsResponse pResponseStatus_ =
  DescribeDBClusterEndpointsResponse'
    { _ddcersDBClusterEndpoints = Nothing
