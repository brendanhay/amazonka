...
--
-- * 'filters' - Used to dynamically select a set of devices for a test run. A filter is made up of an attribute, an operator, and one or more values.
--
--
--     * __Attribute__
-- The aspect of a device such as platform or model used as the selection criteria in a device filter.
...
--
--
-- * 'maxDevices' - The maximum number of devices to be included in a test run.
mkDeviceSelectionConfiguration ::
  -- | 'maxDevices'
  Lude.Int ->
  DeviceSelectionConfiguration
mkDeviceSelectionConfiguration pMaxDevices_ =
  DeviceSelectionConfiguration'
    { filters = Lude.mempty,
...
dscFilters :: Lens.Lens' DeviceSelectionConfiguration [DeviceFilter]
dscFilters = Lens.lens (filters :: DeviceSelectionConfiguration -> [DeviceFilter]) (\s a -> s {filters = a} :: DeviceSelectionConfiguration)
...
dscMaxDevices :: Lens.Lens' DeviceSelectionConfiguration Lude.Int
dscMaxDevices = Lens.lens (maxDevices :: DeviceSelectionConfiguration -> Lude.Int) (\s a -> s {maxDevices = a} :: DeviceSelectionConfiguration)
{-# DEPRECATED dscMaxDevices "Use generic-lens or generic-optics with 'maxDevices' instead." #-}

instance Lude.ToJSON DeviceSelectionConfiguration where
  toJSON DeviceSelectionConfiguration' {..} =
    Lude.object
      ( Lude.catMaybes
          [ Lude.Just ("filters" Lude..= filters),
            Lude.Just ("maxDevices" Lude..= maxDevices)
          ]
      )
