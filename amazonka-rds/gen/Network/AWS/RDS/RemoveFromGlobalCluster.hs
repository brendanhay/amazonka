...
data RemoveFromGlobalCluster = RemoveFromGlobalCluster'
  { _rfgcDBClusterIdentifier     :: !(Maybe Text)
  , _rfgcGlobalClusterIdentifier :: !(Maybe Text)
...
removeFromGlobalCluster =
  RemoveFromGlobalCluster'
    {_rfgcDBClusterIdentifier = Nothing, _rfgcGlobalClusterIdentifier = Nothing}
...
instance AWSRequest RemoveFromGlobalCluster where
        type Rs RemoveFromGlobalCluster =
             RemoveFromGlobalClusterResponse
        request = postQuery rds
        response
          = receiveXMLWrapper "RemoveFromGlobalClusterResult"
              (\ s h x ->
                 RemoveFromGlobalClusterResponse' <$>
                   (x .@? "GlobalCluster") <*> (pure (fromEnum s)))

instance Hashable RemoveFromGlobalCluster where

instance NFData RemoveFromGlobalCluster where
...
instance ToQuery RemoveFromGlobalCluster where
        toQuery RemoveFromGlobalCluster'{..}
          = mconcat
              ["Action" =:
                 ("RemoveFromGlobalCluster" :: ByteString),
...
data RemoveFromGlobalClusterResponse = RemoveFromGlobalClusterResponse'
  { _rfgcrsGlobalCluster  :: !(Maybe GlobalCluster)
  , _rfgcrsResponseStatus :: !Int
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
...
removeFromGlobalClusterResponse
    :: Int -- ^ 'rfgcrsResponseStatus'
    -> RemoveFromGlobalClusterResponse
removeFromGlobalClusterResponse pResponseStatus_ =
  RemoveFromGlobalClusterResponse'
    {_rfgcrsGlobalCluster = Nothing, _rfgcrsResponseStatus = pResponseStatus_}
...
instance NFData RemoveFromGlobalClusterResponse where
