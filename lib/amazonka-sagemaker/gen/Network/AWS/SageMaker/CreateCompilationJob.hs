...
--
-- * 'ccjTags' - An array of key-value pairs that you want to use to organize and track your AWS resource costs. For more information, see <https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/cost-alloc-tags.html#allocation-what Using Cost Allocation Tags> in the /AWS Billing and Cost Management User Guide/ .
--
...
instance ToJSON CreateCompilationJob where
  toJSON CreateCompilationJob' {..} =
    object
      ( catMaybes
          [ ("Tags" .=) <$> _ccjTags,
            Just ("CompilationJobName" .= _ccjCompilationJobName),
