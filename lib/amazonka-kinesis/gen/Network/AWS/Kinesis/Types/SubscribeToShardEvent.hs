...
mkSubscribeToShardEvent
    :: Types.SequenceNumber -- ^ 'continuationSequenceNumber'
    -> Core.Natural -- ^ 'millisBehindLatest'
    -> SubscribeToShardEvent
mkSubscribeToShardEvent continuationSequenceNumber
  millisBehindLatest
  = SubscribeToShardEvent'{records = Core.mempty,
...
{-# INLINEABLE stseContinuationSequenceNumber #-}
{-# DEPRECATED continuationSequenceNumber "Use generic-lens or generic-optics with 'continuationSequenceNumber' instead"  #-}
...
{-# INLINEABLE stseMillisBehindLatest #-}
...
instance Core.FromJSON SubscribeToShardEvent where
        parseJSON
          = Core.withObject "SubscribeToShardEvent" Core.$
              \ x ->
                SubscribeToShardEvent' Core.<$>
                  (x Core..:? "Records" Core..!= Core.mempty) Core.<*>
                    x Core..: "ContinuationSequenceNumber"
