...
instance AWSPager DescribeDBParameterGroups where
  page rq rs
    ...
    | otherwise = Just $ rq & ddpgMarker .~ rs ^. ddpgrsMarker
...
instance ToQuery DescribeDBParameterGroups where
  toQuery DescribeDBParameterGroups' {..} =
    mconcat
      [ "Action" =: ("DescribeDBParameterGroups" :: ByteString),
        "Version" =: ("2014-10-31" :: ByteString),
        "Filters" =: toQuery (toQueryList "Filter" <$> _ddpgFilters),
...
data DescribeDBParameterGroupsResponse = DescribeDBParameterGroupsResponse'
  { _ddpgrsMarker ::
      !(Maybe Text),
    _ddpgrsDBParameterGroups ::
      !( Maybe
           [DBParameterGroup]
       ),
    _ddpgrsResponseStatus ::
      !Int
  }
  deriving (Eq, Read, Show, Data, Typeable, Generic)
