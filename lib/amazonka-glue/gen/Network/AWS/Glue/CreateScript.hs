...
mkCreateScript
    :: CreateScript
mkCreateScript
  = CreateScript'{dagEdges = Core.Nothing, dagNodes = Core.Nothing,
...
instance Core.ToHeaders CreateScript where
        toHeaders CreateScript{..}
          = Core.pure ("X-Amz-Target", "AWSGlue.CreateScript") Core.<>
...
mkCreateScriptResponse responseStatus
  = CreateScriptResponse'{pythonScript = Core.Nothing,
