...
--
-- Deletes a <http://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html custom key store> . This operation does not delete the AWS CloudHSM cluster that is associated with the custom key store, or affect any users or keys in the cluster.
--
--
-- The custom key store that you delete cannot contain any AWS KMS <http://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#master_keys customer master keys (CMKs)> . Before deleting the key store, verify that you will never need to use any of the CMKs in the key store for any cryptographic operations. Then, use 'ScheduleKeyDeletion' to delete the AWS KMS customer master keys (CMKs) from the key store. When the scheduled waiting period expires, the @ScheduleKeyDeletion@ operation deletes the CMKs. Then it makes a best effort to delete the key material from the associated cluster. However, you might need to manually <http://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-orphaned-key delete the orphaned key material> from the cluster and its backups.
--
...
deleteCustomKeyStoreResponse pResponseStatus_ =
  DeleteCustomKeyStoreResponse' {_delrsResponseStatus = pResponseStatus_}
