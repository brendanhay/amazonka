...
-- values. For example, if you have 'owner' and 'environment' for tag keys,
-- and 'admin' and 'test' for tag values, all HSM client certificates that
-- have any combination of those values are returned. If both tag keys and
-- values are omitted from the request, HSM client certificates are returned
