...
instance AWSRequest DescribeCertificateAuthorityAuditReport where
  type
...
instance Hashable DescribeCertificateAuthorityAuditReport

instance NFData DescribeCertificateAuthorityAuditReport

instance ToHeaders DescribeCertificateAuthorityAuditReport where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
...
instance ToJSON DescribeCertificateAuthorityAuditReport where
  toJSON DescribeCertificateAuthorityAuditReport' {..} =
    object
      ( catMaybes
          [ Just ("CertificateAuthorityArn" .= _dcaarCertificateAuthorityARN),
            Just ("AuditReportId" .= _dcaarAuditReportId)
          ]
      )

instance ToPath DescribeCertificateAuthorityAuditReport where
  toPath = const "/"

instance ToQuery DescribeCertificateAuthorityAuditReport where
...
instance NFData DescribeCertificateAuthorityAuditReportResponse
