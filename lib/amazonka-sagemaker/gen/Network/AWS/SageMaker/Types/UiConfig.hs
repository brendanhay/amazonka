...
mkUiConfig
    :: UiConfig
mkUiConfig
  = UiConfig'{humanTaskUiArn = Core.Nothing,
...
{-# INLINEABLE ucUiTemplateS3Uri #-}
{-# DEPRECATED uiTemplateS3Uri "Use generic-lens or generic-optics with 'uiTemplateS3Uri' instead"  #-}
...
instance Core.FromJSON UiConfig where
        parseJSON
          = Core.withObject "UiConfig" Core.$
              \ x ->
                UiConfig' Core.<$>
                  (x Core..:? "HumanTaskUiArn") Core.<*> x Core..:? "UiTemplateS3Uri"
