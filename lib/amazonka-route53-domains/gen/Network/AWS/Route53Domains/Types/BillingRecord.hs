...
mkBillingRecord
    :: BillingRecord
mkBillingRecord
  = BillingRecord'{billDate = Core.Nothing,
                   domainName = Core.Nothing, invoiceId = Core.Nothing,
                   operation = Core.Nothing, price = Core.Nothing}
...
{-# INLINEABLE brBillDate #-}
{-# DEPRECATED billDate "Use generic-lens or generic-optics with 'billDate' instead"  #-}
...
{-# INLINEABLE brInvoiceId #-}
{-# DEPRECATED invoiceId "Use generic-lens or generic-optics with 'invoiceId' instead"  #-}
...
instance Core.FromJSON BillingRecord where
        parseJSON
          = Core.withObject "BillingRecord" Core.$
              \ x ->
                BillingRecord' Core.<$>
                  (x Core..:? "BillDate") Core.<*> x Core..:? "DomainName" Core.<*>
                    x Core..:? "InvoiceId"
