...
data RegisterTaskDefinition = RegisterTaskDefinition'
  { _rtdInferenceAccelerators ::
    ...
    _rtdRequiresCompatibilities ::
      !(Maybe [Compatibility]),
    _rtdPidMode :: !(Maybe PidMode),
    _rtdIpcMode :: !(Maybe IPcMode),
...
instance ToJSON RegisterTaskDefinition where
  toJSON RegisterTaskDefinition' {..} =
    object
      ( catMaybes
          [ ("inferenceAccelerators" .=) <$> _rtdInferenceAccelerators,
            ("executionRoleArn" .=) <$> _rtdExecutionRoleARN,
            ("requiresCompatibilities" .=) <$> _rtdRequiresCompatibilities,
            ("pidMode" .=) <$> _rtdPidMode,
            ("ipcMode" .=) <$> _rtdIpcMode,
            ("memory" .=) <$> _rtdMemory,
            ("proxyConfiguration" .=) <$> _rtdProxyConfiguration,
            ("taskRoleArn" .=) <$> _rtdTaskRoleARN,
            ("placementConstraints" .=) <$> _rtdPlacementConstraints,
            ("networkMode" .=) <$> _rtdNetworkMode,
            ("volumes" .=) <$> _rtdVolumes,
            ("cpu" .=) <$> _rtdCpu,
            ("tags" .=) <$> _rtdTags,
            Just ("family" .= _rtdFamily),
            Just ("containerDefinitions" .= _rtdContainerDefinitions)
          ]
      )
