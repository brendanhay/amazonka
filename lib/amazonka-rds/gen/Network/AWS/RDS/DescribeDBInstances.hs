...
--
-- * 'ddbiMaxRecords' - The maximum number of records to include in the response. If more records exist than the specified @MaxRecords@ value, a pagination token called a marker is included in the response so that you can retrieve the remaining results.  Default: 100 Constraints: Minimum 20, maximum 100.
describeDBInstances ::
  DescribeDBInstances
describeDBInstances =
  DescribeDBInstances'
...
instance AWSPager DescribeDBInstances where
  page rq rs
    ...
    | otherwise = Just $ rq & ddbiMarker .~ rs ^. ddbirsMarker
  ...
  toQuery DescribeDBInstances' {..} =
    mconcat
      [ "Action" =: ("DescribeDBInstances" :: ByteString),
        ...
        "Filters" =: toQuery (toQueryList "Filter" <$> _ddbiFilters),
