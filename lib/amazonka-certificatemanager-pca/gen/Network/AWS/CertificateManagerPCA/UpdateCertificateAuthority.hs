...
instance Hashable UpdateCertificateAuthority

instance NFData UpdateCertificateAuthority
...
instance ToHeaders UpdateCertificateAuthority where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target"
              =# ("ACMPrivateCA.UpdateCertificateAuthority" :: ByteString),
...
instance ToJSON UpdateCertificateAuthority where
  toJSON UpdateCertificateAuthority' {..} =
    object
      ( catMaybes
          [ ("Status" .=) <$> _ucaStatus,
            ("RevocationConfiguration" .=) <$> _ucaRevocationConfiguration,
            Just ("CertificateAuthorityArn" .= _ucaCertificateAuthorityARN)
          ]
      )
