...
--
-- * 'cDefaultRootDeviceType' - The default root device type. This value is used by default for all instances in the cloned stack, but you can override it when you create an instance. For more information, see <https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ComponentsAMIs.html#storage-for-the-root-device Storage for the Root Device> .
--
...
--
-- * 'cRegion' - The cloned stack AWS region, such as "ap-northeast-2". For more information about AWS regions, see <https://docs.aws.amazon.com/general/latest/gr/rande.html Regions and Endpoints> .
--
...
instance AWSRequest CloneStack where
  ...
  response =
    receiveJSON
      ( \s h x ->
          CloneStackResponse' <$> (x .?> "StackId") <*> (pure (fromEnum s))
      )
...
instance ToHeaders CloneStack where
  toHeaders =
    const
      ( mconcat
          [ "X-Amz-Target" =# ("OpsWorks_20130218.CloneStack" :: ByteString),
...
instance ToJSON CloneStack where
  toJSON CloneStack' {..} =
    object
      ( catMaybes
          [ ("DefaultInstanceProfileArn" .=) <$> _cDefaultInstanceProfileARN,
            ...
            ("DefaultAvailabilityZone" .=) <$> _cDefaultAvailabilityZone,
            ("Attributes" .=) <$> _cAttributes,
            ("Name" .=) <$> _cName,
            ("DefaultOs" .=) <$> _cDefaultOS,
            ("UseOpsworksSecurityGroups" .=) <$> _cUseOpsworksSecurityGroups,
