...
instance AWSRequest RemoveListenerCertificates where
  ...
  response =
    receiveXMLWrapper
      ...
      ( \s h x ->
          RemoveListenerCertificatesResponse' <$> (pure (fromEnum s))
      )
...
instance ToQuery RemoveListenerCertificates where
  toQuery RemoveListenerCertificates' {..} =
    mconcat
      [ "Action" =: ("RemoveListenerCertificates" :: ByteString),
        "Version" =: ("2015-12-01" :: ByteString),
        "ListenerArn" =: _rlcListenerARN,
        "Certificates" =: toQueryList "member" _rlcCertificates
      ]
