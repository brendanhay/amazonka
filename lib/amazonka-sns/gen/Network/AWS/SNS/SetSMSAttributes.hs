...
--
--
-- You can override some of these settings for a single message when you use the @Publish@ action with the @MessageAttributes.entry.N@ parameter. For more information, see <http://docs.aws.amazon.com/sns/latest/dg/sms_publish-to-phone.html Sending an SMS Message> in the /Amazon SNS Developer Guide/ .
--
module Network.AWS.SNS.SetSMSAttributes
    (
    -- * Creating a Request
...
instance AWSRequest SetSMSAttributes where
        ...
        response
          = receiveXMLWrapper "SetSMSAttributesResult"
              (\ s h x ->
                 SetSMSAttributesResponse' <$> (pure (fromEnum s)))

instance Hashable SetSMSAttributes where

instance NFData SetSMSAttributes where
        ...
        toQuery SetSMSAttributes'{..}
          = mconcat
              ["Action" =: ("SetSMSAttributes" :: ByteString),
               ...
               "attributes" =:
                 toQueryMap "entry" "key" "value" _ssmsaAttributes]
...
instance NFData SetSMSAttributesResponse where
