...
mkImage
    :: Image
mkImage
  = Image'{imageId = Core.Nothing, imageManifest = Core.Nothing,
           imageManifestMediaType = Core.Nothing, registryId = Core.Nothing,
...
{-# INLINEABLE iImageManifestMediaType #-}
...
instance Core.FromJSON Image where
        parseJSON
          = Core.withObject "Image" Core.$
              \ x ->
                Image' Core.<$>
                  (x Core..:? "imageId") Core.<*> x Core..:? "imageManifest" Core.<*>
                    x Core..:? "imageManifestMediaType"
