...
data DescribeDBInstanceAutomatedBackups = DescribeDBInstanceAutomatedBackups'
  { _ddbiabFilters              :: !(Maybe [Filter])
  , _ddbiabDBInstanceIdentifier :: !(Maybe Text)
  , _ddbiabMarker               :: !(Maybe Text)
  , _ddbiabMaxRecords           :: !(Maybe Int)
  , _ddbiabDBiResourceId        :: !(Maybe Text)
...
--
-- * 'ddbiabMaxRecords' - The maximum number of records to include in the response. If more records exist than the specified @MaxRecords@ value, a pagination token called a marker is included in the response so that the remaining results can be retrieved.
--
...
instance AWSPager DescribeDBInstanceAutomatedBackups
         where
        page rq rs
          | stop (rs ^. ddiabrsMarker) = Nothing
          | stop (rs ^. ddiabrsDBInstanceAutomatedBackups) =
            Nothing
          | otherwise =
            Just $ rq & ddbiabMarker .~ rs ^. ddiabrsMarker
...
instance ToHeaders DescribeDBInstanceAutomatedBackups
         where
        toHeaders = const mempty

instance ToPath DescribeDBInstanceAutomatedBackups
         where
        toPath = const "/"

instance ToQuery DescribeDBInstanceAutomatedBackups
         where
        toQuery DescribeDBInstanceAutomatedBackups'{..}
          = mconcat
              ["Action" =:
               ...
               "Filters" =:
                 toQuery (toQueryList "Filter" <$> _ddbiabFilters),
...
data DescribeDBInstanceAutomatedBackupsResponse = DescribeDBInstanceAutomatedBackupsResponse'
  { _ddiabrsDBInstanceAutomatedBackups :: !(Maybe [DBInstanceAutomatedBackup])
  , _ddiabrsMarker                     :: !(Maybe Text)
  , _ddiabrsResponseStatus             :: !Int
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
...
describeDBInstanceAutomatedBackupsResponse
    :: Int -- ^ 'ddiabrsResponseStatus'
    -> DescribeDBInstanceAutomatedBackupsResponse
describeDBInstanceAutomatedBackupsResponse pResponseStatus_ =
  DescribeDBInstanceAutomatedBackupsResponse'
    { _ddiabrsDBInstanceAutomatedBackups = Nothing
