...
-- @DELETING@ state until the stream deletion is complete. When you delete a
-- stream, any shards in that stream are also deleted, and any tags are
-- dissociated from the stream. You can use the DescribeStream> operation to
-- check the state of the stream, which is returned in @StreamStatus@.
